...
test("기본 이미지 제작은 별도 설명 문구 생성 없이 Codex 직접 생성만 실행하고 내부 입력은 공개 응답에서 제거한다", async () => {
  ...
  const archiveUi = await read("app/components/creative-archive/CreativeArchiveWorkspace.tsx");
  const archiveCopyRoute = await read("app/api/creative-archive/[entryId]/ad-copy/route.ts");
  const legacyJobCopyRoute = await read("app/api/creative-generation/jobs/[jobId]/ad-copy/route.ts");
  const copyGenerator = await read("app/lib/ad-copy/adCopyGenerator.server.ts");
  assert.doesNotMatch(native, /ensureProductAdCopy/);
  assert.doesNotMatch(runner, /ensureProductAdCopy|planReferenceAdaptedCopies|ensureReferenceCopyPlanning/);
  assert.doesNotMatch(manualUi, /ProductAdCopyPanel|수정 문구로 전체 광고 재생성/);
  assert.match(manualUi, /codexDirectTest:/);
  assert.match(autoRunner, /createNativeGenerationJob/);
  assert.match(autoRunner, /codexDirectTest:/);
  assert.match(autoUi, /\/create-product\?view=results/);
  assert.doesNotMatch(autoUi, /task\.adCopy|광고 등록 문구|광고 문구와 제목을 만들고 있습니다/);
  assert.doesNotMatch(autoUi, /ProductAdCopyPanel/);
  assert.match(archiveUi, /문구·제목 생성/);
  assert.match(archiveUi, /\/api\/creative-archive\/\$\{encodeURIComponent\(entry\.id\)\}\/ad-copy/);
  assert.match(archiveCopyRoute, /ensureArchiveEntryAdCopy/);
  assert.match(legacyJobCopyRoute, /상품 전체 문구 생성은 종료되었습니다/);
  assert.doesNotMatch(legacyJobCopyRoute, /ensureProductAdCopy/);
  assert.match(copyGenerator, /archiveEntryId/);
  assert.match(copyGenerator, /responses\.create/);
  assert.match(copyGenerator, /type: "input_image"/);
  assert.match(copyGenerator, /store: false/);
  assert.match(copyGenerator, /CREATIVE_COPY_MODEL/);
  assert.match(copyGenerator, /ADATLAS_AD_COPY_REASONING/);
  assert.doesNotMatch(copyGenerator, /@openai\/codex-sdk|requireFreshCodexLocalChatGptLogin|type: "local_image"/);

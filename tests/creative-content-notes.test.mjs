...
import { applyCreativeContentNotesToCopy, contentNotePromptContext, resolveCreativeContentNotes, validateCreativeContentNotes } from "../app/lib/creative-content-notes/service.ts";
...
test("광고주→카테고리→상품→프로모션 범위를 모두 매칭하고 구체 범위를 우선한다", () => {
  const result = resolveCreativeContentNotes([note({ id: "advertiser", scope: "advertiser", scopeId: "adv" }), note({ id: "category", scope: "category", scopeId: "beauty" }), note({ id: "product", scope: "product", scopeId: "mint" }), note({ id: "promotion", scope: "promotion", scopeId: "summer" }), note({ id: "other", scope: "product", scopeId: "other" })], context);
  assert.deepEqual(
    result.notes.map((item) => item.id),
    ["promotion", "product", "category", "advertiser"]
  );
});

test("필수·금지 규칙은 일반 참고사항보다 항상 우선한다", () => {
  const result = resolveCreativeContentNotes([note({ id: "general-product", scope: "product", scopeId: "mint" }), note({ id: "required-advertiser", required: true, content: "브랜드명" })], context);
  assert.equal(result.notes[0].id, "required-advertiser");
});

test("비활성·시작 전·종료 후 참고사항은 적용하지 않는다", () => {
  const result = resolveCreativeContentNotes([note({ id: "inactive", active: false }), note({ id: "future", startsAt: "2026-09-01T00:00:00Z" }), note({ id: "expired", endsAt: "2026-07-01T00:00:00Z" }), note({ id: "active" })], context);
  assert.deepEqual(
    result.notes.map((item) => item.id),
    ["active"]
  );
...
test("같은 문구의 필수와 금지 참고사항 충돌을 차단 상태로 만든다", () => {
  const result = resolveCreativeContentNotes([note({ id: "must", type: "MUST_INCLUDE", content: "민트 쿨링", required: true }), note({ id: "ban", type: "PROHIBITED_EXPRESSION", content: "민트 쿨링", prohibited: true })], context);
...
test("금지 표현을 제거하고 필수 문구를 자동 추가한다", () => {
  const resolved = resolveCreativeContentNotes([note({ id: "must", type: "MUST_INCLUDE", content: "오리지널 소스", required: true }), note({ id: "ban", type: "PROHIBITED_EXPRESSION", content: "100% 효과", prohibited: true })], context).notes;
...
test("금지 참고사항은 ProductTruth의 차단 패턴에 포함된다", () => {
  ...
  const truth = buildProductTruth({
    product: {
      ...
      productImagePath: "/mint.jpg",
      backgroundImagePath: "",
      creativeContext: { advertiserId: "adv", productId: "mint", appliedContentNotes },
    },
    source: "landing-page",
  });
...
test("근거가 있는 추천 후킹과 USP·배경 참고사항이 6장 후킹 계획에 반영된다", () => {
  const appliedContentNotes = resolveCreativeContentNotes([note({ id: "usp", type: "PRODUCT_USP", content: "민트 사용감" }), note({ id: "review", type: "REVIEW_INSIGHT", content: "사용 후 산뜻하다는 후기" }), note({ id: "hook", type: "PREFERRED_HOOK", content: "review-ugc" }), note({ id: "background", type: "BACKGROUND_STYLE", content: "청량한 파란 배경" })], context).notes;
  const truth = buildProductTruth({
    product: {
      productName: "민트젤",
      category: "뷰티",
      ...
      productImagePath: "/mint.jpg",
      backgroundImagePath: "",
      creativeContext: { advertiserId: "adv", productId: "mint", recommendedHookTypes: ["price-value"], appliedContentNotes },
    },
    source: "landing-page",
  });
  ...
  const library = Array.from({ length: 6 }, (_, index) => ({
    ...
    contrast: "medium",
    orientation: "square",
    sourceType: "stock_photo",
    sourceName: "test",
    sourcePageUrl: "https://example.com",
    originalImageUrl: "https://example.com/bg.jpg",
  ...
  assert.equal(
    scenes.every((scene) => scene.reason.includes("청량한 파란 배경")),
    true
  );
});

test("근거 없는 후기 선호는 강제하지 않고 제외 후킹 없이 안전한 6개 가설을 만든다", () => {
  const appliedContentNotes = resolveCreativeContentNotes([note({ id: "preferred", type: "PREFERRED_HOOK", content: "review-ugc" }), note({ id: "avoided", type: "AVOIDED_HOOK", content: "긴급 후킹은 다음 제작에서 제외" })], context).notes;
  const truth = buildProductTruth({
    product: {
      productName: "민트젤",
      category: "뷰티",
  ...
  assert.equal(
    plan.hookPlans.some((hook) => hook.hookType === "review-ugc"),
    false
  );
  assert.equal(
    plan.hookPlans.some((hook) => hook.hookType === "urgency"),
    false
  );
});

test("동일 후킹의 선호·제외 규칙은 범위가 달라도 충돌로 표시한다", () => {
  const resolution = resolveCreativeContentNotes([note({ id: "preferred", scope: "category", scopeId: "beauty", type: "PREFERRED_HOOK", content: "urgency" }), note({ id: "avoided", scope: "product", scopeId: "mint", type: "AVOIDED_HOOK", content: "긴급 후킹은 다음 제작에서 제외", prohibited: true })], context);

...
function input(overrides = {}) {
  const creative = (index = 1) => ({
    hookCode: `H0${index}`,
    materialCode: `AT-ORS-65-H0${index}-T01`,
    mediaPath: `/creative-${index}.jpg`,
    mediaType: "image",
    mediaRatio: "1:1",
    primaryText: `문구 ${index}`,
    headline: `후킹 ${index}`,
    description: "확인된 상품 근거",
    landingUrl: "https://shop.example.com/product/65",
    utm: `utm_source=meta&utm_content=H0${index}`,
    approved: true,
  });
  return {
    requestKey: "adv:product:T01:H01",
    advertiserId: "adv",
    advertiserName: "광고주",
    productId: "65",
    productName: "상품",
    testRound: 1,
    testType: "creative-combination",
    archiveEntryIds: ["asset:one"],
    adAccount: { id: "act_1", name: "계정", currency: "USD", timezoneName: "Asia/Seoul" },
    campaign: { id: "cmp_1", accountId: "act_1", name: "판매", objective: "OUTCOME_SALES", status: "PAUSED", budgetMode: "ABO", specialAdCategories: [] },
    baselineAdSet: {
      id: "base_1",
      accountId: "act_1",
      campaignId: "cmp_1",
...
test("CBO, non-USD without approved budget, vertical mismatch, and unapproved media are blocked", () => {
  ...
  unapproved.creatives[0].approved = false;
...
test("confirmation token is payload-bound, one-time, and expires", () => {
  ...
  const expired = service.issue(payload);
  now = 2_000;
...
test("Graph provider does not retry 4xx and retries transient 5xx at most three times", async () => {
  ...
  try {
    globalThis.fetch = async () => {
      calls += 1;
      return new Response(JSON.stringify({ error: { message: "bad request", code: 100 } }), { status: 400 });
    };
    ...
    globalThis.fetch = async () => {
      calls += 1;
      return new Response(JSON.stringify({ error: { message: "temporary", is_transient: true } }), { status: 500 });
    };
    await assert.rejects(provider.request({ operation: "accounts", method: "GET", path: "me/adaccounts" }));
...
test("mock registration is sequential, idempotent, and never creates ACTIVE", async () => {
  ...
  try {
    const provider = new MockMetaProvider({
      "adset.create": { id: "set_new" },
      "media.upload": { hash: "media_hash" },
      "creative.create": { id: "creative_new" },
      "ad.create": { id: "ad_new" },
      adsets: { id: "set_new", status: "PAUSED", daily_budget: 500, campaign_id: "cmp_1" },
      ads: { id: "ad_new", status: "PAUSED", adset_id: "set_new" },
    });
    ...
    assert.deepEqual(
      ...
      ["adset.create", "media.upload", "creative.create", "ad.create", "adsets", "ads"]
    );
...
test("performance snapshots dedupe by ad/date and avoid division errors", () => {
  const experiment = {
    ...
    trackingStatus: "collecting",
    timezoneName: "UTC",
    attributionSetting: "7d",
    rows: [
      { hookCode: "H01", materialCode: "AT-H01", adId: "ad1", adName: "H01", impressions: 0, spend: 0, outboundClicks: 0, landingPageViews: 0, purchases: 0, purchaseValue: 0, ctr: 0, cpc: 0, cpa: 0, roas: 0, spendShare: 0, status: "" },
      { hookCode: "H02", materialCode: "AT-H02", adId: "ad2", adName: "H02", impressions: 0, spend: 0, outboundClicks: 0, landingPageViews: 0, purchases: 0, purchaseValue: 0, ctr: 0, cpc: 0, cpa: 0, roas: 0, spendShare: 0, status: "" },
    ],
  };
  ...
  const updated = aggregateMetaPerformance(
    experiment,
    [
      { ...base, adId: "ad1", impressions: 1000, spend: 90, outboundClicks: 0, purchases: 0, purchaseValue: 0 },
      { ...base, adId: "ad1", impressions: 1000, spend: 90, outboundClicks: 0, purchases: 0, purchaseValue: 0 },
      { ...base, adId: "ad2", impressions: 1000, spend: 10, outboundClicks: 0, purchases: 0, purchaseValue: 0 },
    ],
    { impressions: 1, outboundClicks: 0, purchases: 0, spend: 1 }
  );

...
import { aggregateProductMetrics, deduplicateDailyMetrics, isoDateShift } from "../app/lib/crema-market/aggregation.ts";
...
import { normalizeDate, normalizeMetricRows, normalizeProductRows, normalizeWorkbookRows, nullableNumber } from "../app/lib/crema-market/normalizer.ts";
import { runOpportunityAnalysis } from "../app/lib/crema-market/opportunityEngine.ts";
import { averageNullable, percentile, safeChangeRate, safeDivide, smoothedConversionRate, sumNullable, weightedAvailableScore } from "../app/lib/crema-market/math.ts";
...
test("상품이나 날짜가 연결되지 않는 지표는 경고 후 제외한다", () => {
  ...
  const result = normalizeMetricRows({
    advertiserId: "adv",
    products,
    provider: "file_upload",
    rows: [
      { 상품코드: "B", 날짜: "2026-08-01", 조회수: 4 },
      { 상품코드: "A", 날짜: "", 조회수: 3 },
    ],
  });
...
test("중복 일별 지표는 합산하고 업로드 출처를 우선 기록한다", () => {
  const base = {
    ...
    stockCount: null,
    reviewCount: null,
    photoReviewCount: null,
    ratingSum: null,
    ratingCount: null,
  };
  const result = deduplicateDailyMetrics([
    { ...base, source: "crema_api" },
    { ...base, views: 6, paidOrders: 2, source: "file_upload" },
  ]);
...
test("개발 fixture는 실제 출처와 명확히 구분되고 28일 상품 지표를 제공한다", () => {
  ...
  assert.equal(
    fixture.products.every((product) => Object.values(product.provenance).every((source) => source === "development_fixture")),
    true
  );
...
test("상품당 하나의 주 후보와 중복 없는 보조 후보 유형을 저장한다", () => {
  ...
  assert.equal(
    result.opportunities.every((item) => item.primaryType === item.type),
    true
  );
  assert.equal(
    result.opportunities.every((item) => new Set(item.secondaryTypes).size === item.secondaryTypes.length),
    true
  );
  assert.equal(
    result.opportunities.every((item) => item.analysisPeriodStart && item.comparisonPeriodStart),
    true
  );
});

test("취소·환불을 제외한 순 주문·순매출을 분석 집계에 사용한다", () => {
  const imported = normalizeWorkbookRows({
    advertiserId: "adv",
    advertiserName: "광고주",
    productRows: [{ 상품코드: "A", 상품명: "A", 상품URL: "https://example.com/a" }],
    metricRows: [{ 상품코드: "A", 날짜: "2026-08-01", 결제주문수: 10, 매출: 100000, 취소주문: 2, 취소금액: 20000, 환불건수: 1, 환불금액: 10000 }],
  });
...
test("통합 정규화는 후기 요약을 원문 개인정보 없이 인사이트로 저장한다", () => {
  const imported = normalizeWorkbookRows({
    advertiserId: "adv",
    advertiserName: "광고주",
    productRows: [{ 상품코드: "A", 상품명: "A" }],
    metricRows: [{ 상품코드: "A", 날짜: "2026-08-01", 주문: 1 }],
    reviewRows: [{ 상품코드: "A", 후기주제: "향", 후기요약: "향이 산뜻함", 긍부정: "긍정", 후기수: 3 }],
  });

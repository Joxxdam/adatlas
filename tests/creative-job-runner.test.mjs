...
test("11-1. 다른 메뉴에서도 백그라운드 제작 진행률을 전역으로 표시한다", async () => {
  ...
  assert.match(indicator, /setActiveCreativeJobId\(\)/);
  assert.match(storage, /ACTIVE_CREATIVE_JOB_CHANGED_EVENT/);
  assert.match(storage, /window\.dispatchEvent\(new CustomEvent\(ACTIVE_CREATIVE_JOB_CHANGED_EVENT/);
  assert.match(client, /setActiveCreativeJobId\(payload\.job\.id\)/);
  assert.match(client, /!job && !loading/);
  assert.match(client, /광고 6장 등록 상태/);
  assert.match(client, /clearCurrentJobIdFromAddress/);
  assert.match(client, /function writeCurrentJobIdToAddress/);
  assert.match(client, /productScopedJobMatchesLoadedProduct/);
  assert.match(client, /완료된 광고 콘텐츠 작업을 불러왔습니다/);
  assert.match(client, /props\.analyzedProductUrl \|\| props\.product\.landingUrl/);
  assert.match(client, /addressedJobId && addressedJobId !== announcedJobId/);
  assert.match(publicJob, /function redactLocalPaths/);
  assert.match(publicJob, /localPathTestPattern = \/\^/);
  assert.doesNotMatch(publicJob, /\[A-Z\]:\[\\\\\/\]\)\[\^\\s"'\]\+\/gi/);
  assert.match(client, /addEventListener\(ACTIVE_CREATIVE_JOB_CHANGED_EVENT, restoreAnnouncedJob\)/);
  assert.match(client, /restoreStoredActiveJob/);
  assert.match(client, /window\.setInterval\(\(\) => void restoreStoredActiveJob\(\), 1_500\)/);
  assert.match(client, /requestedJobId/);
  assert.match(indicator, /addEventListener\(ACTIVE_CREATIVE_JOB_CHANGED_EVENT/);
  assert.match(indicator, /addEventListener\("storage"/);
  ...
  assert.match(indicator, /\?step=product&productUrl=/);
  assert.match(indicator, /&jobId=/);
  ...
  assert.match(client, /setActiveCreativeJobId\(\)/);

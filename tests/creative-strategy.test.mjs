...
test("자동 광고문구 파이프라인은 정확히 6개를 생성하고 첫 문구를 적용한다", async () => {
  const [strategySource, apiSource, copyApiSource, workflowSource, dashboardSource] =
    await Promise.all([
      readFile(path.join(projectRoot, "app/lib/mvp/creativeStrategy.ts"), "utf8"),
      readFile(path.join(projectRoot, "app/api/strategy/generate-directions/route.ts"), "utf8"),
      readFile(path.join(projectRoot, "app/api/strategy/generate-copy/route.ts"), "utf8"),
      readFile(
        path.join(projectRoot, "app/components/features/creative-workflow/useCreativeWorkflow.ts"),
        "utf8"
      ),
      readFile(path.join(projectRoot, "app/components/MvpDashboard.tsx"), "utf8"),
    ]);

...
for (const item of prompts) {
  ...
  const response = await fetch("https://api.openai.com/v1/images/generations", {
    ...
    body: JSON.stringify({ model: process.env.OPENAI_IMAGE_MODEL || "gpt-image-2", prompt, size: "1200x1200", quality: "high", background: "opaque", output_format: "png", n: 1 }),
    signal: AbortSignal.timeout(180_000),
  });
  if (!response.ok) {
    process.stderr.write(`FAIL ${item.id} HTTP ${response.status}\n`);
    continue;
  }
  ...
  if (!source) {
    process.stderr.write(`FAIL ${item.id} 이미지 데이터 없음\n`);
    continue;
  }
  ...
  current.push({
    ...
    licenseUrl: "",
    authorName: "AdAtlas",
    generationModel: process.env.OPENAI_IMAGE_MODEL || "gpt-image-2",

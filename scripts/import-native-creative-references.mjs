...
const layoutFamilies = ["problem-objection", "price-offer", "situation-story", "usp-evidence", "social-proof", "sensory-editorial"];
...
function compatibilityFor(categoryGroup, ordinal, layoutFamily) {
  ...
  const compositionType = naturalFood ? (["sensory-editorial", "situation-story"].includes(layoutFamily) ? "natural-food-scene" : layoutFamily === "price-offer" ? "price-card" : "product-packshot") : layoutFamily === "price-offer" ? "price-card" : layoutFamily === "social-proof" ? "review-card" : layoutFamily === "situation-story" ? "lifestyle-scene" : layoutFamily === "sensory-editorial" ? "sensory-closeup" : "product-packshot";
  return {
...
for (let index = 0; index < sources.length; index += 1) {
  ...
  await sharp(sources[index]).rotate().resize(1200, 1200, { fit: "contain", background: "#ffffff", withoutEnlargement: false }).flatten({ background: "#ffffff" }).toColorspace("srgb").jpeg({ quality: 88, progressive: true, mozjpeg: true }).toFile(outputPath);

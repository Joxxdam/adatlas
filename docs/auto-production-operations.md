...
`/admin/auto-production`은 기존 상품 분석, ProductTruth, 문구/HookPlan, 호환 ZIP 레퍼런스 배정, 원본 무손실 복사, 단계형 상품·문구 교체, QA와 소재코드 저장 흐름을 재사용하는 내부 운영 기능입니다. 광고 플랫폼 등록이나 집행은 자동으로 수행하지 않습니다.

...
`/auto-production`은 기존 상품 분석, ProductTruth, 문구/HookPlan, 상품 카테고리별 ZIP 풀의 중복 없는 무작위 레퍼런스 배정, 단계형 상품·문구 교체, QA와 소재코드 저장 흐름을 재사용하는 내부 운영 기능입니다. 광고 플랫폼 등록이나 집행은 자동으로 수행하지 않습니다.
...
- 광고주별 상품: 최대 4개
- 상품별 실행: 문구/HookPlan 6개와 중복 없는 상품군별 ZIP 레퍼런스 6장 전체
- 생성 흐름: 수동 제작과 동일한 레퍼런스 재현 → URL 상품 교체 → ProductTruth 문구 교체 → QA → 저장

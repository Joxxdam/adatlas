...
> 레거시·고급 관리 도구 문서입니다. 현재 `/create-product`의 ZIP 레퍼런스 광고 6장 기본 경로에서는 이 라이브러리로 배경을 추천하거나 자동 fallback하지 않습니다.

## 목적

상품 URL마다 배경 생성을 기다리지 않고, 검수된 공용 배경을 상품·후킹·타깃에 맞춰 추천해 1200×1200 광고를 빠르게 합성한다. 이 레거시 관리 도구 내부 흐름은 `상품 분석 → 후킹 3안 → 후킹 선택 → 배경 6안 → 배경 선택 → 적응형 레이아웃 3안 → 편집·생성`이다. AI 생성은 공용 자산을 미리 만들고 검수하는 관리 작업에만 사용한다.

...
→ 상세페이지 상품 레퍼런스 준비
→ AI가 글자와 상품이 없는 후킹별 장면 플레이트를 단발 생성
→ 원본 상품·정확한 한국어 문구·가격·도형·레이아웃을 로컬 합성
→ 로컬 규격·사실·문구 검수, 치명 오류일 때만 장면 1회 재생성
→ 1200×1200 JPEG 내보내기
→ 소재코드와 결과 저장

규칙:

- 새 native AI 작업에서는 기존 배경 라이브러리와 레거시 고정 템플릿 렌더러를 호출하지 않습니다. 단, 원본 상품과 정확한 한국어 문구를 보존하기 위한 native 전용 로컬 Sharp/SVG 합성기는 사용합니다.
- H01~H06은 문구만 바꾸는 변형이 아니라 서로 다른 후킹과 전체 장면을 갖습니다.
- Codex 로컬 로그인 엔진이 기본이며 유료 API로 자동 전환하지 않습니다. 후킹 계획은 상품 fingerprint 캐시를 우선 사용하고, 장면 생성은 후킹마다 영구 대화를 만들지 않는 일회성 실행입니다.
...
- 자동 6장 생성은 `ProductTruth → HookPlan → CreativeBrief → Text-free AI Scene → Local Product/Copy Composition → Local QA → Export` 순서를 유지합니다.
- 6장은 서로 다른 후킹과 독립적인 전체 광고 콘텐츠를 사용하며 한 장의 실패가 나머지 결과를 막지 않습니다.
- 상세페이지의 실제 상품·사용·질감 이미지는 상품 사실과 합성 원본으로 사용하며 기존 배경 라이브러리는 기본 생성 방식으로 사용하지 않습니다.
- AI 장면에는 글자·가격·CTA·로고·가짜 패키지를 생성하지 않습니다. 원본 상품과 한국어 문구는 native 전용 로컬 합성기로 정확히 배치합니다.

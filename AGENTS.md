...
- 단계 1은 레퍼런스 원본 파일을 확장자와 픽셀을 바꾸지 않고 `01-structure`로 바이트 동일 복사합니다. 이 단계에서 `provider.generate()` 또는 다른 생성형 편집을 호출하지 않습니다.
...
- Codex 로컬 로그인 엔진이 기본이며 유료 API로 자동 전환하지 않습니다. 문구 계획은 상품 fingerprint 캐시를 우선 사용하고, 각 편집 단계는 영구 대화를 저장하지 않는 일회성 실행입니다.

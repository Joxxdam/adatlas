...
→ 사용자가 상품과 전략 선택
→ 기존 소재 제작 화면으로 전달
→ 상품 카테고리와 같은 사용자 제공 ZIP 풀에서 광고 레퍼런스 6장 무작위 선택
...
→ 상품 카테고리와 같은 ZIP 풀에서 중복 없이 레퍼런스 6장 무작위 선택
...
- 상품 카테고리별 ZIP 풀에서 중복 없는 광고 레퍼런스 6장 무작위 선택(대기 레퍼런스는 기본 UI에 미리보기하지 않음)
- 한 번의 상품 확정 클릭 뒤 현재 제작 장수, 완료 x/6, 1~6 상태와 서버 러너 연결 상태 표시
- 서버 백그라운드 작업 생성·복원
- 레퍼런스별 디자인 재현·상품 교체·문구 교체·AI 검수
...
→ 상세페이지 상품 레퍼런스 준비
→ 상품 카테고리와 같은 사용자 제공 ZIP 풀에서 중복 없는 광고 레퍼런스 6장 무작위 선택
→ 선택 결과를 GenerationJob에 저장해 재시도·복구에도 고정
→ 선택 광고를 고해상도 작업 마스터로 충실히 재현
→ URL 상세페이지 원본을 기준으로 상품만 교체하고 비상품 영역 잠금
→ ProductTruth 한국어 문구·가격·혜택만 교체하고 상품·디자인 잠금
→ 원본 레퍼런스·상품 원본·정확 문구를 함께 비교하는 AI QA 및 최대 1회 단계 수정
→ 1200×1200 JPEG 내보내기
→ 소재코드와 결과 저장
...
- 새 수동·자동 작업의 레퍼런스 구분은 `패션 / 식품 / 화장품` 세 그룹만 사용합니다. 건강·웰니스와 퍼스널케어는 화장품에 포함합니다. 같은 풀에서 중복 없이 6장을 무작위 선택하고, 같은 풀이 부족할 때만 가까운 풀에서 보충합니다. 패션 풀이 부족할 때는 식품이 아니라 화장품 풀에서만 보충합니다.
...
- 단계 1은 레퍼런스의 구성·색·타이포·상품 수·배치·하단 띠를 작업용 중간 이미지에 최대한 유지합니다.
...
- 새 native AI 작업에서는 기존 배경 라이브러리, 장면 생성 provider, 레거시 고정 템플릿 렌더러, 로컬 상품/문구 합성기를 호출하지 않습니다.

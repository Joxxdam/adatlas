...
- HookPlan과 가격형/감각형 같은 카피 분류는 레퍼런스 선택 조건으로 사용하지 않습니다.
- 선택된 6장은 작업 생성 시 각 GenerationResult에 저장합니다. 새로고침, 서버 복구, 개별 재시도와 같은 레퍼런스 재생성에서는 다시 추첨하지 않습니다.
- H01~H06은 문구/성과 추적 코드로 유지할 수 있지만 시각 장면을 새로 설계하거나 레퍼런스를 선택하는 기준으로 사용하지 않습니다.
...
- Codex 로컬 로그인 엔진이 기본이며 유료 API로 자동 전환하지 않습니다. 문구 계획은 상품 fingerprint 캐시를 우선 사용합니다.
...
- 자동 6장 생성은 `ProductTruth → Copy/HookPlan → Category-matched ZIP References × 6 → Reference Recreation → Product-only Replacement → Copy-only Replacement → Reference/Product QA → Export` 순서를 유지합니다.
...
- 광고주별 기본 제작량은 상품 최대 4개 × 상품별 6장입니다. 상품 확정 뒤에는 수동 제작과 같은 `createNativeGenerationJob` 및 `reference-staged-edit` 엔진을 사용합니다.

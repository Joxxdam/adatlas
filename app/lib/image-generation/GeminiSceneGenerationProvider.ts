...
export class GeminiSceneGenerationProvider implements SceneGenerationProvider {
  ...
  async generateScene(input: SceneGenerationInput): Promise<SceneGenerationResult> {
    ...
    const prompt = [input.prompt, input.negativePrompt ? `Negative constraints: ${input.negativePrompt}` : "", "OUTPUT CONTRACT: return only one edge-to-edge, fully opaque 1:1 commercial photography plate. Fill all four corners and every pixel with a continuous environment.", "A product-safe zone is a low-detail photographed surface, not a transparent, black, blank, masked, boxed, or cutout area.", "Do not render the sold category, a substitute product, food, package, bottle, label, logo, price, readable text, or isolated hero object anywhere."].filter(Boolean).join("\n\n");

...
첫 문장부터 상품명을 설명하지 말고 실제 숏폼에서 사람이 멈춰 볼 사건이나 자연스러운 한마디로 시작한다. title은 ‘정체를 확인합니다’, ‘차이를 알아봅니다’ 같은 설명형 제목이 아니라 누가 어떤 상황에서 무슨 일을 겪는지 한눈에 보이는 사건형 제목으로 쓴다. 상품 사실은 바꾸지 않되 시장 장보기, 명절 선물 고민, 가족 식사처럼 널리 통용되는 생활 상황은 과감하게 창작한다. ProductTruth와 상세페이지 정보는 사용할 사실의 상한선이지 전부 자막에 넣어야 하는 체크리스트가 아니다. 콘셉트 하나는 핵심 구매 이유 하나와 이를 받치는 가격·구성·품질 사실 두세 개만 골라 전개하고 나머지는 버린다. 배송·배송비·도서산간·제주 추가비·배송지 안내는 기획 제목, 사건, 소구, CTA에서 완전히 제외한다. coreTarget은 분석용 고객 정의로 쓰고, targetCallout은 그 고객의 행동·불편·욕망을 찌르는 자연스러운 직접 호명으로 별도 작성한다. targetCallout은 ‘운동하는 남성’ 같은 일반 명사가 아니라 ‘추석 선물 고민 중인 분들, 잠깐’처럼 첫 3초 자막에 바로 쓸 수 있어야 한다. 첫 자막, 중심 사건, 화자 시점, 갈등 원인, 상품 등장 방식, 핵심 소구, 결말·CTA, 화면 스타일을 기존 기획안과 다르게 만든다. copyVoiceDirection에는 이 콘셉트에서 실제로 사용할 호칭·문장 길이·직설 강도를 구체적으로 적고 ‘친근한 말투’, ‘자연스러운 구어체’처럼 일반화하지 않는다. 각 콘셉트는 배정된 주 블루프린트의 전체 전개를 우선하고 보조 블루프린트에서는 훅 또는 CTA 장치 하나만 가져온다. 사건·상황극형은 서버가 지정한 세부 장르를 블루프린트보다 우선하며 다른 장르로 바꾸거나 혼합하지 않는다. 참고 영상은 사건 구조와 말투 장치를 현재 상품에 맞게 변환하되 원문 전체·특정 인물·상품 사실은 복제하지 않는다. 확인되지 않은 수치나 효능은 claimsToVerify에만 쓰고 확정 문구로 쓰지 않는다. hookId와 evidenceIds는 입력에 존재하는 값만 쓴다. 실제 이미지나 영상을 생성하지 않으며 상세 대본은 아직 만들지 않는다.
${archetypes?.includes("secret-benefit") && !hasVerifiedBenefit ? "secret-benefit 기획안은 확인된 혜택이 없으므로 benefitAvailability를 insufficient로 두고, keyAppeal과 narrativeSummary에는 ‘확인 가능한 혜택 정보가 부족합니다’를 포함하며 가격·할인·증정을 창작하지 않는다." : ""}
...
[레퍼런스 말투 전용 규칙]
${internetVoiceRules(input.analysis.category)}
[분석된 11개 기본 영상 레퍼런스에서 이 콘셉트에 배정된 블루프린트]

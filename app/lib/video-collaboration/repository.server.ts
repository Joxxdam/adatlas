...
import type { CreateVideoProjectInput, ReviewComment, VideoCollaborationStore, VideoConcept, VideoProject, VideoProjectStatus, VideoVersion, VideoHookCandidate, VideoPipelineProgress, ProductLockedAsset, ReferenceVideoAnalysis, VideoGenerationFailure } from "./types.ts";
...
import { assertVideoProjectTransition, createVideoMaterialCode, newHistoryId, validateVideoMaterialCode, videoProjectSummary } from "./workflow.ts";
...
const repositoryQueues = globalVideoQueues.__adAtlasVideoProjectQueues || new Map<string, Promise<void>>();
...
function normalizeConcept(concept: VideoConcept): VideoConcept {
  const cuts = Array.isArray(concept.cuts) ? concept.cuts.map((cut, index) => normalizeVideoCut(cut, index)) : [];
  return {
    ...concept,
    cuts,
...
function normalizeProject(project: VideoProject): VideoProject {
  ...
  return {
    ...
    conceptFormat: project.conceptFormat && VIDEO_CONCEPT_FORMATS.includes(project.conceptFormat) ? project.conceptFormat : undefined,
    ...
    concepts: Array.isArray(project.concepts) ? project.concepts.map((concept) => normalizeConcept(concept)) : [],
    ...
    designerAssignmentHistory: Array.isArray(project.designerAssignmentHistory) ? project.designerAssignmentHistory : [],
...
function conceptText(concept: VideoConcept) {
  return [concept.title, concept.coreTarget, concept.openingHook, concept.fullScript, concept.cta, ...concept.requiredSources, ...concept.cuts.flatMap((cut) => [cut.sceneDescription, cut.caption, cut.narration, ...cut.requiredSources])].join(" ");
}
...
function validateConceptForProject(project: VideoProject, concept: VideoConcept) {
  ...
  if (!clean(concept.title) || !clean(concept.openingHook) || !clean(concept.fullScript)) throw new Error("기획안 제목, 첫 3초 후킹, 전체 대본을 모두 입력해 주세요.");
  if (!clean(concept.cta)) throw new Error("마지막 CTA를 입력해 주세요.");
  if (!Array.isArray(concept.cuts) || concept.cuts.length < 1 || concept.cuts.length > 40) throw new Error("장면 구성은 1개부터 40개까지 저장할 수 있습니다.");
  const ordered = [...concept.cuts].sort((left, right) => left.cutNumber - right.cutNumber);
  for (let index = 0; index < ordered.length; index += 1) {
    const cut = ordered[index];
    if (!Number.isFinite(cut.startSecond) || !Number.isFinite(cut.endSecond) || cut.startSecond < 0 || cut.endSecond <= cut.startSecond || cut.endSecond > project.duration) {
      throw new Error(`컷 ${cut.cutNumber}의 시간 구간이 올바르지 않습니다.`);
    }
    if (index > 0 && cut.startSecond < ordered[index - 1].endSecond) throw new Error("컷 시간 구간이 서로 겹칩니다.");
    if (!clean(cut.sceneDescription) || !clean(cut.caption)) throw new Error(`컷 ${cut.cutNumber}의 장면 설명과 자막을 입력해 주세요.`);
    if (!Array.isArray(cut.referenceImages) || cut.referenceImages.length > 2) throw new Error(`장면 ${cut.cutNumber}의 참고 이미지는 최대 2개까지 저장할 수 있습니다.`);
    for (const image of cut.referenceImages) {
      if (!clean(image.id, 120) || !clean(image.filePath, 2400)) throw new Error(`장면 ${cut.cutNumber}의 참고 이미지 정보가 올바르지 않습니다.`);
      if (image.source === "external") {
        ...
        if (!["http:", "https:"].includes(referenceUrl.protocol)) throw new Error("외부 참고 이미지는 HTTP 또는 HTTPS URL만 지원합니다.");
  ...
  if (ordered.at(-1)?.endSecond !== project.duration) throw new Error(`마지막 컷의 종료 시간을 ${project.duration}초로 맞춰 주세요.`);
  const text = conceptText(concept);
  const forbidden = project.brandGuideline.forbiddenPhrases.find((phrase) => clean(phrase, 120) && text.includes(clean(phrase, 120)));
...
export function createVideoProjectRepository(options: { dataDirectory?: string } = {}) {
  const dataDirectory = options.dataDirectory || path.join(process.cwd(), "data", "video-collaboration");
  ...
  async function readStore(): Promise<VideoCollaborationStore> {
    try {
      const parsed = JSON.parse(await fs.readFile(storePath, "utf8")) as Partial<VideoCollaborationStore>;
      return {
        ...emptyStore(),
        projects: Array.isArray(parsed.projects) ? parsed.projects.map((project) => normalizeProject(project)) : [],
      };
    } catch (error) {
  ...
  return {
    async list() {
      const store = await readStore();
      return [...store.projects].sort((left, right) => right.updatedAt.localeCompare(left.updatedAt)).map(videoProjectSummary);
    },
    ...
    async create(input: CreateVideoProjectInput) {
      ...
      return locked((store) => {
        ...
        const project: VideoProject = {
          ...
          productOriginalAsset: input.productOriginalAsset ? clone(input.productOriginalAsset) : undefined,
    ...
    async updateDetails(projectId: string, changes: Partial<Pick<VideoProject, "projectName" | "advertiserName" | "marketerName" | "designerName" | "additionalRequests" | "requiredContent" | "excludedContent" | "productionNotes" | "deadline" | "productAnalysis" | "brandGuideline">>) {
      return update(projectId, (project) => {
        if (changes.projectName !== undefined) project.projectName = clean(changes.projectName, 140);
        if (changes.advertiserName !== undefined) project.advertiserName = clean(changes.advertiserName, 120);
        if (changes.marketerName !== undefined) project.marketerName = clean(changes.marketerName, 80) || "마케터";
        ...
        if (changes.additionalRequests !== undefined) project.additionalRequests = clean(changes.additionalRequests, 5000);
        if (changes.requiredContent !== undefined) project.requiredContent = clean(changes.requiredContent, 3000);
        if (changes.excludedContent !== undefined) project.excludedContent = clean(changes.excludedContent, 3000);
        if (changes.productionNotes !== undefined) project.productionNotes = clean(changes.productionNotes, 5000);
        if (changes.deadline !== undefined) project.deadline = clean(changes.deadline, 40);
        if (changes.productAnalysis) {
          if (!clean(changes.productAnalysis.productName)) throw new Error("분석 상품명을 입력해 주세요.");
    ...
    async saveConceptSummaries(
      ...
      return update(projectId, (project) => {
        ...
        if (project.planningMode === "four-concepts" && !options.replaceConceptId && (concepts.length !== 4 || !validateConceptDiversity(concepts).valid)) {
          throw new Error("패러디·리얼 사용·USP·시크릿 혜택의 서로 다른 기획안 4개가 필요합니다.");
        }
        if (project.planningMode !== "four-concepts" && !options.replaceConceptId && concepts.length > 1 && !validateConceptDiversity(concepts).valid) {
        ...
        if (project.status === "script_pending") {
          transition(project, "script_review", "시스템", concepts.length === 1 ? "선택한 영상 콘셉트 기획안 생성" : `후킹 평가 및 기획안 요약 ${concepts.length}개 생성`);
    ...
    async saveGenerationFailure(projectId: string, failure: VideoGenerationFailure, options: { conceptId?: string } = {}) {
      return update(projectId, (project) => {
    ...
    async saveGeneratedConcepts(
      ...
      return update(projectId, (project) => {
        ...
        for (const concept of concepts) {
          ...
          if (incomingCodes.has(concept.materialCode)) throw new Error("생성된 기획안 사이에 중복 소재코드가 있습니다.");
          incomingCodes.add(concept.materialCode);
        }
        if (options.conceptId) {
    ...
    async updateConcept(projectId: string, conceptId: string, concept: VideoConcept, actor: string) {
      return update(projectId, (project) => {
        if (!["script_review", "concept_selected"].includes(project.status)) throw new Error("대본 검토 중에만 기획안을 수정할 수 있습니다.");
    ...
    async saveScript(projectId: string, conceptId: string, concept: VideoConcept, actor: string, options: { productionNotes?: string; createRevision?: boolean } = {}) {
      return update(projectId, (project) => {
        if (project.status === "script_pending") throw new Error("먼저 영상 대본을 생성해 주세요.");
        if (project.status === "approved") throw new Error("최종 승인된 대본은 복제한 뒤 수정해 주세요.");
        const index = project.concepts.findIndex((item) => item.id === conceptId);
        const current = project.finalScript?.id === conceptId ? project.finalScript : index >= 0 ? project.concepts[index] : null;
        ...
        if (normalized.cuts.length >= 15) {
          normalized.validation = validateDetailedPlanning(normalized, project.productAnalysis, project.duration);
        }
        ...
        if (options.createRevision) {
          ...
          const previousLatestIndex = project.scriptRevisions.findLastIndex((revision) => revision.conceptId === current.id && revision.stage === "post-request-latest");
          if (previousLatestIndex >= 0) project.scriptRevisions[previousLatestIndex] = latestRevision;
        ...
        if (options.productionNotes !== undefined) project.productionNotes = clean(options.productionNotes, 5000);
    ...
    async restoreScriptRevision(projectId: string, revisionId: string, actor: string) {
      return update(projectId, (project) => {
        if (project.status === "approved") throw new Error("최종 승인된 대본은 복제한 뒤 복원해 주세요.");
        ...
        const current = index >= 0 ? project.concepts[index] : project.finalScript?.id === revision.conceptId ? project.finalScript : undefined;
    ...
    async selectConcept(projectId: string, conceptId: string) {
      return update(projectId, (project) => {
        if (!["script_review", "concept_selected"].includes(project.status)) throw new Error("대본 검토 중에만 기획안을 선택할 수 있습니다.");
        if (!project.concepts.some((item) => item.id === conceptId)) throw new Error("선택할 기획안을 찾지 못했습니다.");
    ...
    async requestProduction(input: { projectId: string; conceptId: string; deadline: string; actor: string; requestNote?: string }) {
      return update(input.projectId, (project) => {
        if (!["script_review", "concept_selected"].includes(project.status)) throw new Error("대본 검토 중인 프로젝트만 제작 요청할 수 있습니다.");
        ...
        const validation = concept.cuts.length >= 15 ? validateDetailedPlanning(concept, project.productAnalysis, project.duration) : undefined;
        ...
        if (!/^\d{4}-\d{2}-\d{2}$/.test(clean(input.deadline))) throw new Error("올바른 제작 마감일을 입력해 주세요.");
        ...
        transition(project, "production_requested", input.actor, clean(input.requestNote, 500) || "최종 대본 확정 및 제작 요청");
      });
    },
    ...
    async addVersion(projectId: string, version: VideoVersion, actor: string) {
      return update(projectId, (project) => {
        ...
        if (version.versionNumber !== expected) throw new Error("영상 버전 번호가 올바르지 않습니다.");
    ...
    async addComment(projectId: string, comment: ReviewComment, options: { requestRevision: boolean; actor: string }) {
      return update(projectId, (project) => {
        if (!project.versions.some((item) => item.id === comment.versionId)) throw new Error("피드백을 남길 영상 버전을 찾지 못했습니다.");
        project.comments.push(clone(comment));
        if (options.requestRevision) {
          if (project.status !== "marketer_review") throw new Error("마케터 검수 중에만 수정 요청할 수 있습니다.");
          project.versions = project.versions.map((item) => (item.id === comment.versionId ? { ...item, reviewStatus: "changes_requested" } : item));
    ...
    async approveVersion(projectId: string, versionId: string, actor: string) {
      return update(projectId, (project) => {
        if (project.status !== "marketer_review") throw new Error("마케터 검수 중인 영상만 최종 승인할 수 있습니다.");
    ...
    async duplicateApproved(projectId: string, actor: string) {
      return locked((store) => {
        ...
        if (source.status !== "approved" || !source.finalScript) throw new Error("최종 승인된 제작 대본만 복제할 수 있습니다.");
        const nowDate = new Date();
        const now = nowDate.toISOString();
        const existingCodes = store.projects.flatMap((project) => project.concepts.map((concept) => concept.materialCode));
...
export function validateCreateVideoProjectInput(input: Partial<CreateVideoProjectInput>) {
  ...
  if (![15, 20, 30, 45, 60].includes(Number(input.duration))) throw new Error("영상 길이는 15초, 20초, 30초, 45초, 60초만 지원합니다.");
  if (!input.format || !["short-form", "reels", "feed", "other"].includes(input.format)) throw new Error("영상 형식이 올바르지 않습니다.");
  if (!input.objective || !["purchase", "new-customer-hook", "retargeting", "usp", "review-ugc", "interest", "new-product", "benefit"].includes(input.objective)) throw new Error("영상 목적이 올바르지 않습니다.");
  if ((input.referenceAssets || []).some((asset) => !asset.filePath.startsWith("/video-collaboration/references/") || asset.size <= 0 || asset.size > 100 * 1024 * 1024)) {
    throw new Error("참고 파일 정보가 올바르지 않습니다.");
  }
  if (input.platform && !["meta", "instagram", "tiktok", "youtube-shorts"].includes(input.platform)) throw new Error("영상 플랫폼이 올바르지 않습니다.");
  if (input.conceptFormat && !VIDEO_CONCEPT_FORMATS.includes(input.conceptFormat)) throw new Error("영상 콘셉트 형식이 올바르지 않습니다.");
  if (input.productOriginalAsset && (!input.productOriginalAsset.mimeType.startsWith("image/") || !input.productOriginalAsset.filePath.startsWith("/video-collaboration/references/"))) throw new Error("상품 원본 이미지 정보가 올바르지 않습니다.");
  let url: URL;
  try {
  ...
  if (!["http:", "https:"].includes(url.protocol)) throw new Error("HTTP 또는 HTTPS 상품 URL만 지원합니다.");
}

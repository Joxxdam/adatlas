...
const PROMOTION_WORDS = [/\b\d+\s*%\s*(?:할인|OFF)?\b/gi, /(?:사전\s*예약|한정\s*판매|무료\s*배송|오늘만|단독\s*특가|쿠폰\s*적용|후기\s*\d+등|베스트\s*상품)/gi, /(?:오르기\s*전|가격에|(?:추석|설날?|명절)\s*(?:사전\s*)?예약\s*가능|예약\s*가능|소량\s*입고|선별\s*상품)/gi, /(?:★|☆|♥|♡|◆|▶|\*{1,}|_{1,})/g];

const TITLE_PROMOTION_PATTERNS = [/(?:추석|설날?|명절)\s*(?:사전\s*)?예약\s*가능/gi, /사전\s*예약/gi, /후기\s*\d+등/gi, /무료\s*배송/gi, /(?:한정|단독)\s*(?:판매|특가)?/gi, /소량\s*입고/gi, /\d+\s*%\s*(?:할인|OFF)?/gi];
...
export function extractVideoTitleMetadata(value: unknown, brandName = "") {
  const rawTitle = String(value || "")
    .replace(/\s+/g, " ")
    .trim();
  const promotions = TITLE_PROMOTION_PATTERNS.flatMap((pattern) => rawTitle.match(pattern) || []).map((item) => item.replace(/\s+/g, " ").trim());
  const volumeOrOption = [...rawTitle.matchAll(/\d+(?:[.,]\d+)?\s*(?:(?:kg|g|ml|l)\s*(?:박스|팩|세트)?|개|입|팩|박스|세트)(?![a-z가-힣])/gi)].map((match) => match[0].replace(/\s+/g, "").trim())[0] || "";
  return {
    rawTitle,

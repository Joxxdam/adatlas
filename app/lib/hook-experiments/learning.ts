...
function lift(value: number, benchmark: number, direction: "higher" | "lower") {
  ...
  const result =
    direction === "higher"
      ...
      : (benchmark - value) / Math.abs(benchmark);
...
export const ObjectiveHookLearningService = {
  build(store: HookExperimentStore): ObjectiveHookInsight[] {
    ...
    for (const analysis of eligibleAnalyses) {
      ...
      const direction = ObjectiveMetricService.primary(
        ...
        analysis.groups[0]?.metrics
      ).direction;
      for (const group of analysis.groups.filter(
        (item) => item.eligible && item.primaryMetricValue !== null
      )) {
        const competitors = analysis.groups
          .filter((item) => item.hookGroupId !== group.hookGroupId && item.eligible)
          .map((item) => item.primaryMetricValue);
        const benchmark = median(competitors);
        const groupLift =
          benchmark === null ? null : lift(group.primaryMetricValue!, benchmark, direction);
        const key = [
          experiment.advertiserId,
        ...
        store.experimentAssets
          .filter(
            (asset) =>
              asset.experimentId === experiment.id &&
              asset.hookGroupId === group.hookGroupId &&
              asset.assetId
          )
          .forEach((asset) => bucket.assets.add(asset.assetId!));
        buckets.set(key, bucket);
      }
    }
    return Array.from(buckets.entries()).map(([id, bucket]) => {
      const experimentCount = store.experiments.filter(
        (item) =>
          item.advertiserId === bucket.experiment.advertiserId &&
          item.categoryId === bucket.experiment.categoryId &&
          item.productId === bucket.experiment.productId &&
          item.objective === bucket.experiment.objective
      ).length;
      const eligibleExperimentCount = bucket.experiments.size;
      const assetCount = bucket.assets.size;
      const confidenceScore = Math.round(
        Math.min(1, eligibleExperimentCount / 3) * Math.min(1, assetCount / 6) * 100
      );
      const status: ObjectiveHookInsight["status"] =
        eligibleExperimentCount >= 3 && assetCount >= 6
          ? "VERIFIED"
  ...
  recommendations(insights: ObjectiveHookInsight[], objective: ExperimentObjective) {
    return insights
      .filter(
        (item) =>
          item.objective === objective && item.status === "VERIFIED" && item.medianLift !== null
      )
      .sort((left, right) => (right.medianLift || 0) - (left.medianLift || 0));
  },
};
...
export const NextExperimentService = {
  nextStage(experiment: CreativeExperiment, analysis: ExperimentAnalysis) {
    if (analysis.needsMoreData)
      throw new Error("비교 가능한 데이터가 부족해 다음 실험을 만들 수 없습니다.");
    if (experiment.stage === "DISCOVERY") {
      if (analysis.selectedHookCodes.length < 3)
        throw new Error("T02로 진행할 상위 후킹 3개가 필요합니다.");
      return {

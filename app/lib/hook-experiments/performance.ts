...
export const ObjectiveMetricService = {
  primary(objective: ExperimentObjective, metrics: AggregatedMetrics) {
    if (objective === "AWR")
      return { name: "CPM", value: metrics.cpm, direction: "lower" as const };
    if (objective === "TRF")
      return {
        name: "랜딩페이지 조회당 비용",
    ...
    if (objective === "SLS")
      return { name: "ROAS", value: metrics.roas, direction: "higher" as const };
    if (objective === "ENG")
      return { name: "참여당 비용", value: metrics.costPerEngagement, direction: "lower" as const };
...
function assetEligible(experiment: CreativeExperiment, metrics: AggregatedMetrics) {
  ...
  if (
    (metrics.spend || 0) < rules.minimumSpend ||
    (metrics.impressions || 0) < rules.minimumImpressions
  )
    return false;
  if (experiment.objective === "TRF")
    return (
      (metrics.linkClicks || 0) >= rules.minimumClicks &&
      (metrics.landingPageViews || 0) >= rules.minimumLandingPageViews
    );
  if (experiment.objective === "SLS")
    return (
      (metrics.linkClicks || 0) >= rules.minimumClicks &&
      (metrics.purchases || 0) >= rules.minimumPurchases
    );
  if (experiment.objective === "ENG") return (metrics.engagements || 0) >= rules.minimumClicks;
  return true;
}

function groupResult(input: {
  ...
  const matched = input.records.filter(
    (record) => record.hookGroupId === input.group.id && record.matchStatus === "matched"
  );
  ...
  const byAsset = input.assets
    ...
    .filter(
      (item): item is { code: string; value: number; eligible: boolean; spend: number } =>
        item.value !== null
    );
  ...
  if (eligibleAssets.length < input.experiment.ruleConfig.minimumEligibleAssetsPerHook)
    warnings.push("최소 집행기준을 충족한 소재가 부족합니다.");
  if (
    concentration !== null &&
    concentration > input.experiment.ruleConfig.maximumSingleAssetSpendShare
  )
    warnings.push("특정 소재에 지출이 과도하게 집중됐습니다.");
...
export const ExperimentComparabilityService = {
  inspect(
    ...
    if (matched.some((record) => record.objective !== experiment.objective))
      warnings.push("서로 다른 캠페인 목표의 보고서 행이 포함됐습니다.");
    if (new Set(matched.map((record) => `${record.dateStart}|${record.dateEnd}`)).size > 1)
      warnings.push("비교 기간이 서로 다른 보고서 행이 포함됐습니다.");
    const spends = groups.map((group) => group.metrics.spend || 0).filter((value) => value > 0);
    if (
      spends.length > 1 &&
      Math.max(...spends) / Math.min(...spends) > experiment.ruleConfig.maximumSpendImbalanceRatio
    )
      warnings.push("후킹별 지출 차이가 비교 허용 범위를 넘었습니다.");
    if (groups.filter((group) => group.eligible).length < Math.min(2, groups.length))
      warnings.push("비교 가능한 후킹 그룹이 부족합니다.");
    return { comparable: warnings.length === 0, warnings };
  },
};

function rankGroups<T extends HookPerformanceResult>(
  experiment: CreativeExperiment,
  groups: T[]
): T[] {
...
> {
  const direction = ObjectiveMetricService.primary(
    experiment.objective,
    groups[0]?.metrics || HookPerformanceAggregationService.aggregate([])
  ).direction;
  return groups.map((group) => {
    if (experiment.stage !== "VALIDATION" || group.eligibleAssetCount < 3)
      return {
        ...group,
        stability: group.eligibleAssetCount
          ? ("UNSTABLE" as const)
          : ("INSUFFICIENT_DATA" as const),
      };
    const competitors = groups
      .filter((item) => item.hookGroupId !== group.hookGroupId)
      .flatMap((item) =>
        item.assetPrimaryValues.filter((asset) => asset.eligible).map((asset) => asset.value)
      );
    ...
    const better = eligible.filter((asset) =>
      direction === "higher" ? asset.value > benchmark : asset.value < benchmark
    ).length;
    const stableMinimum = Math.max(3, Math.ceil(eligible.length * 0.6));
    const stability =
      better >= stableMinimum &&
      (group.spendConcentration || 0) <= experiment.ruleConfig.maximumSingleAssetSpendShare
        ? "STABLE_WINNER"
        : better <= 1 ||
            (group.spendConcentration || 0) > experiment.ruleConfig.maximumSingleAssetSpendShare
          ? "SINGLE_ASSET_WINNER"
...
export const HookValidationService = {
  analyze(input: {
    ...
    const comparability = ExperimentComparabilityService.inspect(
      input.experiment,
      rawGroups,
    ...
    const selectedHookCodes =
      input.experiment.stage === "DISCOVERY" && canSelect
        ? topEligible.slice(0, 3).map((group) => group.hookCode)
        : [];
    const winner =
      input.experiment.stage === "VALIDATION" &&
      canSelect &&
      topEligible[0]?.stability === "STABLE_WINNER"
        ? topEligible[0].hookCode
        : input.experiment.stage === "REFINEMENT" && canSelect
          ? topEligible[0]?.hookCode
          : undefined;
    return {
      ...
      warnings: [
        ...
        ...ranked.flatMap((group) =>
          group.warnings.map((warning) => `${group.hookCode}: ${warning}`)
        ),
      ],

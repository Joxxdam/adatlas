...
export function roundCode(round: number) {
  if (!Number.isInteger(round) || round < 1 || round > 99) throw new Error("실험 회차는 1~99 사이여야 합니다.");
  return `T${String(round).padStart(2, "0")}`;
}

export function createExperimentCode(input: { brandCode: string; originalHostProductNo: string; objective: ExperimentObjective; testRound: number }) {
...
export function createExperimentAssetCode(input: { brandCode: string; originalHostProductNo: string; hookCode: ExperimentHookCode; generationRound: number; variant: string; version?: number }) {
  const variant = compact(input.variant, 3);
  if (!/^[A-Z][A-Z0-9]{0,2}$/.test(variant)) throw new Error("소재 변형 코드는 A~Z로 시작해야 합니다.");
...
export function createHookCategoryCode(input: { brandCode: string; originalHostProductNo: string; objective: ExperimentObjective; testRound: number; hookCode: ExperimentHookCode }) {
...
export const experimentAssetCodeSource = "AT-[A-Z0-9]{3,5}-[A-Z0-9]{1,12}-(?:SEN|CUR|PRB|BRD|PRC|REV|USP|EMP|URG|VAL|EVT|RPT|CRT|BND|NEW|GRW|CTL)-T\\d{2}-[A-Z][A-Z0-9]{0,2}(?:-V\\d{2})?";

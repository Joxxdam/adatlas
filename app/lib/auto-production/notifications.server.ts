...
export function notificationForRuns(runs: AutoProductionRun[]): AutoProductionNotification | undefined {
  ...
  if (active.length) {
    ...
    return { level: "progress", message: `자동 제작 중 · ${completed}/${total}`, href: "/auto-production" };
  }
  const latest = runs[0];
  ...
  return failed
    ? { level: "warning", message: `광고 ${completed}장 완료 · ${failed}장 확인 필요`, href: "/auto-production" }
    : { level: "success", message: `오늘의 광고 콘텐츠 ${completed}장이 완성됐어요.`, href: "/auto-production" };
}

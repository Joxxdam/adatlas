...
export function recentTasks(runs: AutoProductionRun[], advertiserId: string, days: number, now = new Date()) {
  return runs
    .filter((run) => run.advertiserId === advertiserId && withinDays(run.createdAt, days, now))
...
export function selectFreshHook(
  hypotheses: AutoHookHypothesis[],
  recent: AutoProductionProductTask[],
  options: { explorationRatio?: number; hasPerformanceLearning?: boolean; seed?: string } = {}
) {
  ...
  const scored = hypotheses.map((hook, index) => {
    ...
    const similarity = previous.reduce(
      (highest, item) => Math.max(highest, textSimilarity(text, `${item.mainHook} ${item.messageHypothesis} ${item.recommendedScene}`)),
      0
    );
  ...
  return fresh
    ? {
        hook: fresh.hook,
        reason: exploring
          ? `성과 학습을 참고하되 새 메시지 탐색 비율 ${Math.round(ratio * 100)}%에 따라 중복이 적은 후킹을 선택했습니다.`
          : options.hasPerformanceLearning
            ? "기존 성과 학습과 상품 근거를 함께 반영한 상위 후킹을 선택했습니다."
            : previous.length
              ? "최근 메시지와 겹치지 않으면서 상품 근거가 강한 후킹을 선택했습니다."
              : "성과 이력이 없어 상품 근거가 가장 강한 탐색형 후킹을 선택했습니다.",
      }
    : null;
}

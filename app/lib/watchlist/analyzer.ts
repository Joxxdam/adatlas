...
function extractCta(text: string) {
  return unique(splitPhrases(text).filter((phrase) => ctaKeywords.some((keyword) => phrase.toLowerCase().includes(keyword)))).slice(0, 4);
}

function extractUsp(brand: WatchlistBrand, text: string) {
  const base = [brand.referenceStrength, brand.hookPattern, brand.category];
  const phrases = splitPhrases(text).filter((phrase) => /(단독|무료|프리미엄|편안|빠른|공식|랭킹|후기|특가|고급|기능|효과)/.test(phrase));
...
export function analyzeContent(brand: WatchlistBrand, content: CrawledContent): ContentAnalysis {
  const text = [content.title, content.description, content.text, brand.referenceStrength, brand.hookPattern].filter(Boolean).join("\n").slice(0, 6000);
  ...
  return {
    ...
    improvementSuggestions: ["첫 문장에 문제 상황이나 숫자 근거를 더 명확히 배치하세요.", "USP는 기능보다 고객이 얻는 변화 중심으로 다시 쓰면 전환성이 높아집니다.", "CTA는 하나의 행동으로 좁히고 기간/혜택 조건을 함께 제시하세요."],
    rawText: text,
    analyzedAt,
  };
}

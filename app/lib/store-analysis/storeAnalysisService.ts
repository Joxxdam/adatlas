...
export async function normalizeStoreAnalysisOptions(
  input: Partial<StoreAnalysisOptions>
): Promise<StoreAnalysisOptions> {
...
async function copyGuideMatchFor(result: {
  storeUrl: string;
  storeName?: string;
  brandName?: string;
  category?: string;
}): Promise<CopyGuideMatch> {
  ...
  const confidence = guide.matchedBy.includes("domain")
    ? 1
    : guide.matchedBy.some((value) => value === "brandName" || value === "advertiserName")
      ? 0.9
      : 0.7;
  return {
    matched: true,
...
export class StoreAnalysisService {
  constructor(
    ...
    private readonly createCrawler: () => Pick<
      StoreCrawler,
      "discover" | "fetchProductPage"
    > = () => new StoreCrawler()
  ) {}
  ...
  async analyze(input: Partial<StoreAnalysisOptions>): Promise<StoreAnalysisResult> {
    ...
    const analyzed = await mapWithConcurrency(
      discovery.productLinks,
      3,
      async (link): Promise<ProductDetailAnalysis | null> => {
        try {
          ...
          const detail = discovery.extractor.extractProductDetail(
            page.finalUrl,
            page.html,
            summary,
            options.analyzeReviews
          );
          return await filterVerifiedImages(detail);
        } catch (error) {
          warnings.push(
            `${link.label || link.url} 상품 분석 실패: ${error instanceof Error ? error.message : "요청 실패"}`
          );
          return null;
        }
      }
    ...
    for (const detail of details) {
      const peers = details
        .filter((item) => item.product.category === detail.product.category)
    ...
    const recommendedProducts = details
      .filter(
        (detail) =>
          detail.advertisingAnalysis &&
          detail.advertisingAnalysis.recommendationType !== "low-priority"
      )
      .sort(
        (a, b) =>
          (b.advertisingAnalysis?.overallScore || 0) - (a.advertisingAnalysis?.overallScore || 0)
      )
      .map((detail, index) => ({
        rank: index + 1,
    ...
    const limitations = [
      "광고 적합도는 공개된 상품정보, 가격, 후기, 이미지, USP를 바탕으로 계산하며 실제 매출 성과를 보장하지 않습니다.",
      "실제 판매량, 매출, 재고, 마진, 전환율, ROAS는 외부 데이터 연동 없이 확인할 수 없습니다.",
      "동적 로딩·로그인·외부 리뷰 위젯의 정보는 공개 HTML에서 수집되지 않을 수 있습니다.",
    ];
    if (missingReviewCount) {
      limitations.push(
        `${missingReviewCount}개 상품은 공개 리뷰 본문 또는 리뷰 지표를 확인하지 못해 confidence를 낮췄습니다.`
      );
    }
    if (details.length < discovery.productLinks.length) {
      warnings.push(
        `상품 ${discovery.productLinks.length}개 중 ${details.length}개 상세 분석을 완료했습니다. 실패 상품은 제외하고 결과를 계산했습니다.`
      );
    }
    ...
    const result: StoreAnalysisResult = {
      ...
      stats: {
        ...
        reviewAnalyzedProductCount: details.filter((detail) => Boolean(detail.reviewAnalysis))
          .length,

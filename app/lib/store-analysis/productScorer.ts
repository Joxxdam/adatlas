...
export function scoreProductForAdvertising(params: {
  detail: ProductDetailAnalysis;
  categoryPeers: StoreProductSummary[];
  angles: ContentAngleRecommendation[];
}): ProductAdvertisingAnalysis {
  const { detail, categoryPeers, angles } = params;
  const { product } = detail;
  const productStrengthScore = roundScore(
    28 +
      (product.isSoldOut ? -35 : 18) +
      (product.isBest ? 18 : 0) +
      (product.isNew ? 9 : 0) +
      (product.isSetProduct ? 6 : 0) +
      (product.category ? 7 : 0) +
      (detail.description ? 8 : 0) +
      Math.min(12, detail.uspCandidates.length * 3)
  );
  const priceAttractivenessScore = relativePriceScore(product, categoryPeers);
  const hasReviewSignal = Boolean(
    product.reviewCount || product.rating || detail.reviewAnalysis?.sourceReviewCount
  );
  const reviewUsabilityScore = hasReviewSignal
    ? roundScore(
        Math.min(55, Math.log10((product.reviewCount || 0) + 1) * 22) +
          (product.rating ? Math.max(0, Math.min(35, (product.rating - 3) * 18)) : 0) +
          Math.min(20, (detail.reviewAnalysis?.repeatedBenefits.length || 0) * 5)
      )
    : undefined;
  const imageCount = detail.imageUrls.length;
  const imageUsabilityScore = imageCount
    ? roundScore(
        42 + Math.min(42, imageCount * 8) + Math.min(16, detail.detailImageUrls.length * 3)
      )
    : undefined;
  const uspClarityScore =
    detail.uspCandidates.length || detail.description
      ? roundScore(Math.min(100, detail.uspCandidates.length * 17 + (detail.description ? 24 : 0)))
      : undefined;
  const seasonFitScore = seasonalScore(detail);
  const contentExpansionScore = angles.length
    ? roundScore(
        36 +
          Math.min(54, angles.length * 10) +
          Math.min(10, new Set(angles.map((angle) => angle.type)).size * 2)
      )
    : undefined;
  ...
  const confidence =
    Math.round(Math.max(0.18, Math.min(1, 0.12 + weighted.availableWeight / 115)) * 100) / 100;
  ...
  if (product.discountRate)
    reasons.push(`확인된 할인율 ${product.discountRate}%를 가격 소구에 활용할 수 있습니다.`);
  if (product.reviewCount)
    reasons.push(
      `공개 리뷰 ${product.reviewCount.toLocaleString("ko-KR")}개를 신뢰 근거로 활용할 수 있습니다.`
    );
  if (detail.reviewAnalysis?.repeatedBenefits.length) {
    reasons.push(
      `리뷰에서 ${detail.reviewAnalysis.repeatedBenefits.slice(0, 3).join(", ")} 장점이 반복 확인됩니다.`
    );
  }
  if (imageCount >= 3)
    reasons.push(`상품·상세 이미지 ${imageCount}개가 광고 제작 후보로 확인되었습니다.`);
  if (detail.uspCandidates.length >= 2)
    reasons.push("상세페이지에서 여러 USP 후보가 확인되었습니다.");
  if (angles.length >= 3)
    reasons.push(`서로 다른 콘텐츠 방향 ${angles.length}개로 확장할 수 있습니다.`);
  ...
  if (detail.reviewAnalysis?.repeatedComplaints.length) {
    risks.push(
      `리뷰에서 ${detail.reviewAnalysis.repeatedComplaints.slice(0, 3).join(", ")} 불만이 반복 확인됩니다.`
    );
  }
  ...
  if (
    product.isSoldOut ||
    weighted.score < 43 ||
    (!product.salePrice && !imageCount && !detail.uspCandidates.length)
  ) {
  ...
  return {
    ...
    scoreAvailability: Object.fromEntries(
      Object.entries(scores).map(([key, value]) => [key, value !== undefined])
    ) as ProductAdvertisingAnalysis["scoreAvailability"],
  };
}

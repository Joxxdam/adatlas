...
const CAFE24_PRODUCT_PATTERN = /\/product\/(?:[^/?#]+\/)?\d+(?:\/category\/\d+)?|\/product\/detail\.html\?[^#]*product_no=\d+/i;

export class Cafe24Extractor extends GenericStoreExtractor {
  canHandle(url: string, html = "") {
    return /cafe24|EC_SHOP|CAFE24API|shop_no|xans-product/i.test(html) || /\.cafe24\.com$/i.test(new URL(url).hostname);
  }
  ...
  discoverCategoryUrls(url: string, html: string): DiscoveredStorePage[] {
    ...
    const extras = extractLinks(html, url)
      .filter((link) => isSameStoreDomain(link.url, url) && /\/product\/(?:list|best|new|project|search)\.html|[?&]cate_no=\d+/i.test(link.url) && !CAFE24_PRODUCT_PATTERN.test(link.url))
      .map((link): DiscoveredStorePage => {
        const signal = `${link.text} ${link.url}`.toLowerCase();
        const kind: DiscoveredStorePage["kind"] = /best|베스트|인기/.test(signal) ? "best" : /new|신상품|신상/.test(signal) ? "new" : /project|sale|할인|기획전/.test(signal) ? "promotion" : "category";
        return { url: link.url, label: link.text || "카테고리", kind };
      });
    return [...base, ...extras].filter((page, index, pages) => pages.findIndex((candidate) => candidate.url === page.url) === index);
  }
  ...
  discoverProductUrls(url: string, html: string, source: DiscoveredStorePage): DiscoveredProductLink[] {
    ...
    return [...base, ...extras].filter((item, index, items) => items.findIndex((candidate) => candidate.url === item.url) === index);
  }
}

import type { DiscoveredProductLink, DiscoveredStorePage, StoreExtractor, StoreInfo, StoreProductSummary } from "../types";
import { absoluteHttpUrl, cleanText, extractLinks, metaContent, tagAttribute, titleContent, uniqueStrings } from "../htmlUtils";
...
const PRODUCT_QUERY_PATTERN = /[?&](?:product_no|pseq|goodsno|goodsNo|branduid|itemid|item_id|productId)=/i;
const LIST_PATH_PATTERN = /\/(?:category|categories|collections|best|new|sale|event|promotion|exhibition|plan|shop|goods\/goods_list|products?\/products?_list\.php)(?:\/|\.|\?|$)|\/c\/[a-z0-9]+(?:x[a-z0-9]+)+(?:\/|$)/i;

function isLikelyProductUrl(url: string) {
  const parsed = new URL(url);
  const path = parsed.pathname;
  if (/(?:\/list|\/category|\/categories|\/collections(?:\/all)?\/?$|\/products?\/products?_list\.php|\/goods\/(?:lastitem|best|new|sale|event|category|list))/i.test(path)) {
    return false;
  }
  return PRODUCT_QUERY_PATTERN.test(parsed.search) || /\/goods\/(?:detail|view)\//i.test(path) || /\/goods\/(?:goods_view|view)\.(?:php|html)$/i.test(path) || /\/shop\/shopdetail\.html$/i.test(path) || /\/products?\/products?_view\.php$/i.test(path) || /\/product\/detail\.html$/i.test(path) || /\/product\/(?:[^/?#]+\/)?\d+(?:\/|$)/i.test(path) || /\/products\/[^/?#]+\/?$/i.test(path) || /\/p\/[^/?#]+(?:\/[^?#]*)?\/?$/i.test(path) || /\/(?:item|product-detail|product_detail)\/[a-z0-9_-]+\/?$/i.test(path);
}
...
function extractBrandPhrases(html: string) {
  ...
  for (const match of html.matchAll(/<(?:h1|h2|h3|strong|em)[^>]*>([\s\S]*?)<\/(?:h1|h2|h3|strong|em)>/gi)) {
...
export class GenericStoreExtractor implements StoreExtractor {
  ...
  discoverCategoryUrls(url: string, html: string): DiscoveredStorePage[] {
    ...
    for (const link of extractLinks(html, url)) {
      ...
      if (!LIST_PATH_PATTERN.test(link.url) && !/베스트|신상품|신제품|할인|특가|카테고리|기획전|세일|전체상품|상품보기/i.test(signal)) {
        continue;
      }
  ...
  discoverProductUrls(url: string, html: string, source: DiscoveredStorePage): DiscoveredProductLink[] {
    ...
    for (const link of extractLinks(html, url)) {
      ...
      discovered.push({
        ...
        category: source.kind === "category" || source.kind === "product-list" ? source.label : undefined,
  ...
  extractProductDetail(url: string, html: string, summary: StoreProductSummary, analyzeReviews: boolean) {

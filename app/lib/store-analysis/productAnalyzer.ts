...
function breadcrumbCategory(html: string) {
  const breadcrumb = extractJsonLdNodes(html).find((node) => jsonLdTypeIncludes(node, "breadcrumblist"));
  const items = Array.isArray(breadcrumb?.itemListElement) ? breadcrumb.itemListElement : [];
  const names = items
    .map((item) => stringValue(firstRecord(firstRecord(item).item).name) || stringValue(firstRecord(item).name))
    .filter(Boolean)
...
function arrayImageValues(value: unknown, baseUrl: string): string[] {
  ...
  if (value && typeof value === "object") {
    ...
    return [absoluteHttpUrl(stringValue(object.url), baseUrl), absoluteHttpUrl(stringValue(object.contentUrl), baseUrl)].filter(Boolean);
  }
  return [];
}

function fallbackPrice(html: string) {
  const known = metaContent(html, "product:price:amount") || metaContent(html, "product:sale_price:amount") || metaContent(html, "og:price:amount");
...
function fallbackOriginalPrice(html: string, salePrice?: number) {
  const known = metaContent(html, "product:original_price:amount") || metaContent(html, "product:retail_price:amount");
  const matched = known || html.match(/(?:정상가|소비자가|기존가|시중가)[^\d]{0,40}([\d,]{3,})\s*원/i)?.[1];
...
function reviewSignals(product: Record<string, unknown>, html: string) {
  ...
  const reviewCount = numberFromUnknown(aggregate.reviewCount) || numberFromUnknown(aggregate.ratingCount) || numberFromUnknown(metaContent(html, "product:review_count"));
  const rating = numberFromUnknown(aggregate.ratingValue);
  const reviewNodes = [...(Array.isArray(product.review) ? product.review : product.review ? [product.review] : []), ...extractJsonLdNodes(html).filter((node) => jsonLdTypeIncludes(node, "review"))];
  const reviewTexts = reviewNodes.map((node) => stringValue(firstRecord(node).reviewBody) || stringValue(firstRecord(node).description)).filter(Boolean);
  for (const match of html.matchAll(/<(?:li|div|article)[^>]+(?:class|id)=["'][^"']*(?:review|후기)[^"']*["'][^>]*>([\s\S]*?)<\/(?:li|div|article)>/gi)) {
...
function extractSpecifications(html: string) {
  ...
  for (const match of html.matchAll(/<tr[^>]*>[\s\S]*?<\/(?:tr)>/gi)) {
    const cells = Array.from(match[0].matchAll(/<(?:th|td)[^>]*>([\s\S]*?)<\/(?:th|td)>/gi)).map((cell) => cleanText(cell[1], 160));
...
function extractUspCandidates(html: string, description: string, specifications: Record<string, string>) {
  const candidates: string[] = [];
  const sourceBlocks = [...description.split(/[.!?。]\s*/), ...Array.from(html.matchAll(/<(?:h1|h2|h3|strong|em|li)[^>]*>([\s\S]*?)<\/(?:h1|h2|h3|strong|em|li)>/gi)).map((match) => cleanText(match[1], 180)), ...Object.entries(specifications).map(([key, value]) => `${key}: ${value}`)];
  for (const raw of sourceBlocks) {
    const text = cleanText(raw, 180);
    if (text.length < 8 || text.length > 180) continue;
    if (/(원산지|구성|용량|중량|품질|프리미엄|특허|인증|무첨가|저당|단백질|신선|숙성|향|식감|편리|무료배송|보관|제조|성분|효과|특징|장점|선별|직송|산지|세트)/i.test(text) || candidates.length < 2) {
...
export function extractProductSummaryFromHtml(url: string, html: string, discovered?: DiscoveredProductLink): StoreProductSummary {
  ...
  const category = normalizeCategoryName(stringValue(product.category)) || breadcrumbCategory(html) || discovered?.category;
  const productSpecificSoldOut = /outofstock/i.test(availability) || /품절된\s*상품|현재\s*품절|sold\s*out/i.test(pageSignals);
  const isSetProduct = /세트|묶음|패키지|\d+\s*(?:개|팩|입|종)\s*(?:구성|세트)?/i.test(`${name} ${stringValue(product.description)}`);

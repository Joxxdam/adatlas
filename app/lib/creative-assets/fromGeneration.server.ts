...
export async function createAssetFromGenerationResult(input: {
  ...
  const created = await creativeAssetRepository.create({
    ...
    materialCode: input.result.materialCode,
    copyPlanMode: input.job.copyPlanMode || "legacy-hook-first",
    referenceId: input.result.referenceAdaptedCopyPlan?.referenceId || input.result.nativeCreative?.adReference?.id,

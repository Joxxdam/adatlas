...
if (projectId !== BIGQUERY_ALLOWED_PROJECT) {
  throw new Error(
    `BIGQUERY_PROJECT_ID는 허용된 프로젝트(${BIGQUERY_ALLOWED_PROJECT})와 일치해야 합니다.`
  );
}
...
export const bigQueryConfig = Object.freeze({
  ...
  maxBytesBilled: positiveInteger(
    process.env.BIGQUERY_MAX_BYTES_BILLED,
    DEFAULT_MAX_BYTES_BILLED
  ),
  queryTimeoutMs: positiveInteger(
    process.env.BIGQUERY_QUERY_TIMEOUT_MS,
    DEFAULT_QUERY_TIMEOUT_MS
  ),
});

export const bigQueryAllowedDatasets = [
  "DIM_MALL",
  "DIM_PDT",
  "FACT_MALL",
  "FACT_RANK",

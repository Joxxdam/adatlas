...
const genericProductTokens = new Set(["오리지널소스", "오리지널", "소스", "original", "source", "샤워젤", "바디워시", "제품", "상품", "세트", "250ml"]);
...
function profileExplicitlyMatches(profile: AdvertiserProfile, product: ProductInfoForPrompt) {
  ...
  return (profile.domains || []).some((domain) => haystack.includes(normalize(domain))) || [profile.name, ...(profile.aliases || [])].some((alias) => normalize(alias) && haystack.includes(normalize(alias)));
}

function urlProductIds(product: ProductInfoForPrompt) {
  const values = [product.creativeContext?.productId, ...Array.from(decode(product.landingUrl).matchAll(/(?:product_no=|\/)(\d{1,12})(?=\/category\/|[/?#&]|$)/gi)).map((match) => match[1])];
 ...
 */
export function matchKnownProductAsset(product: ProductInfoForPrompt): KnownProductAssetMatch | null {
  ...
  for (const profile of advertiserProfiles) {
    ...
    for (const asset of profile.productAssets) {
      ...
      if (assetName && productName && (productName.includes(assetName) || assetName.includes(productName))) {
        score += 120;
      }
...
function registeredSourceCandidate(match: KnownProductAssetMatch, existing: SourceImageCandidate[]): SourceImageCandidate[] {
  ...
  return [registered, ...existing.filter((candidate) => candidate.imagePath !== match.cutoutPath).map((candidate) => ({ ...candidate, selected: false }))];
}
...
export function applyKnownProductAssets(product: ProductInfoForPrompt): ProductInfoForPrompt {
  ...
  const paths = unique([match.cutoutPath, ...(product.productImagePaths || []), product.productImagePath, product.secondaryProductImagePath]).slice(0, 12);
  const gallery = unique([...(product.extractedGalleryImages || []), product.extractedMainImage, product.productImagePath, product.secondaryProductImagePath]).filter((path) => path !== match.cutoutPath);
  return {
    ...product,

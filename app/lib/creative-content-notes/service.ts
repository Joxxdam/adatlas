import type {
  ...
  CreativeContentNote,
  CreativeContentNoteContext,
  CreativeNoteCompliance,
...
export function resolveCreativeContentNotes(
  notes: CreativeContentNote[],
  context: CreativeContentNoteContext
): ContentNoteResolution {
  ...
  for (let leftIndex = 0; leftIndex < selected.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < selected.length; rightIndex += 1) {
      ...
      const oppositeHookRules = new Set([left.type, right.type]).size === 2
        && [left.type, right.type].every((type) => type === "PREFERRED_HOOK" || type === "AVOIDED_HOOK")
        && getHookCode(left.content) !== "ETC"
        && getHookCode(left.content) === getHookCode(right.content);
      ...
      conflicts.push({
        ...
        message: oppositeHookRules
          ? `같은 후킹을 선호하면서 제외하도록 설정했습니다: ${left.content} / ${right.content}`
          : `필수 문구와 금지 문구가 충돌합니다: ${left.content}`,
        blocking: true,
      });
    }
  }
...
export function applyCreativeContentNotesToCopy(
  copy: { headline: string; body: string; proof: string; offer: string; cta: string },
  ...
  const prohibited = notes.filter(
    (note) => (note.prohibited && note.type !== "AVOIDED_HOOK") || note.type === "PROHIBITED_EXPRESSION"
  );
  for (const note of prohibited) {
    ...
    for (const key of ["headline", "body", "proof", "offer", "cta"] as const) {
      if (next[key].toLowerCase().includes(phrase.toLowerCase())) {
        next[key] = next[key].replace(new RegExp(escapeRegExp(phrase), "gi"), "").replace(/\s{2,}/g, " ").trim();
  ...
  return {
    ...
    compliance: {
      ...verification,
      state:
        verification.requiredMissing.length || verification.prohibitedFound.length
          ? "blocked"
          : repairs.length
            ? "repaired"
            : "passed",
...
export function validateCreativeContentNotes(
  text: string,
  notes: ResolvedCreativeContentNote[]
): Omit<CreativeNoteCompliance, "state" | "repairs"> {

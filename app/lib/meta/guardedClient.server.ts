...
const readOperations = new Set<MetaOperation>([
  "connection",
  "accounts",
  "campaigns",
  "adsets",
...
const writeOperations = new Set<MetaOperation>([
  "media.upload",
  "adset.create",
  "creative.create",
  "ad.create",
...
export function createGuardedMetaClient(input: {
  ...
  return {
    async read<T>(operation: MetaOperation, path: string, params?: Record<string, unknown>) {
      ...
      if (!input.readEnabled)
        throw new Error(
          "Meta 읽기가 꺼져 있습니다. 서버에서 META_READ_ENABLED=true를 설정해 주세요."
        );

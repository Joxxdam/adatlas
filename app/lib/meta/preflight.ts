...
function mediaCompatibility(input: MetaDraftRegistrationInput) {
  ...
  if (needsVertical && !ratios.has("9:16")) {
    return {
      ok: false,
      detail:
        "Stories 또는 Reels 게재 위치에는 9:16 결과가 필요합니다. 자동 자르기는 수행하지 않습니다.",
    };
  }
...
export function validateMetaDraftPreflight(
  ...
  const urlsValid = input.creatives.every(
    (creative) =>
      isSafeLandingUrl(creative.landingUrl) &&
      creative.utm.trim().length > 0 &&
      creative.landingUrl === input.creatives[0]?.landingUrl
  );
  const accountRelation =
    input.campaign.accountId === input.adAccount.id &&
    input.baselineAdSet.accountId === input.adAccount.id &&
    input.baselineAdSet.campaignId === input.campaign.id;
  const purchaseReady =
    input.conversionEvent === "PURCHASE" &&
    input.baselineAdSet.promotedObject.customEventType === "PURCHASE" &&
    Boolean(input.pixelId || input.datasetId);
  const isCatalog = input.campaign.objective === "PRODUCT_CATALOG_SALES";
  const campaignSafe =
    supportedSalesObjectives.has(input.campaign.objective) &&
    !isCatalog &&
    input.campaign.budgetMode === "ABO" &&
    !input.campaign.isAdvantagePlus &&
    !(input.campaign.specialAdCategories || []).length;
  const creativeCountSafe =
    input.creatives.length > 0 &&
    input.creatives.length <= Math.min(6, options.maxAdsPerRequest) &&
    uniqueHooks.size === input.creatives.length;
  const approvedCreatives = input.creatives.every(
    (creative) =>
      creative.approved &&
      Boolean(creative.mediaPath) &&
      Boolean(creative.materialCode) &&
  ...
  const checks = [
    {
      ...
      ok: campaignSafe,
      detail:
        input.campaign.budgetMode === "CBO"
          ? "선택한 캠페인은 캠페인 예산 방식이므로 광고 세트 일 예산 USD 5 또는 승인된 계정 통화 예산을 적용할 수 없습니다."
          : campaignSafe
            ? "캠페인은 수정하지 않고 새 PAUSED 광고 세트만 만듭니다."
            : "수동 판매·웹 전환 ABO 캠페인만 지원합니다.",
    },
    {
      key: "relationship",
    ...
    },
    {
      ...
      detail: purchaseReady
        ? "픽셀·데이터셋과 Purchase 이벤트를 확인했습니다."
        : "Purchase 전환 자산 확인이 필요합니다.",
    },
    {
      key: "budget",
      label: "고정 일 예산",
      ok: budget.ok,
      detail: budget.ok
        ? `${budget.display} · API 값 ${budget.dailyBudgetMinor}`
        : budget.reason || "예산 확인 실패",
    },
    {
      key: "creative-count",
    ...
    },
    {
      ...
      detail: approvedCreatives
        ? "검수 완료된 H01~H06 단일 이미지 결과만 사용합니다."
        : "제작 결과에서 승인된 광고 이미지가 준비되어야 합니다.",
    },
    {
      key: "landing",
      ...
      ok: urlsValid,
      detail: urlsValid
        ? "모든 광고가 동일 승인 URL과 고유 UTM을 사용합니다."
        : "랜딩 URL 또는 UTM을 확인해 주세요.",
    },
    {
      key: "media",
      label: "게재 위치·미디어 비율",

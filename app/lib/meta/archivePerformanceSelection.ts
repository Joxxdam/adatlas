...
function identity(entry: CreativeArchiveEntry, field: "advertiser" | "product") {
  if (field === "advertiser") {
    return String(entry.advertiserId || entry.advertiserName).trim().toLowerCase();
  }
  return String(entry.productId || entry.productName).trim().toLowerCase();
}
...
export function isArchivePerformanceEligible(entry: CreativeArchiveEntry) {
  return Boolean(
    entry.id &&
      entry.assetCode &&
      entry.imageUrl &&
      hookCodePattern.test(String(entry.hookCode).toUpperCase())
  );
}

export function prepareArchivePerformanceSelection(
  input: CreativeArchiveEntry[]
): ArchivePerformanceSelection {
  const entries = input
    .filter(isArchivePerformanceEligible)
    .slice(0, 6)
    .sort((left, right) => left.hookCode.localeCompare(right.hookCode));
  const base = entries[0];
  const sameAdvertiser = Boolean(
    base && entries.every((entry) => identity(entry, "advertiser") === identity(base, "advertiser"))
  );
  const sameProduct = Boolean(
    base && entries.every((entry) => identity(entry, "product") === identity(base, "product"))
  );
  ...
  const visualDirections = new Set(
    entries.map((entry) => String(entry.visualDirection || "").trim()).filter(Boolean)
  );
  const hookOnlyEligible =
    entries.length >= 2 &&
    templateIds.size === 1 &&
    visualDirections.size === 1 &&
    entries.every((entry) => Boolean(entry.templateId && entry.visualDirection));
  const testType: PerformanceTestType = hookOnlyEligible
    ? "hook-only"
  ...
  return {
    entries,
    valid: true,
    message: hookOnlyEligible
      ? "동일한 디자인 조건이 확인되어 후킹 문구 차이를 비교할 수 있습니다."
      : "장면·레이아웃이 서로 달라 후킹 단독 효과가 아닌 전체 소재 조합 성과로 비교합니다.",
    testType,
    hookOnlyEligible,
  };
}

export function archiveEntriesToMetaDrafts(
  entries: CreativeArchiveEntry[],
  landingUrl: string
): MetaCreativeDraft[] {
  return prepareArchivePerformanceSelection(entries).entries.map((entry) => {
    ...
    return {
      ...
      utm:
        entry.utmContent ||
        `utm_source=meta&utm_medium=paid_social&utm_campaign=adatlas_archive&utm_content=${encodeURIComponent(materialCode)}`,
      approved: true,
    };
  });
}

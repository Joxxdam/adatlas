...
import { foodImpactHeroTemplate, templatesById } from "@/lib/bannerTemplates";
import { getSelectedProductImagePath } from "./imageEffects";
import type { ProductImageRenderEffect } from "./types";
import {
  adaptiveLogoDataUrl,
  aiDisclosureSvg,
  buildFontFaceCss,
...
export async function renderFoodImpactHero(body: RenderBody) {
...
export async function renderFoodCategoryTemplate(body: RenderBody, templateId: string) {
  const width = body.canvasSize?.width || 1200;
  const height = body.canvasSize?.height || 1200;

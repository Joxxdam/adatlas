...
type GeneratedCopyInput = Partial<
  Pick<
    GeneratedAdCopy,
...
export function buildImageGenerationPrompt(params: BuildImagePromptParams): {
  ...
  const imageSourceMode =
    params.imageSourceMode ??
    (params.selectedSourceImagePath || product.selectedSourceImagePath
      ? "image-edit"
      : "text-to-image");
  const preservationMode =
    ...
    (params.selectedSourceImagePath || product.selectedSourceImagePath
      ...
      : "free-generate");
  const productName =
    clean(params.productName || product.productName) || "featured ecommerce product";
  const category = clean(params.category || product.category) || "consumer product";
  const mainBenefit = clean(
    params.mainBenefit || product.mainBenefit || product.extractedDescription
  );
  ...
  const selectedSourceImagePath = clean(
    params.selectedSourceImagePath || product.selectedSourceImagePath || productImagePaths[0]
  );
  ...
  const shouldPreserveProduct =
    imageSourceMode === "image-edit" && preservationMode === "preserve-product";
  ...
  const creativeDirection: ImageCreativeDirection =
    mode === "visual-only"
      ? {
          ...
          textPolicy:
            "No readable text of any kind. No letters, no numbers, no typography, no captions, no logo text, no watermark, no emoji.",
      ...
      : {
          ...
          textPolicy:
            "Readable Korean ad text is allowed and requested. Use only the provided copy elements, no emoji or decorative pictograms.",
          productPreservationPolicy: preservationPolicy,
  ...
  const common = [
    ...
    selectedSourceImagePath
      ...
      : "",
    selectedSourceImagePath
      ? "Use the selected source image as the primary product reference."
      : "",
    selectedSourceImagePath
      ? "Preserve the product identity, packaging shape, color, surface details, material texture, scale cues, and overall silhouette based on the selected source image."
      : "",
    selectedSourceImagePath
      ? "Do not redesign the product. Do not replace it with a different bottle, container, package, meat cut, tray, color, pattern, or shape. Keep the product recognizable as the same item."
      : "",
    shouldPreserveProduct ? preservationPolicy : "",
    shouldPreserveProduct
      ? "Make the product the main hero. Create a high-converting Korean ecommerce advertising visual. Use strong commercial lighting and clear separation from the background. The product should look more polished, but not redesigned."
      : "",
    shouldPreserveProduct
      ? "If the source image is a detail-page food image, preserve the detail-page food characteristics and do not reinterpret it as a clean packaged product shot."
      : "",
    productImagePaths.length
      ? `Additional product image references: ${productImagePaths.slice(0, 4).join(", ")}. Use them only as supporting identity guidance, not as text sources.`
      : "",
    visualDetails,
  ...
  const visualOnly = [
    ...
    imageSourceMode === "image-edit"
      ? "Image source mode: image-edit. Use the source product as the hero. Preserve the product. Improve only the background, lighting, and advertising mood."
      : "Image source mode: text-to-image.",
  ...
  const textPieces = [
    shortCopy(generatedCopy.headline, 28) &&
      `Large bold Korean headline: "${shortCopy(generatedCopy.headline, 28)}"`,
    shortCopy(generatedCopy.price, 18) && `Price text: "${shortCopy(generatedCopy.price, 18)}"`,
    shortCopy(generatedCopy.highlightCopy, 32) &&
      `Short highlight banner text: "${shortCopy(generatedCopy.highlightCopy, 32)}"`,
    shortCopy(generatedCopy.cta, 18) && `CTA button text: "${shortCopy(generatedCopy.cta, 18)}"`,
    shortCopy(generatedCopy.bodyCopy, 42) &&
      `Short supporting copy: "${shortCopy(generatedCopy.bodyCopy, 42)}"`,
  ].filter(Boolean);
  ...
  const textInImage = [
    ...
    "Mode: text-in-image.",
    imageSourceMode === "image-edit"
      ? "Image source mode: image-edit. Use the source product as the hero. Preserve the product while creating the ad layout."
      : "Image source mode: text-to-image.",

import type { AdImageLabel, GeneratedAdCopy, GptImageGenerationMode, GptImagePreservationMode, GptImageSourceMode, ImageCreativeDirection, ProductInfoForPrompt } from "./types";
...
function summarizeReferences(labels: AdImageLabel[] = []) {
  ...
  return {
    hookType: compactList(
      finals.map((label) => label.hookType),
      "commerce hook suited to the product"
    ),
    appealPoint: compactList(
      finals.map((label) => label.appealPoint),
      "clear purchase reason"
    ),
    copyNuance: compactList(
      finals.map((label) => label.copyNuance || label.toneOfVoice),
      "direct Korean ecommerce copy tone"
    ),
    visualTone: compactList(
      finals.map((label) => label.visualTone),
      "high contrast Korean ecommerce visual tone"
    ),
    layoutPattern: compactList(
      finals.map((label) => label.layoutPattern || label.visualCopyRelation),
      "hero product centered with strong promotional hierarchy"
    ),
    whyItWorks: compactList(
      finals.map((label) => label.whyItWorks),
      "it reduces purchase hesitation and makes the value easy to understand"
    ),
  };
}
...
export function buildImageGenerationPrompt(params: BuildImagePromptParams): {
  ...
  const productImagePaths = [clean(params.selectedSourceImagePath || product.selectedSourceImagePath), clean(params.productImagePath || product.productImagePath), ...(params.productImagePaths ?? product.productImagePaths ?? [])].filter(Boolean);
  ...
  const preservationPolicy = shouldPreserveProduct ? "Use the provided source image as the primary visual reference. Preserve the original subject, food texture, composition, color tone, product identity, shape, silhouette, packaging if already present, label placement if already present, product count, and visible details. Do not redesign the product. Do not replace the original image with a newly designed product. Do not turn cooked food into packaged raw meat. Do not create a plastic tray package. Do not create a new label, logo, container, or product package. Edit only the background, lighting, color grading, sharpness, subtle shadows, and advertising mood. Keep the product recognizable as the exact same item from the source image." : "Product identity may be newly generated from the prompt; preserve key product cues when available.";

  const creativeDirection: ImageCreativeDirection =
    mode === "visual-only"
      ? {
  ...
  const visualOnly = [...common, "Mode: visual-only.", imageSourceMode === "image-edit" ? "Image source mode: image-edit. Use the source product as the hero. Preserve the product. Improve only the background, lighting, and advertising mood." : "Image source mode: text-to-image.", "Ad-ready product-focused commerce visual for use as a banner background or main product visual.", "Absolutely no readable text.", "No letters.", "No numbers.", "No typography.", "No captions.", "No logo text.", "No watermark.", "Use high contrast, realistic product photography, premium commercial lighting, clean composition.", "Leave negative space where Korean headline can be added later by a separate template renderer."];

  const textPieces = [shortCopy(generatedCopy.headline, 28) && `Large bold Korean headline: "${shortCopy(generatedCopy.headline, 28)}"`, shortCopy(generatedCopy.price, 18) && `Price text: "${shortCopy(generatedCopy.price, 18)}"`, shortCopy(generatedCopy.highlightCopy, 32) && `Short highlight banner text: "${shortCopy(generatedCopy.highlightCopy, 32)}"`, shortCopy(generatedCopy.cta, 18) && `CTA button text: "${shortCopy(generatedCopy.cta, 18)}"`, shortCopy(generatedCopy.bodyCopy, 42) && `Short supporting copy: "${shortCopy(generatedCopy.bodyCopy, 42)}"`].filter(Boolean);

  const textInImage = [...common, "Mode: text-in-image.", imageSourceMode === "image-edit" ? "Image source mode: image-edit. Use the source product as the hero. Preserve the product while creating the ad layout." : "Image source mode: text-to-image.", "Create a complete Korean ecommerce ad image with readable ad text included.", "Include bold readable Korean headline text.", "Include price text if provided.", "Include a short highlight banner text.", "Include CTA style button text.", "Use strong commerce layout, high contrast promotional hierarchy, product remains the hero.", `Copy elements to include by priority: ${textPieces.length ? textPieces.join("; ") : "headline, price, short highlight and CTA based on product information"}.`, "Do not overcrowd the image; use 3 to 5 key text elements only.", "No emoji, no emoticons, no pictograms. Use clean readable Korean typography only."];
  ...
  return {
    prompt: [templatePrompt.promptText, "Additional generation guardrails:", ...(mode === "visual-only" ? visualOnly : textInImage)].filter(Boolean).join("\n"),
    creativeDirection,
  };
}

import type { NormalizedImageBox, ReviewCreativeTemplate, ReviewImageRegion, ReviewPrivacyRegion, ReviewSourceCandidate, ReviewType } from "./types";

export const REVIEW_ANALYSIS_VERSION = "review-analysis-v2";
export const REVIEW_RENDER_VERSION = "review-render-v3";

const reviewContextPattern = /(review|reviews|reviewimg|review_img|photo[_-]?review|testimonial|comment|community|ugc|후기|리뷰|구매평|사용기|사용후기|고객평|댓글|반응|비포|애프터)/i;
const notReviewPattern = /(logo|icon|sprite|button|banner|coupon|delivery|detail[_-]?guide|size[_-]?guide|로고|아이콘|배송|쿠폰|이벤트|사이즈표)/i;
const authorPattern = /(?:^|\s)(?:작성자|아이디|닉네임|구매자|user|by)\s*[:：]?|\*{2,}|\([\d*.]{5,}\)|@[a-z0-9_.-]+/i;
const dateOrderPattern = /(?:20\d{2}[.\-/년]\s*\d{1,2}[.\-/월]\s*\d{1,2}|주문(?:번호)?|배송(?:지|정보)|결제(?:정보)?|order\s*#?\s*[a-z0-9-]+)/i;
const directPrivacyPattern = /(?:[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}|01[016789][-.\s]?\d{3,4}[-.\s]?\d{4}|\b\d{2,3}[-.\s]\d{3,4}[-.\s]\d{4}\b|\b(?:\d{1,3}\.){3}\d{1,3}\b)/i;
const socialUiPattern = /(좋아요|댓글\s*\d*|공유|신고|도움이\s*돼요|likes?|comments?|share)/i;
const policyRiskPattern = /(완치|치료|약보다|의사가|질병|암|당뇨|고혈압|무조건|100%|절대|기적|즉시\s*제거|영구적|부작용\s*없)/i;
...
export function reviewCandidateContextScore(input: { url?: string; alt?: string; context?: string; width?: number; height?: number }) {
  ...
  if (width && height && Math.max(width, height) / Math.max(1, Math.min(width, height)) > 4) score -= 15;
  return score;
}

export function inferReviewType(input: { sourceContext?: string; ocrText?: string; textRegionCount?: number; faceCount?: number; width?: number; height?: number; manuallyUploaded?: boolean }): { type: ReviewType; confidence: number; reason: string } {
  ...
  if (/(댓글|대댓글|조회\s*\d|게시글|좋아요|\([\d*.]{5,}\)|20\d{2}[.\-/]\d{1,2}[.\-/]\d{1,2})/i.test(ocr) || (/(community|comment|커뮤니티)/i.test(input.sourceContext || "") && textCount >= 2)) {
  ...
  const hasNaturalReviewSentence = /(?:저는|나는|제가|우리|써보|먹어보|사용해|재구매|또\s*샀|만족|추천해|좋았|좋아요|놀랐|ㅋㅋ|ㅎㅎ|ㅠㅠ|네요|했어요|해요|합니다|했음|좋음|소리지름)/i.test(ocr);
  const directReviewSource = /(reviewimg|review[_-]?image|photo[_-]?review|후기[_-]?이미지)/i.test(input.sourceContext || "");
...
export function selectKeyReviewSentence(text: string, productText = "") {
  ...
  const candidates = sentenceCandidates(text).map((sentence, index) => {
    ...
    if (/[!?ㅋㅎ]|진짜|정말|대박|미쳤|놀랐|쓰자마자|먹자마자|닿자마자|바르자마자|받자마자|계속|재구매|시원|부드|맛있|편해|좋아|향|촉감|가성비/.test(sentence)) score += 18;
    if (/쓰자마자|먹자마자|닿자마자|바르자마자|받자마자|사용\s*직후|한입\s*먹/.test(sentence)) score += 12;
...
export function recommendReviewCrop(input: { type: ReviewType; textRegions: ReviewImageRegion[]; keySentence: string; width: number; height: number }) {
...
export function detectPrivacyRegions(textRegions: ReviewImageRegion[], faceRegions: ReviewImageRegion[] = []): ReviewPrivacyRegion[] {
  const textMasks = textRegions
    ...
    .map((region, index): ReviewPrivacyRegion => {
      const text = region.text || "";
      const reason = directPrivacyPattern.test(text) ? "연락처·계정 식별정보 가능성" : dateOrderPattern.test(text) ? "작성일·주문정보 가능성" : "작성자·아이디 가능성";
      return {
        ...region,
  ...
  const faceMasks = faceRegions.map((region, index): ReviewPrivacyRegion => ({
    ...region,
...
export function scoreReviewCandidate(input: { width: number; height: number; ocrText: string; ocrConfidence: number; type: ReviewType; keySentence: string; productText: string; privacyCount: number; contextScore?: number }) {
  const minSide = Math.min(input.width, input.height);
  const ratio = Math.max(input.width, input.height) / Math.max(1, minSide);
  const imageQualityScore = Math.max(0, Math.min(1, minSide / 900 - Math.max(0, ratio - 2.8) * 0.12));
  const productTokens =
    input.productText
      .toLowerCase()
      .match(/[0-9a-z가-힣]{2,}/gi)
  ...
  const productRelevanceScore = Math.min(1, Math.max(0, (input.contextScore || 0) / 100 + hits * 0.09 + (input.type !== "not-review" ? 0.3 : 0)));
  const hookStrengthScore = input.keySentence ? Math.min(1, 0.35 + Math.min(0.35, input.keySentence.length / 90) + (/[!?ㅋㅎ]|진짜|정말|쓰자마자|재구매/.test(input.keySentence) ? 0.22 : 0)) : 0;
  const specificityScore = input.keySentence ? Math.min(1, 0.3 + (/\d|향|맛|촉감|구성|사용|배송|가격|시원|부드/.test(input.keySentence) ? 0.45 : 0.15)) : 0;
  const privacyRiskScore = Math.min(1, input.privacyCount * 0.16);
  const policyRiskScore = reviewPolicyRiskScore(input.ocrText);
  const overallReviewScore = Math.max(0, Math.min(1, imageQualityScore * 0.18 + input.ocrConfidence * 0.16 + productRelevanceScore * 0.24 + hookStrengthScore * 0.22 + specificityScore * 0.2 - policyRiskScore * 0.22));
  return {
    imageQualityScore,
...
export function dedupeReviewCandidates(candidates: ReviewSourceCandidate[]) {
  ...
  for (const candidate of [...candidates].sort((a, b) => {
    ...
    const urlKey = (candidate.originalUrl || candidate.imagePath).replace(/([?&])(w|h|width|height|quality|q|resize)=[^&]*/gi, "$1").replace(/[?&]+$/, "");
    const textKey = normalizedOcrKey(candidate.ocrText);
    const duplicate = kept.some((item) => {
      const itemUrlKey = (item.originalUrl || item.imagePath).replace(/([?&])(w|h|width|height|quality|q|resize)=[^&]*/gi, "$1").replace(/[?&]+$/, "");
      return (candidate.contentHash && candidate.contentHash === item.contentHash) || urlKey === itemUrlKey || perceptualHashDistance(candidate.perceptualHash, item.perceptualHash) <= 5 || (textKey.length >= 24 && (textKey === normalizedOcrKey(item.ocrText) || textSimilarity(candidate.ocrText, item.ocrText) >= 0.9));
    });
...
export function recommendReviewTemplate(candidates: ReviewSourceCandidate[], selectedIds: string[] = []): ReviewCreativeTemplate {
  const selected = candidates.filter((candidate) => (selectedIds.length ? selectedIds.includes(candidate.id) : candidate.recommended));
  const pool = selected.length ? selected : candidates.slice(0, 1);
  if (pool.some((candidate) => candidate.reviewType === "before-after")) return "before-after-usage";
  if (pool.length >= 2) return "review-collection";
  if (pool[0]?.reviewType === "community-reaction" || (pool[0]?.keySentence.length || 0) <= 48) return "reaction-comment";
...
export function buildReviewHeadline(candidate?: ReviewSourceCandidate) {
  ...
  const trimmed = [...sentence]
    ...
    .join("")
    .replace(/[“”\"]+/g, "");
  if (/쓰자마자|먹자마자|받자마자|바르자마자|입자마자|닿자마자/.test(trimmed)) return `사용 직후 나온 반응: ${trimmed}`;

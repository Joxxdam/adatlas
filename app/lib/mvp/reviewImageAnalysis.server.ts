...
import { clampReviewBox, dedupeReviewCandidates, detectPrivacyRegions, inferReviewType, recommendReviewCrop, REVIEW_ANALYSIS_VERSION, reviewCandidateContextScore, scoreReviewCandidate, selectKeyReviewSentence } from "./reviewCreative";
import type { NormalizedImageBox, ReviewImageRegion, ReviewSourceCandidate, ReviewSourceType } from "./types";
...
async function normalizedReviewBuffer(imagePath: string) {
  ...
  const buffer = await sharp(source).rotate().resize({ width: 2400, height: 2400, fit: "inside", withoutEnlargement: true }).png().toBuffer();
...
function reviewAnalysisCacheKey(contentHash: string, productText: string) {
  return crypto.createHash("sha256").update(`${contentHash}:${expectedOcrProvider()}:${REVIEW_ANALYSIS_VERSION}:${productText}`).digest("hex");
}
...
async function openAiOcr(buffer: Buffer): Promise<OcrResult | null> {
  ...
  try {
        ...
        input: [
          {
            ...
            content: [
              {
                type: "input_text",
                text: '이미지에 실제로 보이는 텍스트만 OCR 하세요. 보이지 않는 문장은 만들지 마세요. 각 줄의 normalized bounding box는 좌상단 원점 x,y,width,height 0~1입니다. 식별 가능한 얼굴도 boxes로 반환하세요. JSON만 반환: {"lines":[{"text":"","confidence":0,"box":{"x":0,"y":0,"width":0,"height":0}}],"faces":[{"confidence":0,"box":{"x":0,"y":0,"width":0,"height":0}}]}',
              },
    ...
    const lines = (parsed.lines || [])
      ...
      .map((line, index): ReviewImageRegion => ({
        id: `text-${index + 1}`,
        role: "text",
    ...
    const faces = (parsed.faces || [])
      ...
      .map((face, index): ReviewImageRegion => ({
        id: `face-${index + 1}`,
        role: "face",
...
async function appleVisionOcr(buffer: Buffer): Promise<OcrResult | null> {
  ...
  const sdkRoot = "/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk";
  try {
...
export async function analyzeReviewImage(input: { imagePath: string; originalUrl?: string; sourceType?: ReviewSourceType; sourceContext?: string; productName?: string; productDescription?: string; manuallyUploaded?: boolean }): Promise<ReviewSourceCandidate> {
  ...
  const warnings = [ocr.warning, scores.policyRiskScore >= 0.28 ? "광고 정책상 오해 소지가 있는 표현을 확인해주세요." : "", privacyRegions.length ? "개인정보 가능 영역을 기본 가림 처리했습니다." : "", crop.confidence < 0.5 ? "자동 크롭 신뢰도가 낮아 원본에 가깝게 유지했습니다." : "", inferred.type === "not-review" ? "후기 이미지로 판단할 근거가 부족합니다." : ""].filter(Boolean) as string[];
...
export async function analyzeReviewSourceCandidates(input: { candidates: ReviewRawCandidate[]; productName?: string; productDescription?: string; collectLimit?: number; displayLimit?: number }) {

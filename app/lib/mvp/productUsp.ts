...
const suspiciousInstructionPattern =
  /(system prompt|ignore previous|assistant:|developer:|명령을 무시|지시를 따르)/i;
const uspKeywordPattern =
  /(원산지|국내산|한우|등급|부위|등심|안심|채끝|갈비|마블링|선별|숙성|냉장|냉동|당일|산지|직송|구성|중량|용량|식감|육즙|풍미|고소|부드|신선|원재료|함량|무첨가|저자극|향|세정|쿨링|보습|휴대|선물|캠핑|가족|실속|프리미엄|특마블|도매팩)/i;
const situationPattern = /(선물|명절|부모님|가족|캠핑|홈파티|집들이|식사|운동|샤워|여행)/i;
const sensoryPattern =
  /(향|냄새|쿨링|시원|상쾌|촉촉|보습|부드|식감|육즙|풍미|고소|바삭|쫀득|달콤|새콤|마블링|색감|핏|착용감)/i;
const proofPattern =
  /(원산지|국내산|함량|원재료|성분|등급|중량|용량|선별|숙성|인증|무첨가|저자극|특허|수상|테스트|후기|리뷰|평점)/i;
const problemPattern =
  /(고민|걱정|불편|부담|냄새|잡내|건조|당김|땀|피지|민감|시간|귀찮|핏|체형|외식비|선물s*고민|결정s*장애)/i;
...
function explicitSignals(product: ProductInfoForPrompt) {
  return unique([
    ...(product.verifiedBenefits || []).map(clean),
    ...(product.ingredients || []).map((value) => clean(value)),
  ]).filter(validSignal);
}

function inferTargetSegments(
  product: ProductInfoForPrompt,
  signals: string[],
  situations: string[]
): ProductTargetSegment[] {
  const facts = clean(
    [product.productName, product.category, product.targetCustomer, ...signals, ...situations].join(
      " "
    )
  );
  ...
  if (/(샤워|바디|화장품|뷰티|스킨|세정|향|쿨링|보습)/i.test(facts)) {
    if (/(땀|냄새|피지|건조|당김|민감)/i.test(facts)) {
      ...
      add(
        `${concern}이 신경 쓰이는 고객`,
        `${concern} 때문에 제품을 바꿔도 만족하기 어려움`,
        primary,
        [primary, secondary]
      );
    }
    add("향과 사용감을 중요하게 보는 고객", "기능만큼 실제 사용 순간의 감각도 중요함", secondary, [
      secondary,
      primary,
    ]);
    add(
      "성분과 차별점을 비교하는 고객",
      "비슷해 보이는 제품 사이에서 고를 근거가 필요함",
      primary,
      [primary]
    );
    ...
    add(
      "가격과 구성을 비교하는 실속 구매자",
      "가격만 보고 고르면 품질이 아쉬울까 걱정됨",
      primary,
      [primary, product.price || ""]
    );
    add(
      "맛과 품질 기준이 분명한 고객",
      "사진만으로는 식감과 품질 차이를 판단하기 어려움",
      secondary,
      [secondary, primary]
    );
    add(
      "가족 식사나 선물을 준비하는 고객",
      "실패 없이 내놓을 구매 명분이 필요함",
      situations[0] || primary,
      [situations[0] || primary]
    );
  } else if (/(패션|의류|원피스|블라우스|팬츠|스커트|자켓|니트|코디|핏)/i.test(facts)) {
    add("체형과 핏을 꼼꼼히 보는 고객", "예뻐 보여도 실제 핏이 어울릴지 걱정됨", primary, [
      primary,
      secondary,
    ]);
    add("코디 시간을 줄이고 싶은 고객", "가지고 있는 옷과 쉽게 매치할 기준이 필요함", secondary, [
      secondary,
    ]);
    add(
      "출근·모임용 옷을 찾는 고객",
      "한 번 입고 마는 옷보다 활용할 장면이 중요함",
      situations[0] || primary,
      [situations[0] || primary]
    );
  } else {
    add("비슷한 상품을 비교 중인 고객", "상품마다 비슷해 보여 선택 기준이 필요함", primary, [
      primary,
    ]);
    add(
      "실사용 이점을 먼저 보는 고객",
      "기능 설명보다 내 생활에서 달라지는 점이 궁금함",
      secondary,
      [secondary]
    );
    add("구매를 망설이는 고객", "가격과 효용이 납득되어야 결정을 내릴 수 있음", primary, [
      primary,
      product.price || "",
    ]);
  }
...
function buildHookAngles(params: {
  primary: string;
  signals: string[];
  offers: string[];
  situations: string[];
  ...
  const add = (kind: ProductHookAngle["kind"], signal: string, index: number, formula: string) => {
    ...
    if (
      !signal ||
      candidates.some((candidate) => candidate.kind === kind && candidate.signal === signal)
    )
      return;
  ...
  add(
    "problem",
    params.problems[0] || params.primary,
    0,
    "타겟의 신경 쓰이는 문제를 찌른 뒤 확인된 USP로 전환"
  );
  add(
    "usp",
    params.proof[0] || params.signals[1] || params.primary,
    1,
    "비교 기준이 되는 구체적 근거를 첫 문장에 제시"
  );
  add(
    "sensory",
    params.sensory[0] || params.signals[2] || params.primary,
    2,
    "사용 순간에 느낄 감각과 장면을 구체적으로 환기"
  );
  add(
    "value",
    params.offers[0] || params.signals[3] || params.primary,
    0,
    "확인된 가격·구성과 효용을 묶어 구매 명분 제시"
  );
  add(
    "occasion",
    params.situations[0] || params.signals[4] || params.primary,
    2,
    "상품이 필요한 정확한 순간을 먼저 제안"
  );
  add(
    "curiosity",
    params.signals[5] || params.signals[1] || params.primary,
    1,
    "구체적 차이 하나를 숨기지 않으면서 이유를 궁금하게 구성"
  );
  if (params.proof[1])
    add("proof", params.proof[1], 1, "상세페이지의 검증 가능한 근거로 선택 불안 해소");
  return candidates;
}
...
export function analyzeProductUsp(product: ProductInfoForPrompt): ProductUspAnalysis {
  ...
  const ranked = [
    ...explicitSignals(product).map((value) => ({ value, sourceRank: 11 })),
    ...sentenceCandidates(product.mainBenefit).map((value) => ({ value, sourceRank: 8 })),
    ...sentenceCandidates(product.extractedDescription).map((value) => ({ value, sourceRank: 5 })),
    ...reviewSignals(product).map((value) => ({ value, sourceRank: 4 })),
  ]
    .filter((item) => validSignal(item.value))
    .sort(
      (a, b) =>
        signalScore(b.value, productName, b.sourceRank) -
        signalScore(a.value, productName, a.sourceRank)
    );
  const uspSignals = unique(ranked.map((item) => item.value)).slice(0, 12);
  const offerSignals = unique(
    [product.discountInfo, product.price, product.originalPrice || product.oldPrice]
      .map(clean)
      .filter(Boolean)
  );
  ...
  const featureSignals = uspSignals
    .filter((signal) => !situationPattern.test(signal) && !problemPattern.test(signal))
    .slice(0, 6);
  ...
  return {
    ...
    hookAngles,
    evidenceStrength:
      uspSignals.length >= 5 ? "strong" : uspSignals.length >= 2 ? "moderate" : "limited",
...
export function isCopyGroundedInProductUsp(value: string, product: ProductInfoForPrompt) {
  ...
  const source = [product.productName, ...analysis.uspSignals, ...analysis.offerSignals]
    .map(clean)
    .join(" ");
...
function targetAwareAngle(
  analysis: ProductUspAnalysis,
  hookType: CreativeStrategy["hookType"],
  index: number
) {
...
function objectiveHeadline(params: {
  objective: AdBrief["adObjective"];
  angleKind: ProductHookAngle["kind"];
  evidence: string;
  tension: string;
  brand: string;
  strong: boolean;
...
export function buildTargetedStrategyContent(params: {
  product: ProductInfoForPrompt;
  brief: AdBrief;
  hookType: CreativeStrategy["hookType"];
  index?: number;
  ...
  const selectedTarget = analysis.targetSegments.length
    ? analysis.targetSegments[(params.targetIndex ?? index) % analysis.targetSegments.length]
    : undefined;
  ...
  const brand = compactPhrase(
    params.product.brandName || params.product.advertiserName || params.product.productName,
    14
  );
...
export function buildTargetedCopyVariants(params: {
  product: ProductInfoForPrompt;
  brief: AdBrief;
  strategy?: CreativeStrategy | null;
}): Record<"short" | "medium" | "long", GeneratedAdCopyVariant> {
  const analysis = analyzeProductUsp(params.product);
  ...
  const shortHeadline =
    params.brief.adObjective === "purchase"
      ? `${shortEvidence}, 고를 이유`
      : params.brief.adObjective === "signup"
        ? `처음엔 ${shortEvidence}부터`
        : params.brief.adObjective === "awareness"
          ? `${compactPhrase(brand, 6)}, ${compactPhrase(targeted.evidence, 6)}`
          : `${shortEvidence}, 다시 볼 이유`;
  const mediumHeadline =
    params.brief.adObjective === "purchase"
      ? `${mediumEvidence}, 지금 고를 이유`
      : params.brief.adObjective === "signup"
        ? `처음이라면 ${mediumEvidence}부터`
        : params.brief.adObjective === "awareness"
          ? `${compactPhrase(brand, 8)}의 기억점 ${mediumEvidence}`
          : `${mediumEvidence}, 다시 볼 이유`;
  const longHeadline =
    params.brief.adObjective === "purchase"
      ? `${targeted.targetTension}? 결정 근거는 ${targeted.evidence}`
      : params.brief.adObjective === "signup"
        ? `${targeted.targetTension}? ${targeted.evidence}부터 비교하세요`
        : params.brief.adObjective === "awareness"
          ? `${brand}의 한 가지 기준, ${targeted.evidence}`
          : `다시 볼 이유가 필요하다면, ${targeted.evidence}`;
...
export function buildUspFirstFallbackCopy(product: ProductInfoForPrompt) {
  ...
  return {
    ...
    bottomBarCopy: compactPhrase(
      analysis.situationSignals[0] || analysis.uspSignals[2] || primary,
      34
    ),
    ...
    variants: {
      ...
      medium: {
        ...
        bottomBarCopy: compactPhrase(
          analysis.situationSignals[0] || analysis.uspSignals[3] || primary,
          24
        ),
        cta: offer ? "구매 조건 보기" : "상품 정보 보기",
        price: clean(product.price),
      ...
      long: {
        headline: compactPhrase(
          primaryContainsProductIdentity
            ? analysis.uspSignals[0] || primary
            : `${productName}, ${analysis.uspSignals[0] || primary}`,
          34
        ),
        bodyCopy: compactPhrase(
          analysis.uspSignals[1] || analysis.uspSignals[0] || productName,
          42
        ),

...
function referenceText(references: AdImageLabel[]) {
  return references
    .flatMap((label) => {
      ...
      return [
        ...(label.structuredLabels?.hookTypes || []),
        ...(label.structuredLabels?.appealPoints || []),
        final?.hookType,
        final?.appealPoint,
        final?.copyNuance,
        final?.consumerInsight,
        final?.purchaseTrigger,
        final?.reusableCopyPattern,
      ];
    })
    .filter(Boolean)
    .join(" ");
...
export function inferAdBriefContext(params: {
  ...
  const proofElements = Array.from(
    new Set(
      [
        ...(brief.proofElements || []),
        product.price ? `상세페이지 표시 판매가 ${product.price}` : "",
        product.originalPrice ? `상세페이지 표시 정상가 ${product.originalPrice}` : "",
        product.discountInfo ? compact(product.discountInfo) : "",
        product.mainBenefit ? compact(product.mainBenefit) : "",
      ].filter(Boolean)
    )
  ).slice(0, 4);
  ...
  const visualEmphasis =
    brief.creativeIntensity === "brand"
      ? "상품을 크게 보여 주고 여백과 신뢰감을 우선"
      : hasPriceOffer
        ? "상품 비주얼과 확인된 가격·혜택을 한 시선 안에 배치"
        : "상품 비주얼과 핵심 효용을 가장 먼저 인지하도록 배치";

...
export function buildCategoryHookPriorFromHistory(
  ...
  const relevant = store.insights.filter((insight) => insight.categoryId === input.categoryId && insight.objective === input.objective && insight.medianLift !== null && insight.status !== "EXPLORATION");
  ...
  const performanceRows = store.performanceRecords.filter((record) => record.matchStatus === "matched" && record.category === input.categoryId && record.objective === input.objective && record.primaryTag && record.resultStatus !== "needs-more-data");
  const performanceValue = (record: (typeof performanceRows)[number]) => {
    ...
    if (input.objective === "TRF") return { value: record.costPerLandingPageView, direction: "lower" as const };
  ...
  const benchmark = comparableValues.length ? (comparableValues.length % 2 ? comparableValues[Math.floor(comparableValues.length / 2)] : (comparableValues[comparableValues.length / 2 - 1] + comparableValues[comparableValues.length / 2]) / 2) : null;
  if (benchmark !== null && benchmark !== 0) {
    for (const record of performanceRows) {
      const metric = performanceValue(record);
      if (metric.value === null || metric.value === undefined || !Number.isFinite(metric.value)) continue;
      const rawLift = metric.direction === "higher" ? (metric.value - benchmark) / Math.abs(benchmark) : (benchmark - metric.value) / Math.abs(benchmark);
      const tag = record.primaryTag as HookTaxonomyTag;
      const ageDays = Math.max(0, (Date.now() - new Date(record.dateEnd || record.importedAt).getTime()) / 86_400_000);
  ...
  return Object.fromEntries(
    [...buckets.entries()].map(([tag, rows]) => {
      ...
      return [tag, Math.round(Math.max(20, Math.min(80, 50 + weightedLift * evidenceEnvelope * 60)))];
    })
  );
}

export async function readCategoryHookPrior(input: { categoryId: string; objective: ExperimentObjective }): Promise<CategoryHookPrior> {

...
export function resolveProductRenderingPolicy(job: GenerationJob): ProductRenderingPolicy {
  ...
  if (override === "food-meat" || profile === "food_meat" || /한우|소고기|쇠고기|돼지고기|삼겹살|갈비|등심|안심|스테이크|육류|정육/i.test(productText)) {
...
- A cooked or serving view is allowed because the assigned unpackaged/raw source establishes the sold cut AND this material's exact hook/scene or inherited before/after structure calls for cooking, eating or sensory payoff. Seller-provided cooked photos are deliberately not used as generation references.
...
- Treat this material's single assigned unpackaged/raw product source as the visual truth for the sold cut, muscle direction, irregular marbling boundaries, fat-to-lean ratio, meat color, thickness and surface moisture. If a second packaged source is assigned for a mixed layout, use it only for package shape, label and sales-unit evidence. Never average them into a generic steak or chicken image.
- RAW/COOKED HOOK GATE: inspect the assigned raw source and the exact material hook before choosing a cooking state. A cooked view is allowed when that source proves the real raw cut/thickness AND the exact hook or scene calls for cooking, eating, serving, searing or juiciness. A product title containing words such as steak, grill or barbecue is not enough by itself. If the hook does not need a cooked payoff, keep the hero meat raw/chilled or packaged even when the selected ad reference contains cooked food.
...
- Before generating, lock the one assigned raw source's cross-section outline, slice width-to-thickness ratio, fat-cap thickness, muscle-group boundaries, marbling frequency, branch thickness and density range; never make the meat thicker, rounder, redder or more heavily marbled merely to look premium.
- SHAPE CONSERVATION: copy the assigned source's width-to-thickness ratio, irregular perimeter, taper, muscle direction and piece-to-piece variation. Cooking shrinkage may reduce width and add irregular browning, but it must never increase apparent thickness, round the perimeter, regularize every piece into the same rectangle or make multiple pieces look cast from one mold.
...
- Do not use seller cooked-image evidence. If the assigned raw source is small, blurry or compressed, move the camera slightly wider and simplify surface detail; never hallucinate macro pores, dense grooves or decorative marbling to compensate for missing resolution.
...
- A raw-to-cooked or serving scene may be generated when the hook needs it and the assigned raw source clearly establishes the sold cut. Keep the same identifiable cut, pre-cook width-to-thickness ratio and plausible shrinkage, and do not invent a different cut, grade, origin, quantity or package. Infer only physically ordinary cooking changes from that raw source.
...
- Treat this material's one assigned package image as the sole strict visual evidence for the real package. Never average it with another package or redesign its physical label.
- Keep the no-cutout integration boundary: never extract, cut out, paste, locally composite or restore source-product pixels as a detached layer after generation.
...
- Match the real container silhouette, proportions, cap, package geometry, dominant colors, material, physical label, printed logo position, printed volume and verified sales-unit count as closely as the assigned source allows. Do not substitute a generic package or paraphrase readable label text.

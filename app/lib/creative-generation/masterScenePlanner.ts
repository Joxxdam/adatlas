...
function automaticMode(profile: ProductReferenceProfile, preference: "auto" | "actual-product" | "ai-full-scene"): MasterSceneGenerationMode {
  ...
  const hasStrongRealScene = profile.referenceImages.some(
    (image) => image.usableForGeneration && image.importance >= 75 && ["lifestyle", "usage", "worn", "cooked"].includes(image.role)
  );
...
function sceneDirection(category: string, concept: MasterSceneConcept, variation: number) {
  ...
  if (/육류|한우|고기|수산|생선/.test(value)) return {
  ...
  if (/패션|의류|신발|가방/.test(value)) return {
  ...
  if (/화장|뷰티|샤워|바디|세정/.test(value)) return {
  ...
  if (/생활|수납|청소|리빙|가구/.test(value)) return {
  ...
  if (/식품|음료|간식|소스/.test(value)) return {
...
export function planMasterScene(params: {
  productId: string;
  ...
  aiBackgroundOnly?: boolean;
  aiFullCreative?: boolean;
  ...
  const mode: MasterSceneGenerationMode = params.aiFullCreative
    ? "ai-reference-full-creative"
    : params.aiBackgroundOnly
      ? "ai-background-composite"
      : automaticMode(params.profile, params.generationModePreference || "auto");
  ...
  return {
    ...
    lighting: params.creativeBrief?.lightingDirection
      ? `${params.creativeBrief.lightingDirection}. ${direction.lighting}`
      : direction.lighting,
    environment: params.creativeBrief
      ? `${params.creativeBrief.sceneDescription}. ${params.creativeBrief.visualStory}. ${direction.environment}`
      : direction.environment,
    colorDirection: [
      params.creativeBrief?.colorDirection,
      params.masterDesign.palette.background,
      params.masterDesign.palette.accent,
      params.masterDesign.palette.secondary,
    ].filter(Boolean).join(", "),
    cameraDirection: params.creativeBrief?.cameraDirection
      ? `${params.creativeBrief.cameraDirection}. ${direction.camera}`
      : direction.camera,
    depthDirection: direction.depth,
    copySafeZone,
    productSafeZone: params.masterDesign.productPosition,
    forbiddenElements: [
      "임의의 글자·숫자·가격·CTA·워터마크",
      "새 상품명·새 로고·새 라벨·새 인증마크",
      "확인되지 않은 구성품·색상·옵션",
      "제품이 떠 보이는 접촉면과 불일치한 그림자",
      ...(params.creativeBrief?.forbidPromotionalBannerCutouts
        ? ["판촉 배너·기존 광고 썸네일·문구가 박힌 참고 이미지를 배경처럼 잘라 붙이는 구성"]
        : []),
      ...(params.creativeBrief?.forbiddenElements || []),
      ...params.profile.visualIdentity.mustNotGenerate,
    ],

...
export function normalizePlannerScoreValues<T extends Record<string, number>>(scores: T): T {
  ...
  return Object.fromEntries(
    Object.entries(scores).map(([key,value]) => [key, Math.max(0, Math.min(100, Math.round((Number(value) || 0) * scale)))])
  ) as T;
}

export function recomputeHookTotal(score: Omit<HookHypothesisScore,"total">) {
  return Math.round(
    score.evidenceStrength * .18 + score.specificity * .12 + score.purchaseReasonStrength * .12 +
    score.distinctiveness * .12 + score.attentionPotential * .1 + score.visualizability * .12 +
    score.advertisingFit * .09 + score.claimSafety * .1 + score.categoryPrior * .03 + score.novelty * .02
  );
}

function signature(value: string) {
  return value.normalize("NFKC").toLocaleLowerCase("ko-KR").replace(/[^가-힣a-z0-9]/g, "");
}

...
export function findBannedCreativePhrases(value: string) {
  ...
  const phraseMatches = normalizedBans
    .filter((entry) => target.includes(entry.normalized))
    .map((entry) => entry.phrase);
  const narrationMatches = reportNarrationPatterns
    .filter((entry) => entry.pattern.test(value.normalize("NFKC")))
    .map((entry) => entry.phrase);
...
export function looksLikeGenericOrRepetitiveCopy(mainHook: string, subCopy: string) {
  ...
  const mainTokens = new Set(mainHook.normalize("NFKC").split(/[^가-힣A-Za-z0-9]+/u).filter((token) => token.length >= 2));
  const subTokens = new Set(subCopy.normalize("NFKC").split(/[^가-힣A-Za-z0-9]+/u).filter((token) => token.length >= 2));
  const overlap = [...mainTokens].filter((token) => subTokens.has(token)).length;
  return mainTokens.size > 1 && overlap / Math.min(mainTokens.size, Math.max(1, subTokens.size)) >= .75;
}

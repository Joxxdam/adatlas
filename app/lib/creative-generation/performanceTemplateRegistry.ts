...
export const performanceTemplateIds = [
  "T01_PRICE_SHOCK",
  "T02_URGENT_OFFER",
  "T03_QUALITY_PROOF",
  "T04_SENSORY_EXPERIENCE",
  "T05_BELIEF_REVERSAL",
  "T06_USE_CASE",
  "T07_SOCIAL_PROOF",
  "T08_UGC_PROBLEM_SOLUTION",
  "T09_PRODUCT_HERO",
  "T10_LINEUP_BENEFIT",
] as const;
...
export type LayoutZone =
  | "TOP_HEADLINE" | "TOP_BANNER" | "LEFT_COPY" | "RIGHT_COPY" | "CENTER_HERO"
  | "BOTTOM_PRICE_BAR" | "BOTTOM_INFORMATION" | "FLOATING_BADGE" | "HANDWRITTEN_NOTE"
  | "PRODUCT_CUTOUT" | "FULL_BLEED_VISUAL";
...
function availableSignals(truth: ProductTruth) {
  ...
  return new Set([
    truth.product.price || has(/\bprice\b|가격|원/u) ? "price" : "",
    truth.product.discountInfo || has(/할인|쿠폰|무료배송|증정|한정|특가|1\+1|2\+1/u) ? "offer" : "",
    has(/성분|원산지|인증|함량|등급|ingredient|origin|certification|numeric/u) ? "quality" : "",
    truth.product.mainBenefit || has(/향|식감|촉감|쿨링|보습|풍미|산뜻|부드러/u) ? "sensory" : "",
    truth.product.targetCustomer || has(/사용|운동|샤워|식사|출근|선물|상황|usage|target/u) ? "use-case" : "",
    has(/리뷰|후기|평점|review/u) ? "social" : "",
    truth.product.mainBenefit || truth.product.targetCustomer ? "problem" : "",
    has(/세트|구성|옵션|향\s*\d|맛\s*\d|\d\s*종|입고|1\+1|2\+1/u) ? "lineup" : "",
    truth.confirmedProductImage || truth.imageAssets.length ? "product" : "",
  ].filter(Boolean));
}
...
export function selectPerformanceTemplates(truth: ProductTruth, hooks: HookPlan[], count = 6) {
  ...
  return hooks.slice(0, count).map((hook, index) => {
    ...
    const preferred = preferences
      .map((id) => pool.find((template) => template.id === id))
      .find((template) => template && !used.has(template.id) && isPerformanceTemplateEligible(template, truth));

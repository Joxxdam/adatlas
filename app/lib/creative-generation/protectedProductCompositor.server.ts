...
function contactShadow(box: PlacementBox) {
  ...
  return {
    input: Buffer.from(
      `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}"><defs><filter id="b"><feGaussianBlur stdDeviation="${Math.max(8, height * 0.22)}"/></filter></defs><ellipse cx="${width / 2}" cy="${height / 2}" rx="${width * 0.42}" ry="${height * 0.24}" fill="#05070a" opacity=".52" filter="url(#b)"/></svg>`
    ),
...
export async function createProtectedProductComposite(input: {
  ...
  const reflection = await sharp(product)
    .flip()
    .modulate({ brightness: 0.86, saturation: 0.75 })
    .blur(2.2)
    .linear(0.18)
    .png()
    .toBuffer();
  const reflectionHeight = Math.max(1, Math.min(Math.round(target.height * 0.16), 1200 - target.y - target.height));
  const reflectionCrop = reflectionHeight > 1
    ? await sharp(reflection).extract({ left: 0, top: 0, width: target.width, height: reflectionHeight }).png().toBuffer()
    : null;
  const composites: OverlayOptions[] = [
    {
      input: Buffer.from(
        `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="1200"><defs><radialGradient id="r"><stop offset="0" stop-color="#ffffff" stop-opacity=".11"/><stop offset="1" stop-color="#ffffff" stop-opacity="0"/></radialGradient></defs><ellipse cx="${target.x + target.width / 2}" cy="${target.y + target.height / 2}" rx="${target.width * 0.7}" ry="${target.height * 0.68}" fill="url(#r)"/></svg>`
      ),
      left: 0,
      top: 0,
    },
    ...
    ...(reflectionCrop
      ? [{ input: reflectionCrop, left: target.x, top: target.y + target.height, blend: "soft-light" as const }]
      : []),
  ...
  return {
    ...
    repairs: [
      "실제 상품 픽셀 유지",
      ...(angle ? ["장면 방향에 맞춘 미세 원근·각도 보정"] : []),
      "장면 색온도·노출 보정",
      "주변광·림라이트 적용",
      "접촉 그림자와 약한 반사광 적용",
      "투명 경계 feathering과 halo 정리",
    ],
  };
}

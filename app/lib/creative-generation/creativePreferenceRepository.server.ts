...
import { readBrandMemory, saveGoldenReference, updateBrandMemory, type AdvertiserBrandMemory, type GoldenReference } from "./codexRegistry.server.ts";

export type CreativeExpressionStrength = "SAFE" | "STRONG" | "VIRAL";
export type CreativePreferenceState = "approved" | "approved-after-copy-edit" | "approved-after-image-edit" | "rejected" | "never-reuse" | "reusable-across-products" | "feedback";
...
export interface CreativePreferenceRepository {
  ...
  saveGolden(
    input: Omit<GoldenReference, "id" | "advertiserId" | "imagePath" | "approvedAt"> & {
      advertiserId: string;
...
function eventValue(event: CreativePreferenceEvent) {
  return [event.state, event.productId, event.hookCode, event.reason, event.reusableTrait].filter(Boolean).join(" · ");
}
...
export const creativePreferenceRepository: CreativePreferenceRepository = {
  ...
  async record(event) {
    const kind = event.state === "rejected" || event.state === "never-reuse" ? "reject" : event.state === "reusable-across-products" || event.state.startsWith("approved") ? "approve" : "feedback";

...
async function visionIdentityEvaluation(input: {
  profile: ProductReferenceProfile;
  candidate: Buffer;
}) {
  const reference = input.profile.referenceImages.find(
    (image) => image.usableForGeneration && !image.duplicateOf
  );
        ...
        content: [
          {
            type: "input_text",
            text: [
              "첫 이미지는 실제 판매 제품 레퍼런스이고 두 번째 이미지는 광고 장면 후보다.",
              "제품 종류, 실루엣, 비율, 대표 색상, 패키지·라벨 구조, 구성 수량, 옵션을 보수적으로 비교하라.",
              "읽기 어려운 로고 글자를 추측하지 말고, 다른 상품처럼 보이면 낮게 평가하라.",
              `반드시 보존할 특징: ${input.profile.visualIdentity.mustPreserve.join("; ")}`,
              "손·얼굴·신체 왜곡, 제품이 떠 보이는 접촉 그림자, 조명·원근 불일치도 함께 검사하라.",
              "후보 이미지에서 실제 상품이 차지하는 픽셀 면적 비율(0~1)과 1200×1200 좌표계의 상품 경계도 보수적으로 추정하라.",
              "JSON만 반환: {score:0-100,productVisible:boolean,severeDistortion:boolean,textArtifactDetected:boolean,brandMismatch:boolean,humanArtifactDetected:boolean,groundingMismatch:boolean,estimatedProductAreaRatio:0-1,productBounds:{x:number,y:number,width:number,height:number},findings:string[]}",
            ].join("\n"),
          },
  ...
  const text = outputText(await response.json()).replace(/^```(?:json)?\s*|\s*```$/g, "").trim();
  ...
  return {
    ...
    estimatedProductAreaRatio: Number.isFinite(ratio)
    ...
    findings: Array.isArray(parsed.findings)
      ? parsed.findings.map(String).filter(Boolean).slice(0, 8)
      : [],
    method: "vision" as const,
  };
}

export async function evaluateProductIdentity(input: {
  profile: ProductReferenceProfile;
  candidate: Buffer;
  generationMode: MasterSceneGenerationMode;
}) : Promise<ProductIdentityEvaluation> {
  if (
    input.generationMode === "protected-product-composite" ||
    input.generationMode === "real-photo-adaptation" ||
    input.generationMode === "library-fallback"
  ) {
    return {
      score: 100,
      productVisible: true,
      ...
      findings: [
        input.generationMode === "real-photo-adaptation"
          ? "검증된 실제 상품 사진 전체를 유지한 실사 광고 장면"
          : "검증된 실제 상품 픽셀을 유지한 보호 합성",
      ],
      method: "protected-original",
    };
  }
  const canUseVision = Boolean(process.env.OPENAI_API_KEY) &&
    String(process.env.ADATLAS_SCENE_IDENTITY_VISION_ENABLED || "true").toLowerCase() === "true";
  if (canUseVision) {
    try {

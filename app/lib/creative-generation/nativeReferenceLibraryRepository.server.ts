...
const managedManifestVersion = "native-creative-reference-library-v16-gfa-category";
const managedSelectionPolicy = "레퍼런스 관리 화면에 현재 등록된 이미지만 사용하며 등록 여부 자체를 운영자의 품질 승인으로 봅니다. 기본 대카테고리는 패션·식품·화장품·서비스·GFA 다섯 그룹으로 유지하고 건강·웰니스와 퍼스널케어는 화장품에 포함합니다. GFA는 운영자가 업로드할 때 직접 지정한 이미지만 포함합니다. 식품은 육류·간식 하위 풀을, 화장품은 디자인·후킹 하위 풀을 선택할 수 있습니다. 신규 수동·자동 제작은 선택되거나 자동 판정된 풀 전체에서 점수 우선순위 없이 중복 없는 무작위 6장을 선택합니다. 기본 분류는 바꾸지 않은 채 운영자가 추가 제작 풀을 지정해 같은 레퍼런스를 여러 상품군에서 함께 사용할 수 있습니다. 다른 레퍼런스로 재생성할 때 제외 목록이 전달된 경우에는 아직 사용하지 않은 항목에서 먼저 무작위 선택하고, 부족할 때만 해당 풀 전체로 돌아갑니다. 미지정 대카테고리로 임의 보충하지 않으며 삭제된 항목은 즉시 선택 대상에서 제외됩니다.";
    ...
    return serialize(async () => {
      ...
      try {
        for (const file of files) {
          ...
          const classification = options.categoryGroup
            ? { categoryGroup: options.categoryGroup, classificationMethod: "manual" as const, compatibility: undefined }
            : await classifyNativeReferenceImage({

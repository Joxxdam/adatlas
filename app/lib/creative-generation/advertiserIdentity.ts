...
function slug(value: string) {
  return value.toLowerCase().normalize("NFKC").replace(/^www\./, "").replace(/[^a-z0-9가-힣]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80);
}
...
export function resolveAdvertiserIdentity(product: ProductInfoForPrompt) {
  ...
  let domain = "";
  try { domain = new URL(product.landingUrl || "").hostname.replace(/^www\./, "").toLowerCase(); } catch {}

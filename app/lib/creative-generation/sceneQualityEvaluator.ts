...
import type { MasterSceneGenerationMode, MasterSceneSpec, ProductReferenceProfile, SceneQualityResult } from "./types.ts";
...
async function regionComplexity(buffer: Buffer, box: MasterSceneSpec["copySafeZone"]) {
  ...
  const normalized = await sharp(buffer).resize(1200, 1200, { fit: "cover", position: "centre" }).png().toBuffer();
  const { data } = await sharp(normalized).extract({ left: x, top: y, width, height }).resize(24, 24).grayscale().raw().toBuffer({ resolveWithObject: true });
...
export async function evaluateMasterSceneCandidate(input: { buffer: Buffer; profile: ProductReferenceProfile; spec: MasterSceneSpec; generationMode: MasterSceneGenerationMode; identity: ProductIdentityEvaluation }): Promise<SceneQualityResult> {
  const metadata = await sharp(input.buffer).metadata();
  const failures = [...input.identity.findings];
  const square = Boolean(metadata.width && metadata.height) && Math.abs((metadata.width || 0) / Math.max(1, metadata.height || 0) - 1) <= 0.04;
  ...
  const protectedMode = ["ai-background-composite", "protected-product-composite", "real-photo-adaptation", "library-fallback"].includes(input.generationMode);
  const groundingScore = input.identity.groundingMismatch ? 35 : protectedMode ? 86 : input.identity.method === "vision" ? 82 : 68;
  ...
  const score = clamp(productIdentityScore * 0.3 + productVisibilityScore * 0.15 + groundingScore * 0.15 + attentionScore * 0.1 + copySafetyScore * 0.1 + categoryFitScore * 0.1 + factSafetyScore * 0.1);
  const hardFailure = productIdentityScore < 55 || !input.identity.productVisible || input.identity.severeDistortion || input.identity.brandMismatch || input.identity.textArtifactDetected || input.identity.humanArtifactDetected || input.identity.groundingMismatch;
  const recommendation: SceneQualityResult["recommendation"] = hardFailure ? (input.generationMode === "protected-product-composite" ? "manual-review" : productIdentityScore < 45 || input.identity.brandMismatch ? "use-protected-product-composite" : "retry") : productIdentityScore < 78 || copySafetyScore < 55 ? (input.generationMode === "protected-product-composite" ? "manual-review" : "retry") : score >= 78 ? "approve" : "manual-review";
  return {
    score,

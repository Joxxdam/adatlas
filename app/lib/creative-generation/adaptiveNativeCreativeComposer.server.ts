...
function productRegion(plan: AdaptiveLayoutPlan): PlacementBox {
  ...
  const height = Math.round(1200 * Math.min(0.64, plan.productScale * 1.55));
  const map: Record<AdaptiveLayoutPlan["productAnchor"], { x: number; y: number }> = {
    left: { x: 55, y: 350 },
    center: { x: (1200 - width) / 2, y: 330 },
    right: { x: 1200 - width - 55, y: 310 },
    "bottom-left": { x: 50, y: 1200 - height - 45 },
    "bottom-right": { x: 1200 - width - 50, y: 1200 - height - 45 },
  };
...
async function isolateProduct(file: string, transparent: boolean) {
  ...
  const isolated = hasAlpha ? source : await removeBackgroundToPng(source, { extractionScope: "sales-unit", featherRadius: 0.55 });
...
async function createProductLayers(product: Buffer, plan: AdaptiveLayoutPlan, copy: PlacementBox) {
  ...
  if (overlap(region, copy)) {
    region = { ...region, width: Math.round(region.width * 0.82), height: Math.round(region.height * 0.82) };
  ...
  const singleWidth = plan.productCount === 1 ? region.width : Math.round(region.width * 0.52);
  ...
  for (let index = 0; index < plan.productCount; index += 1) {
    const rendered = await sharp(product)
      ...
      .sharpen({ sigma: 0.45, m1: 0.4, m2: 0.9 })
      .png()
      .toBuffer();
    ...
    const shadow = Buffer.from(`<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg"><defs><filter id="b"><feGaussianBlur stdDeviation="14"/></filter></defs><ellipse cx="${width / 2}" cy="${height - 22}" rx="${width * 0.3}" ry="18" fill="#000" opacity=".32" filter="url(#b)"/></svg>`);
...
function motifMarkup(plan: AdaptiveLayoutPlan, box: PlacementBox, palette: (typeof paletteMap)[string]) {
  ...
  switch (plan.graphicMotif) {
    case "marker":
      return `<path d="M ${x + 8} ${y + 210} Q ${x + box.width * 0.45} ${y + 195} ${x + box.width - 10} ${y + 212}" stroke="${palette.accent}" stroke-width="24" opacity=".78" fill="none" stroke-linecap="round"/>`;
    case "speech":
      return `<path d="M ${x + box.width - 95} ${y + 6} q65 0 65 50 q0 42-54 45 l-20 28 3-31 q-43-8-43-42 q0-50 64-50z" fill="${palette.accent}" opacity=".9"/>`;
    case "circle":
      return `<ellipse cx="${x + box.width - 78}" cy="${y + 64}" rx="62" ry="48" fill="none" stroke="${palette.accent}" stroke-width="10" transform="rotate(-8 ${x + box.width - 78} ${y + 64})"/>`;
    case "arrow":
      return `<path d="M ${x + box.width - 95} ${y + 180} q55 45 18 106 m0 0 l-28-13 m28 13 l7-30" fill="none" stroke="${palette.accent}" stroke-width="9" stroke-linecap="round"/>`;
    case "label":
      return `<rect x="${x}" y="${y - 12}" width="126" height="40" rx="20" fill="${palette.accent}"/>`;
    case "receipt":
      return `<path d="M ${x + box.width - 170} ${y - 12} h150 v120 l-15-10-15 10-15-10-15 10-15-10-15 10-15-10-15 10z" fill="${palette.surface}" stroke="${palette.accent}" stroke-width="5"/>`;
    default:
      return "";
  }
}
...
async function textOverlay(job: GenerationJob, result: GenerationResult, plan: AdaptiveLayoutPlan, box: PlacementBox) {
  ...
  const cta = ["PRICE_VALUE", "BUNDLE_LINEUP", "SEASON_URGENCY"].includes(plan.grammarId) ? result.hookPlan.cta.trim() : "";
...
export async function composeAdaptiveNativeCreative(input: { job: GenerationJob; result: GenerationResult; backgroundPath: string; productImagePath: string; productTransparent?: boolean; outputPath: string }) {
  ...
  const background = await sharp(backgroundSource).rotate().resize(1200, 1200, { fit: "cover", position: "attention" }).modulate({ brightness: 0.98, saturation: 1.03 }).png().toBuffer();
  ...
  await sharp(background)
    .composite([{ input: contrastOverlay(plan, copy, copyComposition.palette), left: 0, top: 0 }, ...productComposition.layers, { input: copyComposition.buffer, left: 0, top: 0 }])
    .png()
  ...
  const diagnostic = {
    ...
    minTextContrastRatio: Math.min(contrastRatio(copyComposition.palette.ink, copyComposition.palette.surface), copyComposition.exactText.price ? contrastRatio(copyComposition.palette.ink, copyComposition.palette.accent) : Number.POSITIVE_INFINITY, copyComposition.exactText.cta ? contrastRatio(copyComposition.palette.inverse, copyComposition.palette.ink) : Number.POSITIVE_INFINITY),
  };

...
const unstructuredCustomerOpinionPattern = /(?:너무|정말|진짜|완전)[^.!?]{0,36}(?:좋아요|좋았어요|만족(?:해요|합니다)?|사고\s*싶어요|사보고\s*싶어요)|(?:다른|레몬|민트|코코넛|라임)[^.!?]{0,24}(?:향도|제품도)?\s*(?:사고|사보고)\s*싶어요|(?:가격|맛|품질|향|식감)[^.!?]{0,42}(?:놀랐(?:어요|습니다)|시켜\s*보았|시켜\s*봤|구매해\s*보았|사\s*보았)/iu;

const recommendationUiPattern = /(?:같이\s*담으세요|함께\s*구매|추천\s*상품|관련\s*상품|최근\s*본\s*상품|다른\s*고객|뒤에서\s*훔쳐먹었다던|왕도매\s*가격판매|(?:업그레이드|기본\s*스펙)\s*(?:상품\s*)?선택)/i;
...
const incompleteOcrCopyFragmentPattern = /(?:의|한|으로|은|는|이|가|을|를|와|과|에게|에서)\s*[,:;·\-–—]*$/u;

// 업체의 수상·순위·업력은 상품 자체의 맛·식감·성분이 아니다. 근거가 있으면
// 보조 신뢰 문구로는 쓸 수 있지만 상품 USP나 독립 헤드라인으로 승격하지 않는다.
const merchantCredentialCreativeSignalPattern = /(?:브랜드\s*파워|(?:쇼핑몰|업체|기업|회사)[^.!?\n]{0,20}(?:1\s*위|대상|수상|선정)|(?:^|\s)[^.!?\n]{1,24}\s*쇼핑몰\s*[.!?]*$|국가대표|소비자[^.!?\n]{0,20}브랜드[^.!?\n]{0,12}(?:대상|1\s*위|선정)|\d[\d,.]*\s*만?\s*(?:명\s*)?(?:소비자|고객)[^.!?\n]{0,24}(?:선택|이용|구매)|\d[\d,.]*\s*만?\s*(?:세트|개|팩|건)[^.!?\n]{0,16}(?:판매|출고|돌파)|(?:업력\s*\d+|\d+\s*년\s*(?:업력|전통))|누적\s*고객|브랜드[^.!?\n]{0,12}(?:대상|어워드)\s*(?:수상|선정))/iu;
...
const malformedOcrLexemePattern = /(?:프레이엄|프레미엄|프리이엄|오\s*['’]\s*가성비|(?:향|맛|식감|육질)\s*\/\s*(?:연도|년도|연두)\s*최고)/iu;

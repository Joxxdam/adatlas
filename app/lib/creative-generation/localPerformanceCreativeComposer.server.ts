...
function wrapKorean(value: string, maxChars: number, maxLines = 2) {
  ...
  return Array.from({ length:maxLines }, (_, index) => characters.slice(index * perLine, (index + 1) * perLine).join("").trim()).filter(Boolean);
}
...
async function productLayers(product: Buffer, template: PerformanceTemplate) {
  ...
  const width = count > 1 ? Math.round(box.width / (count + .35)) : box.width;
  const positions = count === 1 ? [box.x] : [box.x, box.x + width * .75, box.x + width * 1.5];
  ...
  for (let index=0; index<count; index += 1) {
    ...
    const rendered = await sharp(product)
      ...
      .sharpen({sigma:.6,m1:.5,m2:1.1})
      .png().toBuffer();
    const meta = await sharp(rendered).metadata();
    const left = Math.round(positions[index]);
    const top = Math.round(box.y + (count > 1 && index !== 1 ? 40 : 0));
    layers.push({ input:Buffer.from(`<svg width="${meta.width}" height="${meta.height}" xmlns="http://www.w3.org/2000/svg"><ellipse cx="${(meta.width||1)/2}" cy="${(meta.height||1)-22}" rx="${(meta.width||1)*.34}" ry="22" fill="#000" opacity=".35" filter="blur(12px)"/></svg>`), left, top });
...
export async function composeLocalPerformanceCreative(input: {
  job: GenerationJob;
  ...
  const [backgroundSource,product] = await Promise.all([
    readFile(input.backgroundPath),
    isolateProduct(input.productImagePath, Boolean(input.productTransparent)),
  ]);
  const background = await sharp(backgroundSource).rotate().resize(1200,1200,{fit:"cover",position:"centre"}).modulate({brightness:.98,saturation:.98}).png().toBuffer();
  ...
  await sharp(background)
    .composite([
      {input:backgroundOverlay(input.template,palette),left:0,top:0},
      ...products.layers,
      {input:copy.buffer,left:0,top:0},
    ])
    .png()
  ...
  await writeFile(`${input.outputPath}.composition.json`, `${JSON.stringify({

...
import { catalogItemToLegacy, productionReady, readBackgroundCollectionConfigs } from "./catalogStore.server.ts";
import type { BackgroundCatalogItem, CatalogRecommendationInput } from "./catalogTypes.ts";
...
export async function recommendCatalogBackgrounds(items: BackgroundCatalogItem[], input: CatalogRecommendationInput): Promise<BackgroundRecommendation[]> {
  const configs = await readBackgroundCollectionConfigs();
  const productPool = normalize([input.product.productName, input.product.category, input.product.mainBenefit, input.product.extractedDescription, input.product.targetCustomer, ...(input.product.productColors || []), ...(input.product.brandColors || []), input.hook?.sceneDescription, ...(input.hook?.mood || []), ...(input.hook?.backgroundTags || [])].join(" "));
  const scored = items
    ...
    .map((item) => {
      ...
      for (const config of collectionConfigs) {
        ...
        if (moodMatches.length) {
          score += Math.min(18, moodMatches.length * 6);
          reasons.push("상품 분위기와 배경 톤이 잘 어울려요.");
        }
        const colorMatches = containsAny(productPool, [item.dominantColor, ...item.secondaryColors, ...config.preferredColors]);
        if (colorMatches.length) {
          score += 12;
          reasons.push("상품 대표색과 조화로운 배경이에요.");
        }
      }
      if (input.hook?.productPosition === item.recommendedProductPosition) score += 14;
      if (input.hook?.textSafeArea === item.recommendedCopyPosition) {
        score += 14;
        reasons.push("선택한 문구 위치의 가독성이 좋아요.");
      }

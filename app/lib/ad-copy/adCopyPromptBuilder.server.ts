...
export function buildAdCopyPrompt(input: { job: GenerationJob; result: GenerationResult; approvedCopies: ApprovedAdCopyMemory[]; copyGuideContent?: string; retryFailures?: string[] }) {
  ...
  const facts = job.productTruth.facts
    ...
    .filter((fact) => {
      const signature = fact.value.normalize("NFKC").replace(/[^\p{L}\p{N}]/gu, "").toLowerCase();
      if (!signature || seenFacts.has(signature)) return false;
...
- 이모지는 없어도 되며 최대 3개만 문장 끝이나 강조 지점에 사용한다. 모든 줄에 기계적으로 붙이지 않는다.
...
- 같은 상품명·중량·특가 문장을 반복하거나 상품명에 섞인 SEO 판매 문구를 그대로 옮겼으면 실패다.

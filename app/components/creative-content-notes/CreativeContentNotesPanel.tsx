...
import type {
  ContentNoteResolution,
  CreativeContentNote,
  CreativeContentNoteScope,
  CreativeContentNoteType,
  ResolvedCreativeContentNote,
} from "../../lib/creative-content-notes/types";

function slug(value: string, fallback: string) {
  const normalized = value.toLowerCase().replace(/[^a-z0-9가-힣]+/g, "-").replace(/^-|-$/g, "").slice(0, 64);
...
export function CreativeContentNotesPanel({ product, onResolvedNotesChange }: Props) {
  ...
  async function addNote() {
    ...
    setTitle(""); setContent(""); setRequired(false); setProhibited(false); setStartsAt(""); setEndsAt("");
    await load();
  ...
  return (
    <details className="creative-content-notes-panel">
      <summary>
        <span><b>광고 콘텐츠 참고사항</b><small>적용 {resolution?.notes.length || 0}개 · 저장 {notes.length}개</small></span>
      ...
      <div className="creative-note-context"><span>광고주 {advertiserId}</span><span>카테고리 {categoryId}</span><span>상품 {productId}</span></div>
      {resolution?.conflicts.map((conflict) => <p className="creative-note-conflict" key={conflict.noteIds.join("-")}>{conflict.message}</p>)}
      {resolution?.notes.length ? <ul className="creative-note-applied-list">{resolution.notes.map((note) => <li key={note.id}><span>{noteTypeLabels[note.type]} · {note.scope}</span><strong>{note.title}</strong><p>{note.content}</p>{note.required ? <b>필수</b> : null}{note.prohibited ? <b className="prohibited">금지</b> : null}<button onClick={() => void toggle(note)} type="button">비활성화</button></li>)}</ul> : <p className="creative-note-empty">현재 상품에 적용할 활성 참고사항이 없습니다.</p>}
      <div className="creative-note-editor">
        <select onChange={(event) => setScope(event.target.value as CreativeContentNoteScope)} value={scope}><option value="advertiser">광고주 공통</option><option value="category">카테고리</option><option value="product">상품</option><option value="promotion">프로모션</option></select>
        <select onChange={(event) => setType(event.target.value as CreativeContentNoteType)} value={type}>{Object.entries(noteTypeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
        ...
        <label>시작일 <input onChange={(event) => setStartsAt(event.target.value)} type="date" value={startsAt} /></label>
        <label>종료일 <input onChange={(event) => setEndsAt(event.target.value)} type="date" value={endsAt} /></label>
        <label><input checked={required} onChange={(event) => setRequired(event.target.checked)} type="checkbox" /> 필수 적용</label>
        <label><input checked={prohibited} onChange={(event) => setProhibited(event.target.checked)} type="checkbox" /> 금지 규칙</label>
        <button onClick={() => void addNote()} type="button">참고사항 저장</button>
      </div>
      {notes.filter((note) => !note.active).length ? <details className="creative-note-inactive"><summary>비활성 참고사항</summary>{notes.filter((note) => !note.active).map((note) => <button key={note.id} onClick={() => void toggle(note)} type="button">{note.title} 다시 활성화</button>)}</details> : null}

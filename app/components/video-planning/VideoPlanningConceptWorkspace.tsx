...
export function VideoPlanningConceptWorkspace({
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.conceptMapPanel}>
        <div className={styles.sectionHead}>
          <div>
            <p className={styles.eyebrow}>CONCEPT SUMMARY</p>
            <h2>콘셉트 요약</h2>
          </div>
        </div>
        <div className={styles.conceptPrimaryHook}>
          <span>첫 자막</span>
          <strong>{concept.openingHook}</strong>
        </div>
        ...
        <details className={styles.conceptDetails}>
          ...
          <div className={styles.conceptDetailsGrid}>
            ...
            <span>타깃 호명</span>
            <strong>{concept.targetCallout || concept.openingHook}</strong>
            <span>자막 말투</span>
            <strong>
              {concept.copyVoiceDirection ||
                "레퍼런스의 호칭·말끝·문장 파편을 상품 상황에 맞춰 유지합니다."}
            </strong>
            <span>전체 흐름</span>
      ...
      {primaryBlueprint ? (
        <details className={styles.blueprintPanel}>
          <summary className={styles.blueprintDisclosureSummary}>
            <div>
              <p className={styles.eyebrow}>REFERENCE BLUEPRINT</p>
              <h2>{primaryBlueprint.title}</h2>
            </div>
            <div className={styles.blueprintDisclosureMeta}>
              <span className={styles.status}>
                {primaryBlueprint.sceneCount}장면 · {primaryBlueprint.duration.toFixed(0)}초
              </span>
              <strong>레퍼런스 구성 보기</strong>
            </div>
          </summary>
          <div className={styles.blueprintBody}>
            <p className={styles.blueprintReason}>{concept.blueprintSelection?.reason}</p>
            <div className={styles.blueprintBeatGrid}>
              {primaryBlueprint.beats.map((beat, index) => (
                <article key={`${primaryBlueprint.id}-${beat.role}`}>
                  <span>
                    {String(index + 1).padStart(2, "0")} · {beat.timing}
                  </span>
                  <strong>{beat.role}</strong>
                  <p>{beat.visual}</p>
                  <small>{beat.direction}</small>
                </article>
              ))}
            </div>
            {secondaryBlueprint ? (
              ...
              <article className={styles.scenePlanCard} key={cut.id}>
                ...
                <div className={styles.sceneDescriptionColumn}>
                  ...
                  (cut.cameraComposition ||
                    cut.motionDirection ||
                    cut.transition ||
                    cut.requiredSources.length) ? (
                    <details className={styles.sceneProductionDetails}>
                      <summary>촬영 상세 보기</summary>
                      <div>
                        {cut.cameraComposition ? (
                          <p>
                            <b>화면·구도</b>
      ...
      <section className={styles.requestPanel}>
        ...
        <div className={styles.formGrid}>
          <label>
            담당 디자이너
            <select required value={VIDEO_DESIGNER_OPTIONS.includes(designerName as (typeof VIDEO_DESIGNER_OPTIONS)[number]) ? designerName : ""} onChange={(event) => setDesignerName(event.target.value)}>

  ...
  async function selectConcept(conceptId: string) {
    ...
    try {
      const response = await fetch(`/api/video-projects/${projectId}`, {
        ...
        body: JSON.stringify({ action: "select-concept", conceptId, actor: project?.marketerName || "마케터" }),
      });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "콘셉트를 선택하지 못했습니다.");
      setProject(payload.project);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "콘셉트 선택 실패"); }
    ...
    <main className={styles.page}>
      <header className={styles.detailHero}>
        ...
        <div className={styles.topActions}>
          ...
          {["production_requested", "in_production", "marketer_review", "revision_requested", "approved"].includes(project.status) ? <Link className={styles.secondaryButton} href={`/video-planning/${project.id}/production`}>제작·검수</Link> : null}
      ...
      <section className={styles.summaryPanel}>
        <div className={styles.sectionHead}>
          <div><h2>서로 다른 영상 콘셉트 4안</h2><p>요약만 비교하고, 자세히 연 콘셉트에 대해서만 15개 이상의 자막·장면안을 생성합니다.</p></div>
          {!project.concepts.length ? <button className={styles.primaryButton} disabled={busy} onClick={generate}>{busy ? "콘셉트 생성 중…" : "4개 콘셉트 다시 생성"}</button> : null}
        </div>
        {project.pipelineProgress?.length ? <div className={styles.pipelineProgress}>{project.pipelineProgress.map((item) => <span data-status={item.status} key={item.stage}>{item.message}</span>)}</div> : null}
            ...
            {project.concepts.map((concept) => {
              ...
              return (
                <article className={styles.conceptCard} data-selected={selected} key={concept.id}>
                  <div className={styles.conceptCardHead}><span className={styles.status}>{archetype?.label || "영상 콘셉트"}</span>{selected ? <b>선택됨</b> : null}</div>
                  <h3>{concept.title}</h3>
                  <blockquote>{concept.openingHook}</blockquote>
                  <dl>
                    <div><dt>중심 사건</dt><dd>{concept.centralIncident || concept.narrativeSummary}</dd></div>
                    <div><dt>화자·시점</dt><dd>{concept.speakerPointOfView || concept.speaker}</dd></div>
                    <div><dt>핵심 소구</dt><dd>{concept.keyAppeal || concept.usp}</dd></div>
                    <div><dt>추천 화면 스타일</dt><dd>{concept.recommendedVisualStyle || archetype?.description}</dd></div>
                    <div><dt>보조 표현</dt><dd>{concept.supportingDevices?.join(" · ") || "상품 근거 중심"}</dd></div>
                    <div><dt>기존안과 차이</dt><dd>{concept.differenceFromPrevious || "첫 사건과 화자 구성을 다르게 설계"}</dd></div>
                  </dl>
                  ...
                  <div className={styles.conceptActions}><Link className={styles.primaryButton} href={`/video-planning/${project.id}/concept/${concept.id}`}>자세히 보기</Link><button className={styles.secondaryButton} disabled={busy || selected} onClick={() => selectConcept(concept.id)}>{selected ? "선택 완료" : "이 콘셉트 선택"}</button></div>
                </article>
              );
            })}
          </div>

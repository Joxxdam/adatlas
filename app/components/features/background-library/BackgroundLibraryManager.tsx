...
export function BackgroundLibraryManager() {
  ...
  const subcategories = useMemo(() => Array.from(new Set(items.flatMap((item) => item.subcategories))).sort(), [items]);
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      if (query && ![item.id, item.scene, ...item.subcategories, ...item.mood, ...item.elements, ...item.colors].join(" ").toLowerCase().includes(query)) return false;
      return true;
    });
  }, [filters, items]);
    ...
    <section className={styles.manager}>
      ...
      <details className={styles.addPanel}>
        ...
        <div className={styles.categoryCreator}>
          <input onChange={(event) => setNewCategory(event.target.value)} placeholder="새 카테고리 (예: outdoor)" value={newCategory} />
          <button disabled={busy || !newCategory.trim()} onClick={() => void addCategory()} type="button">
            카테고리 추가
          </button>
        </div>
        <form className={styles.uploadForm} onSubmit={uploadBackground}>
          <label>
            이미지 파일
            <input accept="image/png,image/jpeg,image/webp,image/avif" name="file" required type="file" />
          </label>
          <label>
            카테고리
          ...
          <label>
            장면 이름
            <input name="scene" placeholder="자연광이 드는 정돈된 주방" required />
          </label>
          <label>
            세부 카테고리
            <input name="subcategories" placeholder="kitchen, home" />
          </label>
          <label>
            후킹 태그
            <input name="hookTypes" placeholder="situation, convenience" />
          </label>
          <label>
            분위기 태그
            <input name="mood" placeholder="따뜻한, 현실적인" />
          </label>
          <label>
            요소 태그
            <input name="elements" placeholder="table, window-light" />
          </label>
          <label>
            색상 태그
            <input name="colors" placeholder="cream, brown" />
          </label>
          <label>
            자산 유형
            <select name="assetType">
              <option value="user_uploaded">사용자 업로드</option>
              <option value="lifestyle_photo">실사 공간</option>
              <option value="people_photo">인물 실사</option>
              <option value="product_set">촬영 세트</option>
              <option value="pattern_texture">패턴·텍스처</option>
              <option value="ingredient_scene">원료 장면</option>
              <option value="ai_generated">검수된 AI</option>
              <option value="designed_asset">제작형</option>
            </select>
          </label>
          <label>
            인물 유무
            <select name="includesPerson">
              <option value="false">인물 없음</option>
              <option value="true">인물 있음</option>
            </select>
          </label>
          <label>
            연령 태그
            <input name="ageGroups" placeholder="twenties, thirties" />
          </label>
          <label>
            문구 영역
            <select name="textSafeArea">
          ...
          <label>
            원본 페이지
            <input name="sourcePageUrl" placeholder="https://..." type="url" />
          </label>
          <label>
            라이선스
            <input name="licenseUrl" placeholder="https://..." type="url" />
          </label>
      ...
      <div className={styles.filters}>
        ...
        <select onChange={(event) => setFilters({ ...filters, subcategory: event.target.value })} value={filters.subcategory}>
          <option value="">전체 세부 카테고리</option>
          {subcategories.map((value) => (
            <option key={value}>{value}</option>
          ))}
        </select>
        <select onChange={(event) => setFilters({ ...filters, ageGroup: event.target.value })} value={filters.ageGroup}>
          <option value="">전체 연령</option>
          <option>teens</option>
          <option>twenties</option>
          <option>thirties</option>
          <option>forties</option>
          <option>fifties</option>
          <option>senior</option>
          <option>kids</option>
          <option>family</option>
          <option>no_people</option>
        </select>
        <select onChange={(event) => setFilters({ ...filters, person: event.target.value })} value={filters.person}>
          <option value="">인물 전체</option>
          <option value="yes">인물 있음</option>
          <option value="no">인물 없음</option>
        </select>
        <select onChange={(event) => setFilters({ ...filters, contentType: event.target.value })} value={filters.contentType}>
      ...
      <p className={styles.resultCount}>
        표시 {filtered.length}개 / 전체 {items.length}개
      </p>
        ...
        {filtered.map((item) => (
          <article className={!item.enabled ? styles.disabled : ""} key={item.id}>
            <img
              ...
              loading="lazy"
              onError={(event) => {
                event.currentTarget.closest("article")?.classList.add(styles.imageError);
              }}
              src={item.file}
            />
            <div className={styles.cardBody}>
              ...
              <span>
                {item.category} · {contentLabel(item)} · {item.includesPerson ? `${item.peopleCount}명` : "인물 없음"}
              </span>
              ...
              <details>
                ...
                <label>
                  장면
                  <input defaultValue={item.scene} id={`${item.id}-scene`} />
                </label>
                <label>
                  태그
                  <input defaultValue={item.elements.join(", ")} id={`${item.id}-tags`} />
                </label>
                <button
                  disabled={busy}
                  onClick={() =>
                    void patchItem(item.id, {
                      ...
                      elements: ((document.getElementById(`${item.id}-tags`) as HTMLInputElement)?.value || "")
                        .split(",")
                ...
                <span>
                  {item.sourceName} {item.authorName ? `· ${item.authorName}` : ""}
                </span>
                {item.sourcePageUrl ? (
              ...
              <div className={styles.actions}>
                <button disabled={busy} onClick={() => void patchItem(item.id, { enabled: !item.enabled })} type="button">
                  {item.enabled ? "비활성화" : "활성화"}
                </button>
                <button className={styles.deleteButton} disabled={busy} onClick={() => void deleteItem(item)} type="button">
                  삭제
                </button>
              </div>
            </div>

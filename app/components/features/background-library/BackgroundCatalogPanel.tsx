...
function recommendationFromItem(item: CatalogItemView): BackgroundRecommendation {
  return {
    ...
    diversityScore: 0,
    reasons: [
      item.recommendedProductPosition.includes("right") ? "상품을 오른쪽에 배치할 여백이 넓어요." : "상품 배치 여백을 분석한 배경이에요.",
      item.overlayReadability >= 0.6 ? "문구 가독성이 좋아요." : "오버레이를 적용하면 문구가 잘 보여요.",
    ],
...
export function BackgroundCatalogPanel(props: {
  product: Partial<ProductInfoForPrompt>;
  hook: CreativeStrategy;
  selectedBackgroundSource: string;
  onSelectBackground: (recommendation: BackgroundRecommendation) => void;
  onSelectFixedBackground: (source: string, label: string) => void;
}) {
  ...
  async function loadRecommendations() {
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : "추천 오류"); }
    finally { setLoading(false); }
  }
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      if (activeTab === "recommended") void loadRecommendationsRef.current();
        ...
        void fetch("/api/background-library/comfyui").then((response) => response.json()).then((result) => setComfyStatus(result.status || null)).catch(() => setComfyStatus({ available: false, workflowValid: false }));
      } else if (activeTab === "pexels") {
        void fetch("/api/background-library/pexels").then((response) => response.json()).then((result) => setPexelsAvailable(Boolean(result.status?.available))).catch(() => setPexelsAvailable(false));
      }
    }, 0);
  ...
  async function searchPexelsPhotos() {
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : "Pexels 검색 오류"); }
    finally { setLoading(false); }
  }

  async function savePexels(photo: PexelsSearchPhoto) {
    if (!selectedConfig || !filters.category) { setMessage("저장할 컬렉션과 카테고리를 먼저 선택해주세요."); return; }
    setLoading(true);
    try {
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : "Pexels 저장 오류"); }
    finally { setLoading(false); }
  }

  async function uploadBackground() {
    if (!uploadFile || !selectedConfig || !filters.category) { setMessage("파일·컬렉션·카테고리를 선택해주세요."); return; }
    const form = new FormData();
    form.set("file", uploadFile);
    form.set("collectionId", selectedConfig.id);
    form.set("categoryId", filters.category);
    form.set("license", JSON.stringify({
      sourceName: licenseForm.sourceName, licenseType: licenseForm.licenseType, licenseUrl: licenseForm.licenseUrl,
      proofPath: licenseForm.proofPath, commercialUseAllowed: licenseForm.commercial,
    ...
    try {
      ...
      setUploadFile(null);
    } catch (error) { setMessage(error instanceof Error ? error.message : "업로드 오류"); }
    finally { setLoading(false); }
  }
  ...
  const cards: CatalogCard[] = activeTab === "recommended"
    ? recommendations.map((recommendation) => ({ key: recommendation.background.id, image: recommendation.background.file, title: recommendation.background.scene, badges: recommendation.reasons, onSelect: () => props.onSelectBackground(recommendation), selected: props.selectedBackgroundSource === recommendation.background.file }))
    : items.map((item) => ({ key: item.id, image: item.thumbnailUrl, title: item.sceneType, badges: [item.licenseStatus === "verified" ? "라이선스 확인" : "라이선스 미확인", `합성 ${Math.round(item.adCompositionScore * 100)}점`], onSelect: () => item.status === "approved" && item.licenseStatus === "verified" ? props.onSelectBackground(recommendationFromItem(item)) : setMessage("승인 및 라이선스 확인이 끝난 배경만 광고에 적용할 수 있습니다."), selected: props.selectedBackgroundSource === item.background.file, item }));

  return (
    <section className={styles.panel} aria-label="대규모 광고 배경 라이브러리">
      <header className={styles.header}>
        <div><strong>광고 배경 라이브러리</strong><p>유료 생성 없이 보유 이미지·Pexels 선택 저장·로컬 ComfyUI를 사용합니다.</p></div>
      ...
      <div className={styles.tabs} role="tablist">
        {tabs.map((tab) => <button aria-selected={activeTab === tab.id} className={activeTab === tab.id ? styles.activeTab : ""} key={tab.id} onClick={() => setActiveTab(tab.id)} role="tab" type="button">{tab.label}</button>)}
      </div>
      ...
      {["owned", "pexels", "generated", "upload"].includes(activeTab) ? (
        <div className={styles.filters}>
          ...
          <select aria-label="세부 카테고리" value={filters.category} onChange={(event) => setFilters((current) => ({ ...current, category: event.target.value }))}>
            <option value="">카테고리 전체</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}
          </select>
          {activeTab === "owned" ? <>
            ...
            <select aria-label="밝기" value={filters.brightness} onChange={(event) => setFilters((current) => ({ ...current, brightness: event.target.value }))}><option value="">밝기 전체</option><option value="bright">밝음</option><option value="medium">중간</option><option value="dark">어두움</option></select>
            <select aria-label="인물" value={filters.people} onChange={(event) => setFilters((current) => ({ ...current, people: event.target.value }))}><option value="all">인물 전체</option><option value="none">인물 없음</option><option value="included">인물 포함</option></select>
            <select aria-label="정렬" value={filters.sort} onChange={(event) => setFilters((current) => ({ ...current, sort: event.target.value }))}><option value="recommended">추천순</option><option value="latest">최신순</option><option value="shuffle">셔플</option></select>
            <button onClick={() => void loadCatalog(1)} type="button">필터 적용</button>
          </> : null}
        </div>
      ) : null}
      {activeTab === "owned" ? <details className={styles.advancedFilters}><summary>상세 필터</summary><div>
        ...
        <select aria-label="문구 여백" onChange={(event) => setFilters((current) => ({ ...current, negativeSpace: event.target.value }))} value={filters.negativeSpace}><option value="">여백 전체</option><option value="center-left">왼쪽</option><option value="center-right">오른쪽</option><option value="top-center">위</option><option value="bottom-center">아래</option></select>
        <select aria-label="실내 실외" onChange={(event) => setFilters((current) => ({ ...current, indoorOutdoor: event.target.value }))} value={filters.indoorOutdoor}><option value="">실내·실외 전체</option><option value="indoor">실내</option><option value="outdoor">실외</option><option value="mixed">혼합</option></select>
        <select aria-label="라이선스" onChange={(event) => setFilters((current) => ({ ...current, license: event.target.value }))} value={filters.license}><option value="">라이선스 전체</option><option value="verified">확인 완료</option><option value="unverified">미확인</option></select>
        <select aria-label="출처" onChange={(event) => setFilters((current) => ({ ...current, source: event.target.value }))} value={filters.source}><option value="">출처 전체</option><option value="local-import">보유 이미지</option><option value="pexels">Pexels</option><option value="local-generation">로컬 생성</option></select>
        <label><input checked={filters.favorite} onChange={(event) => setFilters((current) => ({ ...current, favorite: event.target.checked }))} type="checkbox" /> 즐겨찾기만</label>
      </div></details> : null}

      {activeTab === "pexels" ? (
        <div className={styles.toolArea}>
          <div className={styles.inline}><input disabled={!pexelsAvailable} onChange={(event) => setPexelsQuery(event.target.value)} placeholder="Pexels 영어 검색어" value={pexelsQuery} /><button disabled={!pexelsAvailable || loading} onClick={() => void searchPexelsPhotos()} type="button">무료 검색</button></div>
          ...
          <div className={styles.grid}>{pexelsPhotos.map((photo) => <article className={styles.card} key={photo.id}><img alt={photo.alt} loading="lazy" src={photo.thumbnailUrl} /><strong>{photo.alt || `Pexels #${photo.id}`}</strong><a href={photo.sourcePageUrl} rel="noreferrer" target="_blank">Photo by {photo.photographerName} on Pexels</a><button onClick={() => void savePexels(photo)} type="button">광고 배경 후보로 저장</button></article>)}</div>
          {pexelsPhotos.length ? <a href="https://www.pexels.com/license/" rel="noreferrer" target="_blank">Pexels 라이선스 확인</a> : null}
        </div>
      ) : null}

      {activeTab === "solid" ? <div className={styles.grid}>{fixedBackgrounds.map((background) => <button className={`${styles.swatch} ${props.selectedBackgroundSource.startsWith("data:image/svg+xml") ? "" : ""}`} key={background.id} onClick={() => props.onSelectFixedBackground(fixedDataUrl(background.colors), background.label)} style={{ background: background.css }} type="button"><span>{background.label}</span></button>)}</div> : null}

      {activeTab === "generated" ? <div className={styles.toolArea}><p>{comfyStatus?.available && comfyStatus.workflowValid ? "로컬 ComfyUI 연결 완료" : `로컬 ComfyUI unavailable · ${comfyStatus?.workflowError || "환경변수와 workflow를 설정해주세요."}`}</p><code>npm run backgrounds:comfy:plan -- --collection {selectedConfig?.id || "collection-id"} --dry-run</code></div> : null}

      {activeTab === "upload" ? <div className={styles.uploadArea}>
        ...
        <label><input checked={licenseForm.commercial} onChange={(event) => setLicenseForm((current) => ({ ...current, commercial: event.target.checked }))} type="checkbox" /> 상업적 사용 권한을 확인했습니다.</label>
        <label><input checked={licenseForm.reviewed} onChange={(event) => setLicenseForm((current) => ({ ...current, reviewed: event.target.checked }))} type="checkbox" /> 인물·로고·문구 위험을 직접 검수했습니다.</label>
        <button disabled={loading || !uploadFile || !filters.category} onClick={() => void uploadBackground()} type="button">안전하게 가져오기</button>
      </div> : null}

      {["recommended", "owned", "generated"].includes(activeTab) ? <div className={styles.grid} aria-busy={loading}>{loading ? Array.from({ length: 6 }, (_, index) => <div className={styles.skeleton} key={index} />) : cards.map((card) => <article className={`${styles.card} ${card.selected ? styles.selected : ""}`} key={card.key}><button onClick={card.onSelect} type="button"><img alt={card.title} loading="lazy" src={card.image} /><strong>{card.title}</strong></button><div className={styles.badges}>{card.badges.map((badge) => <span key={badge}>{badge}</span>)}</div>{card.item ? <div className={styles.actions}><button aria-label="즐겨찾기" onClick={() => void patchItem(card.item!, { favorite: !card.item!.favorite })} type="button">{card.item.favorite ? "★" : "☆"}</button>{card.item.status === "review" && card.item.licenseStatus === "verified" ? <button onClick={() => void patchItem(card.item!, { status: "approved" })} type="button">검수 승인</button> : null}<button onClick={() => void patchItem(card.item!, { status: "inactive" })} type="button">사용 제외</button></div> : null}</article>)}</div> : null}

      {activeTab === "owned" && totalPages > 1 ? <div className={styles.pagination}><button disabled={page <= 1} onClick={() => void loadCatalog(page - 1)} type="button">이전</button><span>{page}/{totalPages}</span><button disabled={page >= totalPages} onClick={() => void loadCatalog(page + 1)} type="button">다음</button></div> : null}
      {summary ? <div className={styles.counts}>{summary.collections.map((collection) => <span key={collection.id}>{collection.displayName}: {collection.productionReadyCount}/{collection.targetCount} · 부족 {collection.missingCount}</span>)}</div> : null}

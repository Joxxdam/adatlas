...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "connection") return NextResponse.json({ ok: true, result: await service.verifyConnection() });
    if (action === "accounts") {
      const config = readMetaServerConfig();
      const store = await metaRepository.read();
      const advertiserId = String(body.advertiserId || "");
      const mapping = store.advertiserMappings.find((item) => item.advertiserId === advertiserId) || config.advertiserMap.find((item) => item.advertiserId === advertiserId);
    ...
    return NextResponse.json({ ok: false, error: "지원하지 않는 Meta 읽기 작업입니다." }, { status: 400 });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Meta 읽기 실패" }, { status: 400 });
  }
}

...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!mapping.advertiserId?.trim() || !mapping.advertiserName?.trim()) throw new Error("광고주 ID와 이름이 필요합니다.");
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "광고주 설정 저장 실패" }, { status: 400 });
  }
}

...
export async function GET(
  request: Request,
  context: { params: Promise<{ analysisId: string; productId: string }> }
) {
  try {
    ...
    if (!result) {
      return NextResponse.json(
        { ok: false, error: "분석 결과를 찾지 못했습니다." },
        { status: 404 }
      );
    }
    ...
    if (!product || !handoff) {
      return NextResponse.json(
        { ok: false, error: "분석 상품을 찾지 못했습니다." },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "상품 분석 결과 조회 실패" },
      { status: 400 }
    );
  }
}

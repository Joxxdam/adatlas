...
export async function POST(_request: Request, context: { params: Promise<{ analysisId: string }> }) {
  try {
    ...
    if (!previous) {
      return NextResponse.json({ ok: false, error: "기존 분석 결과를 찾지 못했습니다." }, { status: 404 });
    }
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "재분석 실패" }, { status: 500 });
  }
}

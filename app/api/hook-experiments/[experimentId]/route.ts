...
export async function GET(_request: Request, context: { params: Promise<{ experimentId: string }> }) {
  try {
    ...
    if (!experiment) return NextResponse.json({ ok: false, error: "실험을 찾지 못했습니다." }, { status: 404 });
    return NextResponse.json({ ok: true, experiment });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "실험 조회 실패" }, { status: 400 });
  }
}
...
export async function PATCH(request: Request, context: { params: Promise<{ experimentId: string }> }) {
  try {
    ...
    if (body.action === "update-registration" && body.experimentAssetId) {
      const relation = await hookExperimentRepository.updateAssetRegistration(experimentId, body.experimentAssetId, {
    ...
    if (body.action === "update-experiment") {
      ...
      if (!current) return NextResponse.json({ ok: false, error: "실험을 찾지 못했습니다." }, { status: 404 });
    ...
    if (body.action === "link-asset" && body.experimentAssetId && body.assetCode) {
      ...
      if (!asset) return NextResponse.json({ ok: false, error: "연결할 소재코드를 찾지 못했습니다." }, { status: 404 });
      ...
      if (!slot) return NextResponse.json({ ok: false, error: "연결할 실험 슬롯을 찾지 못했습니다." }, { status: 404 });
      if (asset.hookCode !== slot.hookCode) {
        return NextResponse.json({ ok: false, error: `후킹 코드가 다릅니다. ${slot.hookCode} 소재만 연결할 수 있습니다.` }, { status: 409 });
      }
    ...
    return NextResponse.json({ ok: false, error: "지원하지 않는 수정 요청입니다." }, { status: 400 });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "실험 수정 실패" }, { status: 400 });
  }
}

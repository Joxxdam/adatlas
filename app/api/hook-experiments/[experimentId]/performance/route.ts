...
import { CreativePerformanceMatchingService, PerformanceImportService } from "../../../../lib/hook-experiments/performanceImport.server";
...
export async function GET(_request: Request, context: { params: Promise<{ experimentId: string }> }) {
  try {
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "성과 조회 실패" }, { status: 400 });
  }
}
...
export async function POST(request: Request, context: { params: Promise<{ experimentId: string }> }) {
  try {
    ...
    if (!(file instanceof File) || file.size === 0) {
      return NextResponse.json({ ok: false, error: "Meta 보고서 CSV 또는 XLSX 파일이 필요합니다." }, { status: 400 });
    }
    if (file.size > 20 * 1024 * 1024) {
      return NextResponse.json({ ok: false, error: "보고서 파일은 20MB 이하여야 합니다." }, { status: 413 });
    }
    const rows = PerformanceImportService.parse(Buffer.from(await file.arrayBuffer()), file.name, snapshot.experiment.objective);
    if (!rows.length) return NextResponse.json({ ok: false, error: "보고서에서 읽을 수 있는 행이 없습니다." }, { status: 400 });
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "성과 보고서 처리 실패" }, { status: 500 });
  }
}
...
export async function PATCH(request: Request, context: { params: Promise<{ experimentId: string }> }) {
  try {
    ...
    const snapshot = await hookExperimentRepository.get(experimentId);
    const relation = snapshot?.experimentAssets.find((item) => item.id === body.experimentAssetId && item.assetId && item.assetCode);
    if (!snapshot || !body.recordId || !relation?.assetId || !relation.assetCode) {
      return NextResponse.json({ ok: false, error: "보고서 행과 연결할 실험 소재를 선택해 주세요." }, { status: 400 });
    }
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "수동 연결 실패" }, { status: 400 });
  }
}

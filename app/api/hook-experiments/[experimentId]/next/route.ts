...
export async function POST(_request: Request, context: { params: Promise<{ experimentId: string }> }) {
  try {
    ...
    if (!current?.analysis) return NextResponse.json({ ok: false, error: "먼저 성과 보고서를 분석해 주세요." }, { status: 409 });
    ...
    if (await hookExperimentRepository.findByCode(plan.experiment.experimentCode)) {
      return NextResponse.json({ ok: false, error: "다음 단계 실험이 이미 존재합니다." }, { status: 409 });
    }
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "다음 실험 생성 실패" }, { status: 400 });
  }
}

...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "실험 목록 조회 실패" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (await hookExperimentRepository.findByCode(plan.experiment.experimentCode)) {
      return NextResponse.json(
        { ok: false, error: "같은 상품·목표·회차의 실험이 이미 존재합니다." },
        { status: 409 }
      );
    }
    ...
    return NextResponse.json(
      ...
      { status: /이미 존재/.test(message) ? 409 : 500 }
    );
  }
}

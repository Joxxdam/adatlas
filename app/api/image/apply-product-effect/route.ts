...
export async function POST(request: Request) {
  try {
    ...
    const effectPreset = effectPresets.has(body.effectPreset || "outline-glow-shadow") ? body.effectPreset || "outline-glow-shadow" : "outline-glow-shadow";

    if (!cutoutImagePath) {
      return NextResponse.json({ success: false, error: "cutoutImagePath is required." }, { status: 400 });
    }
    ...
    const styledCutoutImagePath = await saveProcessedProductImage(styledBuffer, `cutout-effect-${Date.now()}-${crypto.randomBytes(4).toString("hex")}.png`);
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "효과 적용에 실패했습니다. 다른 이미지를 선택해 주세요.",
      },
      { status: 500 }
    );
  }

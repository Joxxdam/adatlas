...
async function assessScene(buffer: Buffer): Promise<SceneAssessment> {
  const resizedBuffer = await sharp(buffer)
    .rotate()
    .resize(1200, 1200, { fit: "cover", position: "centre", kernel: sharp.kernel.lanczos3 })
    .ensureAlpha()
    .png()
    .toBuffer();
  ...
  const sample = await sharp(resizedBuffer)
  ...
  const score = Math.max(
    ...
    Math.min(
      ...
      Math.round(
        100 -
          Math.min(58, transparencyRatio * 360) -
          Math.max(0, darkPixelRatio - 0.3) * 55 -
          Math.max(0, 34 - brightness) * 0.7 -
          Math.max(0, brightness - 232) * 0.6 -
          Math.max(0, 14 - contrast) * 1.4
      )
    )
  );
  ...
  try {
    ...
    if (!direction?.scenePromptPlan?.prompt || !direction.id) {
      return NextResponse.json(
        { ok: false, error: "먼저 비주얼 방향을 선택해주세요." },
        { status: 400 }
      );
    }
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      try {
        ...
        if (assessment.needsRetry && result.provider !== "mock") {
          ...
          try {
            const retryResult = await primary.generateScene(
              sceneInput(direction, repairPrompt(variationPrompt, assessment))
            );
            ...
            const retryMessage =
              retryError instanceof Error ? retryError.message : "배경 품질 재생성 실패";
    ...
    return NextResponse.json({
      ...
      message: fallbackUsed
        ? "AI 배경 생성 연결이 불가능해 카테고리별 안전 배경을 만들었습니다. API 키와 서버 로그를 확인해주세요."
        : reviewed
          ? `${candidates.length}개의 장면을 생성했으며 ${reviewed}개는 추가 확인이 필요합니다.`
          : `${candidates.length}개의 불투명 1200x1200 광고 장면을 생성했습니다.`,
      errors,
    });
  } catch (error) {
    return NextResponse.json(

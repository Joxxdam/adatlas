...
export async function POST(request: Request) {
  try {
    ...
    if (!actualType || actualType !== file.type) {
      return NextResponse.json({ success: false, error: "파일 내용과 MIME 형식이 일치하지 않는 이미지입니다." }, { status: 400 });
    }
    ...
    if (!width || !height || width > 10_000 || height > 10_000 || width * height > maxImagePixels) {
      return NextResponse.json({ success: false, error: "이미지 해상도는 최대 10,000px, 총 4천만 픽셀까지 지원합니다." }, { status: 400 });
    }

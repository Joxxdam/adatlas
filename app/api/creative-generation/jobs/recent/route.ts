...
export async function GET(request: Request) {
  try {
    ...
    const jobs = (await creativeGenerationJobStore.recentFor({ advertiserId, productId, limit: principal.kind === "cloudflare-access" ? 500 : limit }))
      .filter((job) => canAccessGenerationJob(principal, job))
      .slice(0, limit);
    ...
    return NextResponse.json({ ok: false, error: toPublicGenerationError(error, "최근 작업 조회 실패") }, { status: localAccessErrorStatus(error) || (localAccessError(error) ? 403 : 500) });
  }
}

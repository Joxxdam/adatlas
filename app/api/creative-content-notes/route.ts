...
export async function POST(request: Request) {
  ...
  const note = await creativeContentNoteRepository.create({
    advertiserId: body.advertiserId.trim(), scope: body.scope, scopeId: body.scopeId.trim(), type: body.type,
    title: body.title.trim().slice(0, 120), content: body.content.trim().slice(0, 1000), required: Boolean(body.required), prohibited: Boolean(body.prohibited), active: body.active !== false,
    startsAt: body.startsAt || null, endsAt: body.endsAt || null, source: body.source || "user",
  });

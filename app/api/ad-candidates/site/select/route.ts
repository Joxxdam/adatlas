...
export async function POST(request: Request) {
  try {
    ...
    if (!/^site-analysis-[a-f0-9-]{36}$/i.test(analysisId) || !candidateId.startsWith("site-candidate-")) {
      return NextResponse.json({ ok: false, error: "선택할 사이트 광고 후보 정보가 올바르지 않습니다." }, { status: 400 });
    }
    const selection = selectSiteCandidate(analysisId, candidateId);
    return NextResponse.json({
      ok: true,
      selectionId: selection.selectionId,
      nextUrl: buildProductCreationHref({ siteCandidateId: selection.selectionId }, selection.candidate.product.productUrl),
    });
  } catch (error) {

...
export async function POST(request: Request) {
  try {
    ...
    if (file.size <= 0 || file.size > MAX_REFERENCE_BYTES) throw new Error("참고 파일은 100MB 이하만 업로드할 수 있습니다.");
    const buffer = Buffer.from(await file.arrayBuffer());
    const detectedType = detectReferenceType(buffer);
    if (!detectedType) throw new Error("PNG, JPG, WEBP, PDF, MP4, MOV, WEBM 파일만 첨부할 수 있습니다.");
    if (role === "product-original" && !detectedType.startsWith("image/")) throw new Error("상품 원본은 PNG, JPG, WEBP 이미지만 업로드할 수 있습니다.");
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "참고 파일 업로드 실패" }, { status: 400 });
  }
}

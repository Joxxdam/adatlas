...
import {
  ...
  validateCreateVideoProjectInput,
} from "../../lib/video-collaboration/repository.server";
...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "프로젝트 목록 조회 실패" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "프로젝트 생성 실패" },
      { status: 400 }
    );
  }
}

...
export async function POST(request: Request) {
  try {
    ...
    const afterCompleteAutoProductionAdvertiserIds = Array.isArray(body.afterCompleteAutoProductionAdvertiserIds)
      ? body.afterCompleteAutoProductionAdvertiserIds.map((value: unknown) => String(value || "").trim()).filter(Boolean)
      : undefined;
    const status = await startReferenceOcrRun({ ids, retryFailed: body.action === "retry-failed", force: body.action === "retry-failed", afterCompleteAutoProductionAdvertiserId, afterCompleteAutoProductionAdvertiserIds });
...
export async function PATCH(request: Request) {
  try {
    ...
    if (body.action !== "cancel") throw new Error("cancel 또는 prioritize-food 액션이 필요합니다.");

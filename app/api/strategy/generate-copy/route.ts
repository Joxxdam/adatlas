...
import { kookdaeGenericForbiddenPhrases, normalizeKookdaePunctuation } from "../../../lib/mvp/kookdaeCopyPatterns";
import { analyzeProductUsp, buildTargetedCopyVariants, buildUspFirstFallbackCopy, isCopyGroundedInProductUsp } from "../../../lib/mvp/productUsp";
...
import type { AdBrief, AdImageLabel, CopyGuideContext, CreativeStrategy, GeneratedAdCopy, GeneratedAdCopyVariant, ProductInfoForPrompt, ReferenceUsageSelection } from "../../../lib/mvp/types";
...
const forbiddenPhrases = ["만나보세요", "기다립니다", "필수 아이템", "특별한 선택", "자세한 정보", "여기를 클릭", "새로워진 즐거움", "만족을 줄 수 있음", "여러분을 기다립니다", "지금 바로 확인하기", "meet", "waiting", "must-have", "special choice", "click here", "learn more", "undefined", "null", "nan"];

const copySlots = ["headline", "bodyCopy", "highlightCopy", "bottomBarCopy", "cta", "price"] as const;

const kookdaeGuideExamplePattern = /(사장님|정육점|직원인 저|담당자 컨펌|경리팀|본사에서 뒤집|마진(?:도)? 안 남|손해 보고|오타 아닙니다|두 번 (?:놀랐|확인)|반신반의|인생 고기|진심 미쳤|가격 실화냐|사장님만 모릅니다|캠핑용 고기로|입에서 살살 녹|배터지게|잡내 없이 부드러운|가족 선물각|오늘의 특가 구성)/i;
...
function hasUnsupportedFact(value: string, product: ProductInfoForPrompt) {
  const text = cleanText(value);
  const facts = cleanText([product.productName, product.price, product.originalPrice, product.oldPrice, product.discountInfo, product.mainBenefit, product.extractedDescription, ...(product.verifiedBenefits || []), ...(product.ingredients || []), ...(product.reviewSources || []).map((review) => review.keySentence)].join(" "));
  const allowedNumbers = new Set((facts.match(/\d[\d,.]*/g) || []).map((number) => number.replace(/[,.]/g, "")));
  const generatedNumbers = (text.match(/\d[\d,.]*/g) || []).map((number) => number.replace(/[,.]/g, ""));
  if (generatedNumbers.some((number) => !allowedNumbers.has(number))) return true;
  const guardedClaims: Array<[RegExp, RegExp]> = [
    [/무료\s*배송/i, /무료\s*배송/i],
    [/오늘만|오늘까지|기간\s*한정|마감\s*임박|품절\s*임박|한정\s*수량/i, /오늘만|오늘까지|기간\s*한정|마감\s*임박|품절\s*임박|한정\s*수량/i],
...
function hasKookdaeForbidden(value: string) {
  const text = cleanText(value);
  return kookdaeGuideExamplePattern.test(text) || kookdaeGenericForbiddenPhrases.some((phrase) => text.includes(phrase));
}

function isBadHeadline(value: string, copyGuide?: CopyGuideContext | null, product?: ProductInfoForPrompt) {
  const text = cleanText(value);
  ...
  if (product && (hasUnsupportedFact(text, product) || !isCopyGroundedInProductUsp(text, product))) {
    return true;
  }
  return false;
}

function isObjectiveHeadlineAligned(value: string, brief: AdBrief | undefined, product: ProductInfoForPrompt) {
...
function safeHeadlineFallback(product: ProductInfoForPrompt, reference?: AdImageLabel) {
  ...
  const source = cleanText(reference?.finalLabel?.firstLineHook || reference?.finalLabel?.reusableCopyPattern || product.mainBenefit || product.productName || product.category);
  if (price && uspFallback.headline) return trimToLimit(`${uspFallback.headline}, ${price}`, 34);
  return trimToLimit(uspFallback.headline || source || product.productName || "상품의 핵심 차이", 34);
}

function bodyFallback(product: ProductInfoForPrompt, reference?: AdImageLabel, copyGuide?: CopyGuideContext | null) {
  if (isKookdaeGuide(copyGuide)) {
    return buildUspFirstFallbackCopy(product).bodyCopy;
  }

  const source = cleanText(analyzeProductUsp(product).uspSignals[1] || product.mainBenefit || reference?.finalLabel?.consumerInsight || product.productName || product.category);
...
function normalizeCta(value?: string, copyGuide?: CopyGuideContext | null, brief?: AdBrief, hasConfirmedOffer = false) {
  const text = isKookdaeGuide(copyGuide) ? applyKookdaeGenericReplacements(value || "") : cleanText(value);
  if (text && !hasForbidden(text) && !hasKookdaeForbidden(text) && (!brief || isObjectiveCtaAligned(text, brief.adObjective))) {
...
function referencePatternUsage(reference?: AdImageLabel): GeneratedAdCopy["referencePatternUsage"] {
  ...
  return {
    ...
    appliedPatterns: [finalLabel?.reusableCopyPattern, finalLabel?.firstLineHook, finalLabel?.copyStructure, finalLabel?.toneOfVoice || finalLabel?.copyNuance, finalLabel?.purchaseTrigger].filter(Boolean) as string[],
...
function guideHeadings(guide: CopyGuideContext) {
  return Array.from(guide.content.matchAll(/^#{2,3}\s+(.+)$/gm)).map((match) => cleanText(match[1]));
}
...
function guideSectionsForUsage(guide: CopyGuideContext) {
  ...
  const picked = [firstHeadingMatching(headings, /^1\./), firstHeadingMatching(headings, /^1-A\./), firstHeadingMatching(headings, /^1-B\./), firstHeadingMatching(headings, /^1-C\./), firstHeadingMatching(headings, /^1-D\./), firstHeadingMatching(headings, /^2\./), firstHeadingMatching(headings, /^3\./), firstHeadingMatching(headings, /^6\./)].filter(Boolean) as string[];
...
function variantFrom(copy: Partial<GeneratedAdCopyVariant> | undefined, fallback: GeneratedAdCopyVariant, limits: Record<keyof GeneratedAdCopyVariant, number>, copyGuide?: CopyGuideContext | null, product?: ProductInfoForPrompt): GeneratedAdCopyVariant {
  const sanitize = (value: string) => (isKookdaeGuide(copyGuide) ? applyKookdaeGenericReplacements(value) : value);
  const safe = (incoming: string | undefined, fallback: string) => {
    const value = sanitize(incoming || fallback);
    return product && (hasForbidden(value) || hasUnsupportedFact(value, product)) ? sanitize(fallback) : value;
  };
  ...
  return {
    ...
    highlightCopy: trimToLimit(safe(copy?.highlightCopy, fallback.highlightCopy), limits.highlightCopy),
    bottomBarCopy: trimToLimit(safe(copy?.bottomBarCopy, fallback.bottomBarCopy), limits.bottomBarCopy),
    cta: trimToLimit(safe(copy?.cta, fallback.cta), limits.cta),
    price: trimToLimit(product && hasUnsupportedFact(copy?.price || "", product) ? fallback.price || "" : copy?.price || fallback.price || "", limits.price),
  };
}

function alignVariantToObjective(incoming: Partial<GeneratedAdCopyVariant> | undefined, fallback: GeneratedAdCopyVariant, brief: AdBrief | undefined, product: ProductInfoForPrompt) {
  if (!incoming || !brief) return incoming;
  const headline = incoming.headline || "";
  return {
    ...incoming,
    headline: isObjectiveHeadlineAligned(headline, brief, product) && isCopyGroundedInProductUsp(headline, product) ? incoming.headline : fallback.headline,
    cta: isObjectiveCtaAligned(incoming.cta || "", brief.adObjective) ? incoming.cta : fallback.cta,
  };
}

function kookdaeVariantSlotValue(slot: keyof GeneratedAdCopyVariant, value: string | undefined, fallback: string, product: ProductInfoForPrompt) {
  const raw = cleanText(value);
  if (!raw || hasForbidden(raw) || hasKookdaeForbidden(raw) || (slot === "headline" && !isCopyGroundedInProductUsp(raw, product))) {
    return fallback;
  }
...
function mergeKookdaeVariant(incoming: Partial<GeneratedAdCopyVariant> | undefined, fallback: GeneratedAdCopyVariant, product: ProductInfoForPrompt): GeneratedAdCopyVariant {
  return {
    ...
    highlightCopy: kookdaeVariantSlotValue("highlightCopy", incoming?.highlightCopy, fallback.highlightCopy, product),
    bottomBarCopy: kookdaeVariantSlotValue("bottomBarCopy", incoming?.bottomBarCopy, fallback.bottomBarCopy, product),
    cta: kookdaeVariantSlotValue("cta", incoming?.cta, fallback.cta, product),
    price: cleanText(incoming?.price || fallback.price || ""),
  };
}

function buildCopyVariants(copy: Partial<GeneratedAdCopy>, product: ProductInfoForPrompt, reference?: AdImageLabel, copyGuide?: CopyGuideContext | null, adBrief?: AdBrief, creativeStrategy?: CreativeStrategy | null): GeneratedAdCopy["copyVariants"] {
  if (isKookdaeGuide(copyGuide)) {
    const fallbackVariants = adBrief ? buildTargetedCopyVariants({ product, brief: adBrief, strategy: creativeStrategy }) : buildUspFirstFallbackCopy(product).variants;
    const preferred = {
      short: mergeKookdaeVariant(alignVariantToObjective(copy.copyVariants?.short, fallbackVariants.short, adBrief, product), fallbackVariants.short, product),
      medium: mergeKookdaeVariant(alignVariantToObjective(copy.copyVariants?.medium, fallbackVariants.medium, adBrief, product), fallbackVariants.medium, product),
      long: mergeKookdaeVariant(alignVariantToObjective(copy.copyVariants?.long, fallbackVariants.long, adBrief, product), fallbackVariants.long, product),
    };
  ...
  const fallback: GeneratedAdCopyVariant = {
    ...
    bottomBarCopy: copy.bottomBarCopy || reference?.finalLabel?.purchaseTrigger || reference?.finalLabel?.whyItWorks || "check bundle",
    cta: copy.cta || "상품 보기",
    price: normalizePrice(product, copy.price),
  };
  const targetedFallbacks = adBrief ? buildTargetedCopyVariants({ product, brief: adBrief, strategy: creativeStrategy }) : { short: fallback, medium: fallback, long: fallback };
  ...
  return {
    ...
    medium: variantFrom(
      alignVariantToObjective(copy.copyVariants?.medium, targetedFallbacks.medium, adBrief, product),
...
function removeForbidden(copy: GeneratedAdCopy, product: ProductInfoForPrompt, reference?: AdImageLabel, copyGuide?: CopyGuideContext | null): GeneratedAdCopy {
  ...
  copySlots.forEach((key) => {
    if (key !== "price" && (hasForbidden(String(next[key] || "")) || hasUnsupportedFact(String(next[key] || ""), product) || (isKookdaeGuide(copyGuide) && hasKookdaeForbidden(String(next[key] || ""))))) {
...
function applyBriefConstraints(copy: GeneratedAdCopy, brief: AdBrief, product: ProductInfoForPrompt, labels: AdImageLabel[], copyGuide?: CopyGuideContext | null) {
  const prohibitedClaims = brief.prohibitedClaims || [];
  const normalizedProhibited = new Set(prohibitedClaims.map((claim) => cleanText(claim).toLowerCase()));
  const mandatoryInfo = (brief.mandatoryInfo || []).map(cleanText).filter((item) => item && !normalizedProhibited.has(item.toLowerCase()));
  ...
  const constrainVariant = <Variant extends GeneratedAdCopyVariant>(variant: Variant): Variant => {
    ...
    if (!next.cta) {
      next.cta = normalizeCta(undefined, copyGuide, brief, Boolean(product.discountInfo || product.price));
    }
...
function normalizeGeneratedCopy(value: Partial<GeneratedAdCopy>, product: ProductInfoForPrompt, labels: AdImageLabel[], copyGuide?: CopyGuideContext | null, context?: { adBrief?: AdBrief; creativeStrategy?: CreativeStrategy | null }): GeneratedAdCopy {
  const reference = labels[0];
  ...
  const objectiveFallback = context?.adBrief
    ...
    : undefined;
  const rejectedGenericExpressions = isKookdaeGuide(copyGuide) ? rejectedKookdaeGenericExpressions(value) : [];
  const headlineSource = isKookdaeGuide(copyGuide) ? applyKookdaeGenericReplacements(value.headline || "") : value.headline || "";
  const headlineNeedsRepair = isBadHeadline(headlineSource, copyGuide, product) || !isObjectiveHeadlineAligned(headlineSource, context?.adBrief, product);
  const headline = headlineNeedsRepair ? objectiveFallback?.headline || safeHeadlineFallback(product, reference) : cleanText(headlineSource);
  const selectedKookdaePattern = isKookdaeGuide(copyGuide) ? `USP 우선: ${productUsp.primaryUsp}` : "";
  ...
  const usedReferenceIds = Array.from(new Set([...labels.map((label) => label.imageId), ...(value.referencePatternUsage?.usedReferenceIds || [])]));
  const normalizedBodyCopy = normalizeBody(value.bodyCopy || "", bodyFallback(product, reference, copyGuide), copyGuide);
  const normalizedHighlightCopy = trimToLimit(cleanText(isKookdaeGuide(copyGuide) ? applyKookdaeGenericReplacements(value.highlightCopy || product.discountInfo || product.mainBenefit || kookdaeFallback.highlightCopy) : value.highlightCopy || product.discountInfo || productUsp.proofSignals[0] || product.mainBenefit || kookdaeFallback.highlightCopy), 28);
  const normalizedBottomBarCopy = trimToLimit(cleanText(isKookdaeGuide(copyGuide) ? applyKookdaeGenericReplacements(value.bottomBarCopy || kookdaeFallback.bottomBarCopy) : value.bottomBarCopy || reference?.finalLabel?.purchaseTrigger || reference?.finalLabel?.whyItWorks || productUsp.situationSignals[0] || productUsp.featureSignals[1] || kookdaeFallback.bottomBarCopy), 36);
  const normalizedCta = normalizeCta(value.cta, copyGuide, context?.adBrief, Boolean(product.discountInfo || product.price));
  ...
  const normalized: GeneratedAdCopy = {
    ...
    whyThisWorks: cleanText(value.whyThisWorks || (isKookdaeGuide(copyGuide) ? "상세페이지에서 확인한 USP를 중심으로 후킹을 만들고 국대한우 가이드는 말투에만 반영했습니다." : "Combined reference copy pattern with product value proof.")),
    messageHierarchy: {
    ...
    reasoning: {
      ...
      selectedKookdaePattern: value.reasoning?.selectedKookdaePattern || selectedKookdaePattern || undefined,
      rejectedGenericExpressions: value.reasoning?.rejectedGenericExpressions || rejectedGenericExpressions,
      productFactsUsed: value.reasoning?.productFactsUsed || (isKookdaeGuide(copyGuide) ? [product.productName, ...productUsp.uspSignals, ...productUsp.offerSignals].filter(Boolean) : undefined),
    },
    templateFit: {
    ...
    copyGuideUsage: value.copyGuideUsage
      ? {
          ...
          selectedPatterns: kookdaeVariantPatternPlan || value.copyGuideUsage.selectedPatterns || baseCopyGuideUsage?.selectedPatterns,
        }
      : baseCopyGuideUsage
        ? {
            ...baseCopyGuideUsage,
    ...
    copyValidation: {
      bodyCopy: {
        ...
        normalized: normalizeBody(value.bodyCopy || "", bodyFallback(product, reference, copyGuide), copyGuide),
        finalLength: visibleLength(normalizeBody(value.bodyCopy || "", bodyFallback(product, reference, copyGuide), copyGuide)),
      },
    },
  ...
  cleaned.copyVariants = buildCopyVariants({ ...cleaned, copyVariants: value.copyVariants }, product, reference, copyGuide, context?.adBrief, context?.creativeStrategy);
  return cleaned;
}
...
function mockCopy(product: ProductInfoForPrompt, labels: AdImageLabel[], copyGuide?: CopyGuideContext | null, adBrief?: AdBrief, creativeStrategy?: CreativeStrategy | null) {
  const reference = labels[0];
  ...
  const strategyEvidence = cleanText(creativeStrategy?.keyAppeal || creativeStrategy?.appeal || objectiveFallback.bodyCopy).replace(/^[^:]{1,18}:\s*/, "");
  ...
  const mockHighlight = creativeStrategy
    ? {
        "problem-solution": (creativeStrategy.expectedCustomerProblem || strategyEvidence).replace(/어려움$/, "어려운 순간").replace(/필요함$/, "필요한 순간"),
...
async function generateWithOpenAI(product: ProductInfoForPrompt, labels: AdImageLabel[], template: TemplateInfo, copyGuide?: CopyGuideContext | null, context?: Pick<Body, "adBrief" | "creativeStrategy" | "referenceUsages" | "referenceLabels">) {
...
export async function POST(request: Request) {
  try {
    ...
    const requestedStrategies = (body.creativeStrategies || []).filter((strategy): strategy is CreativeStrategy => Boolean(strategy?.id)).slice(0, 6);
    const copyStrategies = requestedStrategies.length ? requestedStrategies : body.creativeStrategy ? [body.creativeStrategy] : [];
    ...
    const generatedCopies = copyStrategies.length ? copyStrategies.map((strategy, index) => (index === 0 ? primaryGeneratedCopy : mockCopy(product, selectedLabels, copyGuide, adBrief, strategy))) : [primaryGeneratedCopy];
    const copies = generatedCopies.map((generatedCopy) => applyBriefConstraints(generatedCopy, adBrief, product, selectedLabels, copyGuide));
    const copy = copies[0];
